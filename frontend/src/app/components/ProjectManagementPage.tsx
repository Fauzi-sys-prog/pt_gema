...
export default function ProjectManagementPage() {
  ...
  const handleAddUsage = () => {
    if (!selectedProject || !usageForm || usageForm.qty <= 0) return;
    const so = {
      id: `SO-FR-${Date.now()}`,
  ...
  const handleAddBOQItem = () => {
    ...
    toast.info("Variation Order (VO) added and awaiting Executive Approval.");
  };

  const calculateProjectActuals = (project: Project) => {
    const stockUsageTotal = stockOutList
      .filter(so => so.projectId === project.id)
      .reduce((acc, so) => {
        const itemSum = (so.items || []).reduce((itemAcc, item) => {
          const master = stockItemList.find(s => s.kode === item.kode);
          return itemAcc + (item.qty * (Number(item.hargaSatuan) || master?.hargaSatuan || 0));
        }, 0);
        return acc + itemSum;
      }, 0);
    const laborCosts = attendanceList
      .filter(att => att.projectId === project.id)
      .reduce((acc, att) => {
        const employee = employeeList.find(e => e.id === att.employeeId);
        const hourlyRate = (employee?.salary || 0) / 176;
        const hours = att.workHours || 0;
        const overtimePay = (att.overtime || 0) * hourlyRate * 1.5;
        return acc + (hours * hourlyRate) + overtimePay;
      }, 0);
    const equipmentCosts = (project.equipmentUsage || [])
      .reduce((acc: number, usage: any) => acc + (usage.hoursUsed * usage.costPerHour), 0);

    // Only approved/paid finance transactions are actual costs. Purchase Orders
    // remain commitments and are intentionally excluded to avoid counting the
    // same material again when it is issued through Stock Out.
    const vendorActuals = (expenseList || [])
      .filter((expense: VendorExpense) =>
        expense.projectId === project.id && ['Approved', 'Paid'].includes(expense.status)
      )
      .reduce((sum: number, expense: VendorExpense) => sum + (expense.totalNominal || 0), 0);

    return stockUsageTotal + laborCosts + equipmentCosts + vendorActuals;
  };

  const calculateProjectHPP = (project: Project) =>
    (project.boq || []).reduce(
      (sum: number, item: any) => sum + ((item.qtyEstimate || 0) * (item.unitPrice || 0)),
      0
    );
  ...
  useEscapeKey([
    ...
    { condition: showAddBOQItemModal, close: () => setShowAddBOQItemModal(false) },
      ...
      <div className="bg-white p-2 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col md:flex-row gap-2">
        ...
        <div className="flex gap-2 p-1">
          {["All", "Planning", "In Progress", "Completed"].map((filter) => (
             <button 
               key={filter}
               className="px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-900 hover:text-white transition-all text-slate-400"
             >
               {filter}
             </button>
          ))}
        ...
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-md flex items-center justify-center z-[60] p-2 sm:p-4">
          <div className="bg-white rounded-2xl sm:rounded-[3rem] w-full max-w-6xl h-[95vh] sm:h-[90vh] flex flex-col overflow-hidden shadow-2xl relative">
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-10 bg-slate-50/50">
              {projectTab === "overview" && (() => {
                const spent = calculateProjectActuals(selectedProject);
                const hppBudget = calculateProjectHPP(selectedProject);
                ...
                const actualProfit = selectedProject.nilaiKontrak - spent;
                const actualMargin = selectedProject.nilaiKontrak > 0 ? (actualProfit / selectedProject.nilaiKontrak) * 100 : 0;
                ...
                const budgetUsage = hppBudget > 0 ? (spent / hppBudget) * 100 : 0;
                ...
                // Progress synced with WO tab — both read workOrderList filtered by projectId
                        ...
                        <div className="space-y-3">
                          ...
                          {selectedProject.approvalStatus && (
                            <div>
                              ...
                              <p className="text-sm font-bold text-slate-800 mt-0.5">{selectedProject.approvalStatus}</p>
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
                      ...
                      <div className="space-y-3">
                        ...
                        <button
                          ...
                          disabled={!usageForm || (usageForm?.qty ?? 0) <= 0}
                   ...
                   <div className="bg-white p-4 sm:p-6 md:p-8 rounded-2xl sm:rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                      <div className="flex justify-between items-center mb-8">
                         ...
                         <div className="flex gap-2">
                            {selectedProject.quotationId && (
                              <span className="px-3 py-2 bg-slate-50 text-slate-400 border border-slate-100 rounded-xl text-[9px] font-black uppercase tracking-widest">
                                Item baru = Variation Order
                              </span>
                            )}
              ...
              {projectTab === "vendor-biaya" && (() => {
                ...
                const expenseByCategory = projectExpenses.reduce((acc, expense) => {
                  const category = expense.kategori || 'Biaya Lain';
                ...
                return (
                  <div className="animate-in fade-in duration-500 space-y-5">
                    ...
                    {/* Category recap mirrors the category selected in Finance. */}
                    {Object.keys(expenseByCategory).length > 0 && (
                      <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
                        <div className="px-5 py-4 border-b border-slate-100">
                          ...
                          <p className="text-[9px] text-slate-400 font-bold mt-0.5">Otomatis dari kategori yang dipilih saat input biaya di Finance</p>
                        </div>
                        <div className="overflow-x-auto">
                                  ...
                                  <tr key={inv.id} className="text-xs font-bold hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td className="p-4 text-center">
                                      {!isLunas && (
                                        <button
                                          onClick={() => {
                                            const amount = prompt(`Masukkan jumlah pembayaran untuk ${inv.noInvoice || inv.id}:`);
                                            if (amount && !isNaN(Number(amount))) {
                                              addInvoicePayment(inv.id, Number(amount));
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[70] p-4">
          <div className="bg-white rounded-[2rem] w-full max-w-lg overflow-hidden shadow-2xl max-h-[90vh] flex flex-col">
            ...
            <div className="p-5 bg-slate-50 border-t border-slate-100 flex justify-end gap-3 shrink-0">
              ...
              <button onClick={handleAddWorkOrder} className="px-6 py-3 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg">Create Work Order</button>
            </div>
          </div>
        </div>
      )}
