...
import { useApp, type StockItem } from '../contexts/AppContext';
...
export function QRScannerModal({ isOpen, onClose, onScanSuccess }: QRScannerModalProps) {
  ...
  const handleSimulateScan = () => {
    ...
    setTimeout(() => {
      const randomItem = stockItemList[Math.floor(Math.random() * stockItemList.length)];
      setScanResult(randomItem);
      setIsScanning(false);
      toast.success(`Item detected: ${randomItem.nama}`);
    }, 1500);
  };
