...
import { Box, X, ChevronDown, Save, Info, CheckCircle2, Plus, Zap } from 'lucide-react';
...
export const SKURegistrationModal: React.FC<SKURegistrationModalProps> = ({ isOpen, onClose, initialName = '' }) => {
  ...
  const handleSave = () => {
    ...
    if (isNameConflict) {
      if (!confirm(`Peringatan: Nama barang "${formData.nama}" sudah ada. Tetap simpan sebagai varian baru?`)) {
        return;
      }
    }
    ...
    if (Number(formData.stokAwal) > 0) {
      setStockMovementList(prev => [...prev, {
        id: `MOV-${Date.now()}`,
        tanggal: tanggal,
        type: "IN",
        refNo: "SALDO-AWAL",
