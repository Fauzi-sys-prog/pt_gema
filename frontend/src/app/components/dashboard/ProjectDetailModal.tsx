import { X, Printer, FileDown, FileSpreadsheet, Calendar, MapPin, Users, Package, DollarSign, Activity, GitCommit, CheckCircle2, Clock } from 'lucide-react';
...
export function ProjectDetailModal({ project, isOpen, onClose }: ProjectDetailModalProps) {
  ...
  const handleExportToWord = async () => {
    ...
    try {
      if (logoGTP.startsWith('data:')) {
        const base64Data = logoGTP.split(',')[1];
        const binaryString = window.atob(base64Data);
        const bytes = new Uint8Array(binaryString.length);
        for (let i = 0; i < binaryString.length; i++) {
          bytes[i] = binaryString.charCodeAt(i);
        }
        logoData = bytes.buffer;
      } else {
        const response = await fetch(logoGTP);
        if (response.ok) {
    ...
    const subtotal = project.boq.reduce((sum, item) => {
      ...
      sections: [{
        ...
        children: [
          ...
          new Table({
            ...
            borders: {
              top: { style: BorderStyle.NONE }, bottom: { style: BorderStyle.SINGLE, size: 6, color: "000000" },
                  ...
                  new TableCell({
                    ...
                    children: [
                      new Paragraph({
                        ...
                        children: [
                          new TextRun({ text: "GEMA TEKNIK PERKASA", bold: true, size: 24, font: "Arial" }),
                        ],
                      }),
                      new Paragraph({
                        alignment: AlignmentType.RIGHT,
                        children: [
                          new TextRun({ text: "REFRACTORY FURNACE AND BOILER", size: 14, font: "Arial", italic: true }),
                        ],
                      }),
                      new Paragraph({
                        alignment: AlignmentType.RIGHT,
                        children: [
                          new TextRun({ text: "Jl. Nurushshoba II No 13 Setia Mekar Tambun Selatan Bekasi 17510", size: 12, font: "Arial" }),
                        ],
                      }),
                      new Paragraph({
                        alignment: AlignmentType.RIGHT,
                        children: [
                          new TextRun({ text: "Phone: 085100420221, 021.88354139 | Fax: 021.88354139", size: 10, font: "Arial" }),
                        ],
                      }),
                    ],
                  }),
          ...
          new Table({
            ...
            rows: [
              new TableRow({
                children: [
                  new TableCell({ width: { size: 10, type: WidthType.PERCENTAGE }, children: [new Paragraph({ children: [new TextRun({ text: "No", size: 18, font: "Arial" })] })] }),
                  new TableCell({ width: { size: 2, type: WidthType.PERCENTAGE }, children: [new Paragraph({ children: [new TextRun({ text: ":", size: 18, font: "Arial" })] })] }),
                  new TableCell({ width: { size: 88, type: WidthType.PERCENTAGE }, children: [new Paragraph({ children: [new TextRun({ text: project.kodeProject || "-", bold: true, size: 18, font: "Arial" })] })] }),
                ],
              }),
              new TableRow({
                children: [
                  new TableCell({ width: { size: 10, type: WidthType.PERCENTAGE }, children: [new Paragraph({ children: [new TextRun({ text: "Perihal", size: 18, font: "Arial" })] })] }),
                  new TableCell({ width: { size: 2, type: WidthType.PERCENTAGE }, children: [new Paragraph({ children: [new TextRun({ text: ":", size: 18, font: "Arial" })] })] }),
                  new TableCell({ width: { size: 88, type: WidthType.PERCENTAGE }, children: [new Paragraph({ children: [new TextRun({ text: project.namaProject || "-", bold: true, size: 18, font: "Arial" })] })] }),
                ],
              }),
            ],
          }),
          ...
          new Paragraph({
            children: [
              new TextRun({ text: project.customer || "-", bold: true, size: 18, font: "Arial" }),
            ],
          }),
          new Paragraph({
            children: [
              new TextRun({ text: project.location || "-", size: 16, font: "Arial" }),
            ],
          }),
            ...
            rows: [
              ...
              new TableRow({
                ...
                children: [
                  ...
                  children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: h, bold: true, size: 18, font: "Arial" })] })],
                  verticalAlign: VerticalAlign.CENTER,
                  shading: { fill: "f3f4f6" }
                })),
              }),
              // Items
              ...project.boq.map((item, index) => new TableRow({
                children: [
                  new TableCell({ verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: String(index + 1), size: 18, font: "Arial" })] })] }),
                  new TableCell({ 
                    children: [
                      new Paragraph({ children: [new TextRun({ text: item.materialName, bold: true, size: 18, font: "Arial" })] }),
                      new Paragraph({ children: [new TextRun({ text: `Supplier: ${item.supplier || '-'}`, italic: true, size: 14, font: "Arial", color: "666666" })] }),
                    ] 
                  }),
                  new TableCell({ verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: formatCurrency(item.unitPrice), size: 18, font: "Arial" })] })] }),
                  new TableCell({ verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: `${item.qtyEstimate} ${item.unit}`, size: 18, font: "Arial" })] })] }),
                  new TableCell({ verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ alignment: AlignmentType.RIGHT, children: [new TextRun({ text: formatCurrency(item.qtyEstimate * item.unitPrice), size: 18, font: "Arial" })] })] }),
                ],
              })),
              // Totals
              new TableRow({
                children: [
                  new TableCell({ columnSpan: 4, children: [new Paragraph({ alignment: AlignmentType.RIGHT, children: [new TextRun({ text: "Sub harga", size: 18, font: "Arial" })] })] }),
                  new TableCell({ children: [new Paragraph({ alignment: AlignmentType.RIGHT, children: [new TextRun({ text: formatCurrency(subtotal), size: 18, font: "Arial" })] })] }),
                ],
              }),
              new TableRow({
                children: [
                  new TableCell({ columnSpan: 4, children: [new Paragraph({ alignment: AlignmentType.RIGHT, children: [new TextRun({ text: "PPN 11%", size: 18, font: "Arial" })] })] }),
                  new TableCell({ children: [new Paragraph({ alignment: AlignmentType.RIGHT, children: [new TextRun({ text: formatCurrency(ppn), size: 18, font: "Arial" })] })] }),
                ],
              }),
              new TableRow({
                children: [
                  new TableCell({ columnSpan: 4, children: [new Paragraph({ alignment: AlignmentType.RIGHT, children: [new TextRun({ text: "Grand Total", bold: true, size: 18, font: "Arial" })] })] }),
                  new TableCell({ children: [new Paragraph({ alignment: AlignmentType.RIGHT, children: [new TextRun({ text: formatCurrency(grandTotal), bold: true, size: 18, font: "Arial" })] })] }),
                ],
              }),
            ],
          }),
          ...
          new Paragraph({
            children: [
              new TextRun({ text: `Bekasi, ${new Date().toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })}`, size: 18, font: "Arial" }),
            ],
          }),
          new Paragraph({
            children: [
          ...
          new Paragraph({
            children: [
              new TextRun({ text: project.projectManager || 'Project Manager', bold: true, underline: {}, size: 18, font: "Arial" }),
            ],
          }),
        ],
      }],
    ...
    FileSaver.saveAs(blob, `PROJECT_BOQ_${project.kodeProject}.docx`);
  ...
  const handleExportToExcel = () => {
    ...
    csvContent += `GEMA TEKNIK PERKASA - PROJECT BOQ\n`;
    csvContent += `REFRACTORY FURNACE AND BOILER\n`;
    csvContent += `\n`;
    csvContent += `No:,${project.kodeProject}\n`;
    csvContent += `Perihal:,${project.namaProject}\n`;
    csvContent += `Customer:,${project.customer}\n`;
    csvContent += `Tanggal:,${project.startDate}\n`;
    csvContent += `Project Manager:,${project.projectManager || '-'}\n`;
    csvContent += `\n`;
    csvContent += "No,Material Name,Unit Price (Rp),Quantity,Unit,Total Price (Rp),Supplier,Status\n";
    
    let totalEstimate = 0;
    project.boq.forEach((item, index) => {
      const itemTotal = (item.qtyEstimate || 0) * (item.unitPrice || 0);
      totalEstimate += itemTotal;
      csvContent += `${index + 1},"${item.materialName}",${item.unitPrice},${item.qtyEstimate},${item.unit},${itemTotal},"${item.supplier}",${item.status}\n`;
    });

    csvContent += `\n`;
    csvContent += `Sub Total:,,,,,${totalEstimate}\n`;
    const ppn = totalEstimate * 0.11;
    csvContent += `PPN 11%:,,,,,${ppn}\n`;
    const grandTotal = totalEstimate + ppn;
    csvContent += `Grand Total:,,,,,${grandTotal}\n`;

    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", `BOQ_${project.kodeProject}_${project.customer.replace(/\s+/g, '_')}.csv`);
