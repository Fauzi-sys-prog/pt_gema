...
import {
  ...
  generateSPKWord,
  printCorrespondence,
} from "../../utils/correspondenceWordExports";
...
const JENIS_SPK_CONFIG: Record<
  JenisSPK,
  {
    label: string;
    icon: React.ReactNode;
    color: string;
    defaultJamMasuk: string;
    defaultJamKeluar: string;
...
> = {
  Biasa: {
    ...
    defaultJamKeluar: "17:00",
    inactiveHover: "hover:text-blue-600 hover:border-blue-200",
  ...
  Lembur: {
    ...
    icon: <Moon size={14} />,
    color: "amber",
    defaultJamMasuk: "17:00",
    defaultJamKeluar: "22:00",
    inactiveHover: "hover:text-amber-500 hover:border-amber-200",
  ...
  Daily: {
    ...
    icon: <Hash size={14} />,
    color: "emerald",
    defaultJamMasuk: "07:00",
    defaultJamKeluar: "16:00",
    inactiveHover: "hover:text-emerald-600 hover:border-emerald-200",
    activeText: "text-white",
  },
};
...
export default function SuratPerintahKerjaPage() {
  ...
  const updateTeknisiRow = (i: number, patch: Partial<TeknisiRow>) =>
    set({
      teknisiRows: fd.teknisiRows.map((r, idx) =>
        idx === i ? { ...r, ...patch } : r,
      ),
    });
  const addTeknisiRow = () =>
    set({ teknisiRows: [...fd.teknisiRows, emptyTeknisiRow()] });
  ...
  const productionWorkflowStatus = (spk: any): string => {
    if (
      spk.jenisSPK !== "Biasa" ||
      !["Approved", "Active", "In Progress", "Pending QC"].includes(spk.status)
    )
      return spk.status;
    ...
    const qcPassed = qcInspectionList.some(
      (item) =>
        item.workOrderId === workOrder.id &&
        item.status === "Passed" &&
        item.qtyPassed >= workOrder.targetQty,
    );
    return qcPassed ? "Completed" : "Pending QC";
  };

  // Derived SPK list
  const spkList = projectList
    .flatMap((project) =>
      (project.spkList || []).map((spk: any) => ({
        ...
        teknisi: Array.isArray(spk.teknisi)
          ? spk.teknisi
          : spk.teknisi
            ? spk.teknisi.split(",").map((t: string) => t.trim())
            : [],
      })),
    )
    .sort(
  ...
  const handleCreateSPK = async () => {
    ...
    const project = isInternal
      ? null
      : projectList.find((p) => p.id === fd.projectId);
    const spkCount = project ? project.spkList?.length || 0 : spkList.length;

    const autoNo =
      fd.noSPK ||
      `SPK/${new Date().getFullYear()}/${fd.jenisSPK.toUpperCase().slice(0, 3)}/${String(spkCount + 1).padStart(3, "0")}`;
    const duplicateNo = spkList.some(
      (item) =>
        (item.noSPK || "").trim().toLowerCase() === autoNo.trim().toLowerCase(),
    );
    if (duplicateNo) {
    ...
    try {
      if (project)
        await createProjectSpkWithWorkOrder(project.id, newSPK, newWO);
    ...
    toast.success(
      `SPK ${autoNo} disimpan sebagai draft. Submit untuk meminta approval.`,
    );
    setIsSubmitting(false);
  ...
  const filteredSPK = spkList.filter(
    (spk) =>
  ...
  const canApprove = ["Owner", "Admin", "Manager", "SPV"].includes(
    currentUser?.role || "",
  );
  const updateSPKStatus = (
    spk: any,
    status:
      "Pending Approval" | "Approved" | "Rejected" | "Completed" | "Closed",
  ) => {
    if (isSubmitting) return;
    if (
      status === "Rejected" &&
      !window.confirm(
        `Tolak SPK ${spk.noSPK}? Status akan berubah menjadi Rejected.`,
      )
    )
      return;
    ...
    if (!project) {
      toast.error(
        "SPK internal belum dapat diubah statusnya karena belum terhubung ke proyek.",
      );
      return;
    }
    setIsSubmitting(true);
    ...
    const patch =
      status === "Approved"
        ? {
            status,
            approvedBy: currentUser?.fullName || "Owner",
            approvedAt: now,
          }
        : status === "Rejected"
          ? {
              status,
              rejectedBy: currentUser?.fullName || "Owner",
              rejectedAt: now,
            }
          : status === "Completed"
            ? {
                status,
                completedBy: currentUser?.fullName || "Current User",
                completedAt: now,
              }
            : status === "Closed"
              ? {
                  status,
                  closedBy: currentUser?.fullName || "Owner",
                  closedAt: now,
                }
              : {
                  status,
                  submittedBy: currentUser?.fullName || "Current User",
                  submittedAt: now,
                };
    try {
      updateProject(project.id, {
        ...project,
        spkList: (project.spkList || []).map((item: any) =>
          item.id === spk.id ? { ...item, ...patch } : item,
        ),
      });
      toast.success(`SPK ${spk.noSPK} → ${status}`);
    } catch (err) {
      toast.error(
        "Status SPK gagal diubah: " +
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 font-medium italic">
              Instruksi & Penugasan Workshop PT Gema Teknik Perkasa
            </p>
          </div>
        </div>
        ...
        {[
          {
            ...
            val: spkList.filter((s) => s.workflowStatus === "Pending Approval")
              .length,
            icon: Clock,
            color: "amber",
          },
          {
            label: "SPK Aktif",
            val: spkList.filter((s) =>
              ["Approved", "Active", "In Progress", "Pending QC"].includes(
                s.workflowStatus,
              ),
            ).length,
            icon: Zap,
          ...
          {
            label: "Selesai, Belum Tutup",
            val: spkList.filter((s) => s.workflowStatus === "Completed").length,
            icon: CheckCircle2,
            color: "emerald",
          },
          {
            label: "Prioritas Tinggi",
            val: spkList.filter((s) => s.urgent).length,
            icon: Zap,
            color: "rose",
          },
        ].map((stat, i) => (
          <div
            key={i}
            className="bg-white border-2 border-slate-100 rounded-2xl p-6 shadow-sm flex items-center justify-between"
          >
            <div>
            ...
            <div
              className={`p-4 bg-${stat.color}-50 text-${stat.color}-600 rounded-2xl`}
            >
              <stat.icon size={24} />
            </div>
          </div>
                  ...
                  >
                    <td className="px-8 py-6">
                      <div className="flex flex-col">
                        <span className="text-xs font-black text-blue-600 italic tracking-tighter uppercase">
                          {spk.noSPK}
                        </span>
                      ...
                      <div className="flex flex-col">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-[9px] font-bold text-slate-400 uppercase italic">
                            Project: {spk.projectName}
                          </span>
                          {spk.urgent && (
                            <span className="inline-flex text-[8px] font-black text-rose-600 uppercase italic">
                              ⚠️ Prioritas Tinggi
                            </span>
                          )}
                        </div>
                    ...
                    <td className="px-8 py-6">
                      <div className="flex justify-center -space-x-2">
                        {spk.teknisi.length > 0 ? (
                          spk.teknisi.map((t: string, idx: number) => (
                            <div
                              key={idx}
                              className="w-8 h-8 rounded-full bg-slate-900 border-2 border-white flex items-center justify-center text-[10px] font-black text-white shadow-sm"
                              title={t}
                            >
                    ...
                    <td className="px-8 py-6 text-center">
                      <span
                        className={`inline-flex px-3 py-1 rounded-full text-[9px] font-black uppercase border-2 ${
                          ["Approved", "Active", "In Progress"].includes(
                            workflowStatus,
                          )
                            ? "bg-blue-50 text-blue-600 border-blue-100"
                            : workflowStatus === "Pending Approval" ||
                                workflowStatus === "Pending QC"
                    ...
                    <td className="px-8 py-6 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {workflowStatus === "Draft" && (
                          <button
                            onClick={() =>
                              updateSPKStatus(spk, "Pending Approval")
                            }
                            className="px-2.5 py-2 text-[9px] font-black uppercase text-amber-700 bg-amber-50 hover:bg-amber-100 rounded-xl"
                        ...
                        {workflowStatus === "Pending Approval" &&
                          canApprove && (
                            <>
                              <button
                                onClick={() => updateSPKStatus(spk, "Approved")}
                                className="px-2.5 py-2 text-[9px] font-black uppercase text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-xl"
                                title="Setujui SPK"
                              >
                                Setujui
                              </button>
                              <button
                                onClick={() => updateSPKStatus(spk, "Rejected")}
                        ...
                        {!isProductionSPK &&
                          (workflowStatus === "Approved" ||
                            ...
                            <button
                              onClick={() => updateSPKStatus(spk, "Completed")}
                              className="px-2.5 py-2 text-[9px] font-black uppercase text-blue-700 bg-blue-50 hover:bg-blue-100 rounded-xl"
                              title="Tandai realisasi selesai"
                            >
                              Selesai
                            </button>
                          )}
                        {workflowStatus === "Completed" && canApprove && (
                          <button
                            onClick={() => updateSPKStatus(spk, "Closed")}
                            className="px-2.5 py-2 text-[9px] font-black uppercase text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-xl"
                            title="Tutup SPK"
                          >
                            Tutup
                          </button>
          ...
          <div className="bg-white w-full max-w-2xl rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in duration-300 border-4 border-slate-200 text-slate-900 flex flex-col max-h-[92vh]">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-tight">
                    Instruksi Kerja · PT Gema Teknik Perkasa
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="overflow-y-auto p-8 space-y-5">
                    ...
                    return (
                      <button
                        ...
                        onClick={() =>
                          set({
                            jenisSPK: j,
                            jamMasuk: cfg.defaultJamMasuk,
                            jamKeluar: cfg.defaultJamKeluar,
                          })
                        }
              ...
              <div>
                ...
                <select
                  value={fd.requiresBom ? "yes" : "no"}
                  onChange={(e) =>
                    set({ requiresBom: e.target.value === "yes" })
                  }
                  className="w-full px-5 py-3.5 bg-slate-50 border-2 border-slate-100 rounded-2xl text-xs font-black uppercase italic focus:border-blue-500 outline-none transition-all text-black"
                >
                  <option value="yes">
                    Membutuhkan BOM / Material Inventory
                  </option>
                  <option value="no">
                    Tanpa BOM / Jasa / Pekerjaan Tanpa Material
                  </option>
                </select>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {fd.teknisiRows.map((row, i) => (
                    <div
                      key={i}
                      className="grid grid-cols-[2rem_1fr_1fr_5rem_2rem] gap-2 items-center"
                    >
              ...
              <div className="flex items-center gap-3 pt-1">
                ...
                <label
                  htmlFor="urgent-spk"
                  className="text-[10px] font-black uppercase italic text-rose-600 tracking-widest cursor-pointer select-none"
                >
          ...
          <div className="bg-white w-full max-w-5xl rounded-[3rem] overflow-hidden shadow-2xl animate-in zoom-in duration-300 border-4 border-slate-200">
            <div className="p-6 bg-slate-50 border-b-2 border-slate-100 flex justify-between items-center">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-tight">
                    Verified Scan - PT Gema Teknik Perkasa
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-8 bg-slate-100 flex items-center justify-center overflow-auto h-[70vh] custom-scrollbar">
              <iframe
                title="Preview SPK"
                srcDoc={previewHtml}
                className="bg-white w-full max-w-3xl h-full shadow-2xl border border-slate-200"
              />
            </div>
            <div className="p-6 bg-white border-t-2 border-slate-50 flex justify-end gap-3">
              <div className="mr-auto flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-xl border-2 border-slate-100">
                ...
                <span className="text-[10px] font-black text-slate-500 uppercase italic">
                  Digital Signature: VERIFIED by Syamsudin
                </span>
              </div>
              <button
                onClick={() => selectedSPK && downloadSPK(selectedSPK)}
                className="px-8 py-3 bg-emerald-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-emerald-700 transition-all shadow-lg flex items-center gap-2"
              >
                <Download size={15} /> Download Word
              </button>
              <button
                onClick={() => previewHtml && printCorrespondence(previewHtml)}
                className="px-8 py-3 bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-800 transition-all shadow-lg"
              >
                Print Document
              </button>
              <button
                className="px-8 py-3 border-2 border-slate-200 text-slate-600 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-50 transition-all"
                onClick={() => setShowPreview(false)}
              >
                Tutup
              </button>
