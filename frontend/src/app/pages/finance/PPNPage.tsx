...
import { downloadSPTMasaWordDocument } from '../../components/PPNWordExport';
...
export default function PPNPage() {
  const { invoiceList, customerInvoiceList = [], vendorInvoiceList, projectList } = useApp();
  ...
  // Process PPN Keluaran — gabungkan customerInvoiceList (AR) + invoiceList yang belum masuk AR
  const ppnKeluaran = useMemo(() => {
    // AR entries (primary, after approval/payment)
    const arNos = new Set(customerInvoiceList.map(i => i.noInvoice));
    const fromAR = customerInvoiceList
      .filter(inv => inv.status !== 'Cancelled' && inv.ppn > 0)
      .map(inv => ({
        id: inv.id,
        tanggal: inv.tanggal,
        nomor: inv.noInvoice,
        noFaktur: (inv as any).noFakturPajak,
        pihak: inv.customerName,
        dpp: inv.subtotal || 0,
        ppn: inv.ppn || 0,
        status: inv.status,
        tipe: 'Sales',
    ...
    // invoiceList entries not yet in AR (Draft/Sent)
    const fromInvoice = invoiceList
      .filter(inv => !arNos.has(inv.noInvoice) && inv.status !== 'Paid' && inv.ppn > 0)
      .map(inv => ({
        id: inv.id,
        tanggal: inv.tanggal,
        nomor: inv.noInvoice,
  ...
  const totalKeluaran = useMemo(() => ppnKeluaran.reduce((sum, item) => sum + item.ppn, 0), [ppnKeluaran]);
  const totalMasukan = useMemo(() => ppnMasukan.reduce((sum, item) => sum + item.ppn, 0), [ppnMasukan]);
  ...
  // Monthly Chart Data — grouped from real PPN transactions
  const chartData = useMemo(() => {
    const MONTHS = ['Jan','Feb','Mar','Apr','Mei','Jun','Jul','Agu','Sep','Okt','Nov','Des'];
    const keluaranByMonth = new Array(12).fill(0);
    const masukanByMonth = new Array(12).fill(0);

    ppnKeluaran.forEach(item => {
      if (!item.tanggal) return;
      keluaranByMonth[new Date(item.tanggal).getMonth()] += item.ppn;
    });
    ppnMasukan.forEach(item => {
      if (!item.tanggal) return;
      masukanByMonth[new Date(item.tanggal).getMonth()] += item.ppn;
    });

    return MONTHS.map((name, i) => ({ name, keluaran: keluaranByMonth[i], masukan: masukanByMonth[i] }))
      .filter(d => d.keluaran > 0 || d.masukan > 0);
  }, [ppnKeluaran, ppnMasukan]);
  ...
  const filteredData = useMemo(() => {
    ...
    return all
      ...
      .filter(item => 
        ...
        (item.noFaktur && item.noFaktur.toLowerCase().includes(searchTerm.toLowerCase()))
      )
      .sort((a, b) => new Date(b.tanggal).getTime() - new Date(a.tanggal).getTime());
  }, [ppnKeluaran, ppnMasukan, activeTab, searchTerm]);

  const handleGenerateSPTMasa = () => {
    const ppnData = {
      transactions: filteredData.map(item => ({
        date: item.tanggal,
        invoiceNo: item.nomor,
        description: item.pihak,
        type: item.tipe === 'Sales' ? 'OUT' : 'IN',
        amount: item.dpp + item.ppn,
        ppnAmount: item.ppn
      })),
      period: periodLabel,
      totalPPNIn: totalMasukan,
      totalPPNOut: totalKeluaran,
      netPPN: netPPN
    };
    
    const filename = `SPT_Masa_PPN_${new Date().toISOString().split('T')[0]}.doc`;
    try {
      downloadSPTMasaWordDocument(ppnData, filename);
      toast.success('SPT Masa PPN berhasil dibuat');
    } catch (err) {
      toast.error('Gagal membuat SPT Masa: ' + (err instanceof Error ? err.message : 'Unknown error'));
    }
  };
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap gap-3">
          <button
            onClick={() => {
              try {
                const rows = [['Tanggal','No Invoice','Pihak','Tipe','DPP','PPN']];
                filteredData.forEach(d => rows.push([d.tanggal, d.nomor, d.pihak, d.tipe, String(d.dpp), String(d.ppn)]));
                const csv = rows.map(r => r.map(c => `"${c}"`).join(',')).join('\n');
                const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a'); a.href = url; a.download = `PPN_${periodLabel}.csv`; a.click();
                URL.revokeObjectURL(url);
                toast.success('CSV PPN berhasil diexport');
              } catch (err) {
                toast.error('Gagal export CSV: ' + (err instanceof Error ? err.message : 'Unknown error'));
              }
            }}
          ...
          <button className="px-8 py-4 bg-red-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-red-700 transition-all flex items-center gap-2 shadow-xl shadow-red-200"
            onClick={handleGenerateSPTMasa}
          >
            <FileCheck2 size={16} /> Generate SPT Masa
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          ...
          <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase">
             ...
             <span>Terintegrasi dengan {ppnKeluaran.length} Invoices</span>
          </div>
        </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm">
           <div className="flex items-center justify-between mb-10">
              ...
              <select className="bg-slate-50 border-none rounded-xl px-4 py-2 text-[10px] font-black uppercase text-black outline-none focus:ring-2 focus:ring-red-500">
           ...
           <div className="h-[300px] w-full min-w-0 min-h-0 overflow-hidden flex flex-col relative">
              <ResponsiveContainer width="100%" height={300} minWidth={0} minHeight={0}>
                <BarChart data={chartData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                  ...
                  />
                  <Bar key="ppn-keluaran" name="Keluaran" dataKey="keluaran" isAnimationActive={false} fill="#EF4444" radius={[4, 4, 0, 0]} barSize={25} />
                  <Bar key="ppn-masukan" name="Masukan" dataKey="masukan" isAnimationActive={false} fill="#6366F1" radius={[4, 4, 0, 0]} barSize={25} />
                </BarChart>
              </ResponsiveContainer>
           </div>
        </div>
                ...
                <tr key={`${item.id}-${item.tipe}`} className="hover:bg-slate-50/30 transition-colors">
                  <td className="px-10 py-8">
                    <div className="flex flex-col">
                       ...
                       <span className={`text-[9px] font-bold mt-1 uppercase ${item.tipe === 'Sales' ? 'text-rose-500' : 'text-indigo-500'}`}>
                         {item.tipe === 'Sales' ? 'Pajak Keluaran' : 'Pajak Masukan'}
                       </span>
                    </div>
                  </td>
                  <td className="px-10 py-8">
                  ...
                  <td className="px-10 py-8 text-right">
                    <span className={`text-sm font-black italic ${item.tipe === 'Sales' ? 'text-red-600' : 'text-indigo-600'}`}>
                      {formatCurrency(item.ppn)}
                    </span>
                  </td>
                  <td className="px-10 py-8 text-center">
                    <button className="p-3 bg-white border border-slate-100 text-slate-400 hover:text-red-600 rounded-xl transition-all shadow-sm">
