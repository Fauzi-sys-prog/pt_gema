...
export default function QCInspectionPage() {
  ...
  const warehouseCategories = getWarehouseCategories(
    stockItemList,
    extraCategories || [],
  );
  ...
  const handleDrawingUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () =>
      setNewInspection((prev) => ({
        ...prev,
        drawingUrl: reader.result as string,
      }));
  ...
  const waitingForQC = workOrderList.filter((wo) => {
    ...
    const linkedOutput = productionReportList
      .filter(
        (report) =>
          (report as any).workOrderId === wo.id ||
          (report as any).woNumber === wo.woNumber,
      )
      .reduce((sum, report) => sum + (Number(report.outputQty) || 0), 0);
    return linkedOutput >= (Number(wo.targetQty) || 0) && linkedOutput > 0;
  });
  const manualLHPQueue = productionReportList.filter(
    (r) => !r.woNumber && (r as any).qcStatus === "Pending",
  );
  const [qcTab, setQcTab] = useState<"wo" | "manual">("wo");

  const filteredInspections = qcInspectionList.filter(
    (inspection) =>
  ...
  const handleOpenInspection = (wo: WorkOrder) => {
    ...
    const linkedOutputQty = productionReportList
      ...
      .reduce((sum, report) => sum + (Number(report.outputQty) || 0), 0);
    const actualQty =
      linkedOutputQty || Number(wo.completedQty || wo.targetQty || 0);
    setNewInspection({
      tanggal: new Date().toISOString().split("T")[0],
      visualCheck: true,
      dimensionCheck: true,
      ...
      warehouseLocation: "Gudang Barang Jadi",
      stockCategory: warehouseCategories.includes("Barang Jadi")
        ? "Barang Jadi"
  ...
  const handleRemoveDimension = (index: number) => {
    const updatedDimensions = newInspection.dimensions?.filter(
      (_, i) => i !== index,
    );
  ...
  const handleDimensionChange = (
    index: number,
    field: keyof DimensionMeasurement,
    value: string,
  ) => {
  ...
  const handleSubmitQC = async () => {
    ...
    if (
      ...
      !newInspection.stockCategory
    ) {
      toast.error(
        "Pilih kategori stok yang sudah terdaftar di Monitoring Gudang",
      );
      return;
    }
    ...
    if (
      ...
      qtyPassed < 0 ||
      qtyRejected < 0 ||
      qtyPassed + qtyRejected !== qtyInspected
    ) {
    ...
    const inspection: QCInspection = {
      ...
      warehouseLocation:
        newInspection.warehouseLocation || "Gudang Barang Jadi",
    ...
    if (inspection.status === "Rejected") {
      toast.error(
        `QC Rejected untuk ${inspection.batchNo} — dikembalikan ke produksi.`,
      );
    } else if (inspection.sendToWarehouse && inspection.qtyPassed > 0) {
      toast.success(
        `QC ${inspection.status} tersimpan — draft Stock In Barang Jadi menunggu verifikasi gudang.`,
      );
    } else {
      toast.success(
        `QC ${inspection.status} tersimpan — pekerjaan jasa tidak masuk gudang.`,
      );
    }
  };
  ...
  const handleManualQC = (report: any, result: "Passed" | "Failed") => {
    ...
    if (
      result === "Failed" &&
      !window.confirm(
        `Tandai LHP ${report.id.slice(-6)} sebagai QC Failed dan kembalikan ke produksi?`,
      )
    )
      return;
    ...
    try {
      ...
      if (result === "Passed" && report.outputQty > 0) {
        toast.success(
          `QC Passed — LHP manual tersimpan. Hubungkan ke WO terlebih dahulu jika hasilnya masuk gudang.`,
        );
      } else {
        toast.error(
          `QC Failed — LHP ${report.id.slice(-6)} dikembalikan ke produksi`,
        );
      }
    } catch (err) {
      toast.error(
        "Gagal memperbarui status QC: " +
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">
              Inspection, Verification & Release
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-indigo-50 border border-indigo-100 rounded-2xl px-5 py-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          ...
          <p className="text-sm font-black text-slate-800">
            QC = putuskan hasil produksi diterima atau perlu rework
          </p>
          <p className="text-xs text-slate-500 mt-1">
            QC Passed dapat membuat Draft Stok Masuk Barang Jadi. QC Rejected
            mengembalikan WO ke produksi.
          </p>
        </div>
        <span className="text-[10px] font-black uppercase text-indigo-700 bg-white border border-indigo-200 px-3 py-2 rounded-xl">
          QC → Stok Masuk / Rework
        </span>
      </div>
          ...
          <div className="flex bg-slate-100 rounded-2xl p-1 gap-1">
            ...
            >
              ...
              {waitingForQC.length > 0 && (
                <span className="bg-orange-500 text-white rounded-full w-4 h-4 flex items-center justify-center text-[9px]">
                  {waitingForQC.length}
                </span>
              )}
            </button>
            ...
            >
              ...
              {manualLHPQueue.length > 0 && (
                <span className="bg-rose-500 text-white rounded-full w-4 h-4 flex items-center justify-center text-[9px]">
                  {manualLHPQueue.length}
                </span>
              )}
            </button>
          </div>
            ...
            {qcTab === "wo" ? (
              waitingForQC.length > 0 ? (
                waitingForQC.map((wo) => (
                  <div
                    key={wo.id}
                    className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:border-indigo-400 transition-all group"
                  >
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <span
                        className={`text-[9px] font-black px-2 py-0.5 rounded-full uppercase ${wo.priority === "Urgent" ? "bg-red-100 text-red-600" : "bg-blue-100 text-blue-600"}`}
                      >
                        {wo.priority}
                      </span>
                    </div>
              ...
              manualLHPQueue.map((report) => (
                <div
                  key={report.id}
                  className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:border-rose-300 transition-all"
                >
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex-1 min-w-0">
                      <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        LHP Manual — {report.tanggal}
                      </span>
                      <h4 className="text-sm font-black text-slate-900 leading-tight mt-0.5 truncate">
                        {(report as any).namaBarang || report.activity}
                      </h4>
                    </div>
                    <span className="text-[9px] font-black px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 uppercase ml-2 shrink-0">
                      Pending QC
                    </span>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 py-2 bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase hover:bg-emerald-700 transition-all flex items-center justify-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                    ...
                    <button
                      ...
                      disabled={processingId !== null}
                      className="flex-1 py-2 bg-rose-50 text-rose-600 border border-rose-200 rounded-xl text-[10px] font-black uppercase hover:bg-rose-100 transition-all flex items-center justify-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
        ...
        <div className="xl:col-span-2 space-y-4">
          <div className="flex items-center justify-between px-2">
            <h3 className="font-bold text-slate-900 flex items-center gap-2">
              <ClipboardCheck size={18} className="text-indigo-600" /> Riwayat
              Inspeksi
            </h3>
                  ...
                  {filteredInspections.length > 0 ? (
                    filteredInspections.map((qc) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-right">
                          <span
                            className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase inline-flex items-center gap-1.5 ${
                              qc.status === "Passed"
                    ...
                    <tr>
                      <td colSpan={6} className="px-6 py-12 text-center">
                        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                          Belum ada data inspeksi terbaru
                        </p>
                      </td>
                    </tr>
                  )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-[2.5rem] w-full max-w-6xl relative z-10 overflow-hidden shadow-2xl flex flex-col max-h-[90vh] border border-slate-200">
            ...
            <div className="p-8 overflow-y-auto space-y-6">
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                  Foto Drawing / Referensi (Opsional)
                </label>
                <input
                  ref={drawingFileRef}
                  type="file"
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs">
                      <thead className="bg-slate-50">
                        <tr>
                          <th className="px-3 py-2 text-[9px] font-black text-slate-400 uppercase text-left">
                            Param
                          </th>
                          <th className="px-3 py-2 text-[9px] font-black text-slate-400 uppercase text-left">
                            Spec
                          </th>
                          <th className="px-3 py-2 text-[9px] font-black text-slate-400 uppercase text-center">
                            S1
                          </th>
                          <th className="px-3 py-2 text-[9px] font-black text-slate-400 uppercase text-center">
                            S2
                      ...
                      <tbody className="divide-y divide-slate-50">
                        {newInspection.dimensions &&
                        newInspection.dimensions.length > 0 ? (
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewInspection({
                        ...newInspection,
                        qtyInspected: Number(e.target.value),
                      })
                    }
                  />
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 text-emerald-600">
                    Passed (Good)
                  </label>
                  <input
                    type="number"
                    ...
                    onChange={(e) =>
                      setNewInspection({
                        ...newInspection,
                        qtyPassed: Number(e.target.value),
                        qtyRejected:
                          (newInspection.qtyInspected || 0) -
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 text-red-600">
                    Rejected (NG)
                  </label>
                  <input
                    type="number"
                    ...
                    onChange={(e) =>
                      setNewInspection({
                        ...newInspection,
                        qtyRejected: Number(e.target.value),
                        qtyPassed:
                          (newInspection.qtyInspected || 0) -
                          Number(e.target.value),
                      })
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() =>
                      setNewInspection({ ...newInspection, status: "Partial" })
                    }
                    className={`flex-1 py-3 rounded-2xl border-2 font-black text-[10px] uppercase transition-all ${
                      newInspection.status === "Partial"
                  ...
                  <button
                    onClick={() =>
                      setNewInspection({ ...newInspection, status: "Rejected" })
                    }
                    className={`flex-1 py-3 rounded-2xl border-2 font-black text-[10px] uppercase transition-all ${
                      newInspection.status === "Rejected"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  >
                    ...
                    <span className="block mt-1 text-[10px] font-bold opacity-70">
                      QC membuat draft Stock In untuk diverifikasi gudang.
                    </span>
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    <span className="block text-xs font-black uppercase">
                      Jasa / Pekerjaan Lapangan
                    </span>
                    <span className="block mt-1 text-[10px] font-bold opacity-70">
                      Selesai di proyek dan tidak menambah stok gudang.
                    </span>
                  </button>
                </div>
                {newInspection.sendToWarehouse !== false && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-2">
                    <div className="space-y-1.5">
                      ...
                      >
                        <option value="">
                          — Pilih kategori dari Monitoring Gudang —
                        </option>
                      ...
                      {warehouseCategories.length === 0 && (
                        <p className="text-[10px] font-bold text-rose-500 ml-1">
                          Belum ada kategori. Daftarkan dahulu di Monitoring
                          Gudang.
                        </p>
                      )}
                    </div>
            ...
            <div className="p-8 border-t border-slate-100 bg-slate-50 flex items-center justify-between gap-4 sticky bottom-0">
              <div className="flex items-center gap-2 text-indigo-600">
                ...
                <span className="text-[10px] font-bold uppercase">
                  QC Pass membuat draft Stock In; stok baru bertambah setelah
                  gudang posting.
                </span>
              </div>
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="px-8 py-3 bg-indigo-600 text-white rounded-2xl text-xs font-black uppercase shadow-xl shadow-indigo-200 hover:bg-indigo-700 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isSubmitting ? "Menyimpan..." : "Simpan & Rilis Batch"}
                </button>
              </div>
            </div>
          </div>
            ...
            <div className="p-6 border-b border-slate-200 flex items-center justify-between bg-slate-50 print:hidden">
              <div>
                ...
                <p className="text-xs text-slate-500 font-bold">
                  Inspection Report - {selectedInspection.batchNo}
                </p>
              </div>
              <div className="flex gap-3">
                <button
