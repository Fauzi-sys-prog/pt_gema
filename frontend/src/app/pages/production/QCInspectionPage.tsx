...
export default function QCInspectionPage() {
  ...
  const handleManualQC = (report: any, result: 'Passed' | 'Failed') => {
    ...
    if (result === 'Failed' && !window.confirm(`Tandai LHP ${report.id.slice(-6)} sebagai QC Failed dan kembalikan ke produksi?`)) return;
    ...
    try {
      ...
      toast.error('Gagal memperbarui status QC: ' + (err instanceof Error ? err.message : 'Error'));
    } finally {
                            ...
                            {qc.status === 'Rejected' && (
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  if (wo) {
                                    if (!window.confirm(`Kembalikan WO ${qc.woNumber} ke produksi untuk rework?`)) return;
                                    setProcessingId(qc.id);
                                    try {
                                      updateWorkOrder(wo.id, { status: 'In Progress' });
                                      toast.success(`WO ${qc.woNumber} dikembalikan ke produksi untuk rework.`);
                                    } catch (err) {
                                      toast.error('Gagal rework WO: ' + (err instanceof Error ? err.message : 'Error'));
                                    } finally {
                                      setProcessingId(null);
                                    }
                                  } else {
                                ...
                                className="px-3 py-1.5 bg-amber-500 text-white rounded-lg text-[9px] font-black uppercase hover:bg-amber-600 transition-all inline-flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
