...
export default function ProductionTimelinePage() {
  ...
  const analytics = useMemo(() => {
    ...
    const monthlyReports = productionReportList.filter((r) => {
    ...
    const completedWOs = relevantWOs.filter(
  ...
  }, [workOrderList, productionReportList, selectedMonth, selectedYear]);

  // Chart Data: Daily Production Output from LHP for selected month
  const barData = useMemo(() => {
    const dailyOutput: Record<number, number> = {};
    analytics.monthlyReports.forEach((report) => {
      const day = new Date(report.tanggal).getDate();
      if (day >= 1 && day <= 31) {
        dailyOutput[day] = (dailyOutput[day] || 0) + report.outputQty;
      }
    });
    ...
    return data.length > 0
      ? data
      : [{ key: "day-empty", name: "N/A", id: 0, qty: 0 }];
  ...
  const handleExport = () => {
    try {
      ...
      const rows = analytics.relevantWOs.map((wo, index) => [
        ...
        wo.projectName,
        wo.itemToProduce,
        assetList.find((a) => a.id === wo.machineId)?.name ||
          wo.machineId ||
          "-",
        wo.completedQty,
      ...
      XLSX.writeFile(
        wb,
        `Detail_Monitoring_Produksi_${months[selectedMonth]}_${selectedYear}.xlsx`,
      );

      toast.success(
        `Laporan ${months[selectedMonth]} berhasil diekspor ke Excel`,
      );
    } catch (error) {
  ...
  const pieData = [
    {
      key: "effective",
      name: "Effective Days",
      value: analytics.effectiveDays || 1,
      color: "#10b981",
    },
    {
      key: "non-effective",
      name: "Non-Effective Days",
      value: Math.max(0, analytics.workDays - analytics.effectiveDays) || 1,
      color: "#ef4444",
    },
  ];
    ...
    <div className="space-y-6 pb-12 w-full max-w-full overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tight">
              Timeline & Production Tracker
            </h1>
            <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">
              {months[selectedMonth]} {selectedYear} Performance Analytics
            </p>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 className="text-xl font-black uppercase italic tracking-tight flex items-center gap-2">
                ...
                Real-Time Workshop Machine Monitor
              </h2>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">
                Status Kapasitas Produksi & Utilisasi Aset
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {assetList
              ...
              .map((machine) => {
                ...
                const actualWO = workOrderList.find((wo) => {
                  if (wo.status !== "In Progress") return false;
                  const lastReport = productionReportList
                ...
                return (
                  <div
                    key={machine.id}
                    className={`border p-4 rounded-2xl transition-all group ${
                      actualWO
                  ...
                  >
                    ...
                    <p className="text-[8px] font-bold text-white/40 uppercase mb-2">
                      {machine.assetCode}
                    </p>
                    ...
                    {displayWO ? (
                      <div className="mt-2 pt-2 border-t border-white/5">
                        <p
                          className={`text-[8px] font-black uppercase leading-none mb-1 ${actualWO ? "text-blue-400" : "text-amber-400"}`}
                        >
                          {actualWO ? "Working On:" : "Reserved For:"}
                        </p>
                        <p className="text-[9px] font-bold text-white/70 truncate uppercase">
                          {displayWO.projectName}
                        </p>
                        <p className="text-[7px] text-white/40 font-bold uppercase mt-1">
                          {displayWO.woNumber}
                        </p>
                      </div>
                    ) : machine.status === "Under Maintenance" ? (
                      <div className="mt-2 pt-2 border-t border-white/5 flex items-center gap-1">
                        ...
                        <p className="text-[8px] font-black text-amber-400 uppercase italic">
                          Maintenance
                        </p>
                      </div>
                    ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        {[
          ...
          <div
            key={i}
            className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex flex-col items-center text-center min-w-0"
          >
            <div
              className={`w-10 h-10 ${item.bg} ${item.color} rounded-xl flex items-center justify-center mb-3`}
            >
              <item.icon size={20} />
            </div>
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-tight mb-1 h-8 flex items-center justify-center leading-tight">
              {item.label}
            </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col min-w-0">
          <div className="p-6 border-b border-slate-50 flex items-center justify-between">
            <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest flex items-center gap-2">
              <BarChart size={18} className="text-blue-600" /> Production Output
              Scaling
            </h3>
        ...
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col min-w-0">
          <div className="p-6 border-b border-slate-50">
            <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest flex items-center gap-2">
              <PieChartIcon size={18} className="text-rose-500" /> Efficiency
              Share
            </h3>
          </div>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-50">
              {analytics.relevantWOs.map((wo) => {
                const rawStartDate = wo.startDate
                  ? new Date(wo.startDate)
                  : null;
                const rawEndDate = wo.endDate ? new Date(wo.endDate) : null;
                const rawDeadlineDate = wo.deadline
                ...
                const startDate =
                  rawStartDate && !Number.isNaN(rawStartDate.getTime())
                    ...
                    : null;
                const endDate =
                  rawEndDate && !Number.isNaN(rawEndDate.getTime())
                    ? rawEndDate
                    : null;
                const deadlineDate =
                  rawDeadlineDate && !Number.isNaN(rawDeadlineDate.getTime())
                    ? rawDeadlineDate
                    : null;

                const timelineEndDate =
                  wo.status === "Completed" && endDate ? endDate : deadlineDate;

                const totalDays =
                  startDate &&
                  timelineEndDate &&
                  timelineEndDate.getTime() >= startDate.getTime()
                    ? Math.max(
                        1,
                        Math.ceil(
                          (timelineEndDate.getTime() - startDate.getTime()) /
                  ...
                  >
                    <td className="px-6 py-5">
                      <div className="flex flex-col">
                        ...
                        <span className="text-[9px] text-blue-600 font-black uppercase tracking-widest mt-1">
                          {wo.woNumber}
                        </span>
                      </div>
                    </td>
                      ...
                      {wo.machineId ? (
                        <div className="flex flex-col">
                          <span className="text-sm font-black text-slate-900 leading-tight">
                            {assetList.find((a) => a.id === wo.machineId)
                              ?.name || wo.machineId}
                          </span>
                    ...
                    <td className="px-6 py-5 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <span className="text-xs font-bold text-slate-900">
                          {timelineEndDate
                            ? timelineEndDate.toLocaleDateString("id-ID", {
                                day: "2-digit",
                                month: "short",
                              })
                            : "—"}
                  ...
                  <td colSpan={6} className="px-6 py-20 text-center">
                    <div className="flex flex-col items-center gap-3 opacity-30">
                      ...
                      <p className="text-xs font-black uppercase tracking-widest text-slate-500">
                        Belum ada aktivitas produksi aktif.
                      </p>
                    </div>
                  </td>
                </tr>
