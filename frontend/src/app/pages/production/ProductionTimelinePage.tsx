...
export default function ProductionTimelinePage() {
  ...
  const barData = useMemo(() => {
    ...
    const data = Object.entries(dailyOutput)
      .map(([day, qty]) => ({ key: `day-${day}`, name: `Tgl ${day}`, id: Number(day), qty }))
      .sort((a, b) => a.id - b.id);

    return data.length > 0 ? data : [{ key: 'day-empty', name: 'N/A', id: 0, qty: 0 }];
  ...
  const handleExport = () => {
    try {
      ...
      const rows = analytics.relevantWOs.map((wo, index) => [
        ...
        wo.projectName,
        wo.itemToProduce,
        assetList.find(a => a.id === wo.machineId)?.name || wo.machineId || "-",
        wo.completedQty,
  ...
  const pieData = [
    { key: 'effective', name: 'Effective Days', value: analytics.effectiveDays || 1, color: '#10b981' },
    { key: 'non-effective', name: 'Non-Effective Days', value: Math.max(0, analytics.workDays - analytics.effectiveDays) || 1, color: '#ef4444' },
  ];
      ...
      <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-2xl">
        ...
        <div className="relative z-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {assetList.filter(a => a.category === 'Machine' || a.category === 'Heavy Equipment').map((machine) => {
              ...
              const bookedWO = !actualWO ? workOrderList.find(wo => 
          ...
          <div className="w-full p-4 overflow-hidden h-[350px] min-w-0 min-h-0 flex flex-col relative">
            <ResponsiveContainer width="100%" height={350} minWidth={0} minHeight={0}>
              <ReBarChart data={barData} margin={{ top: 20, right: 30, left: 0, bottom: 20 }}>
                ...
                />
                <Bar dataKey="qty" fill="#3b82f6" radius={[6, 6, 0, 0]} barSize={40} isAnimationActive={false} />
              </ReBarChart>
            </ResponsiveContainer>
          </div>
        </div>
