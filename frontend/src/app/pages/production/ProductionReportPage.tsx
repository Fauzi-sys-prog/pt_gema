...
export default function ProductionReportPage() {
  ...
  // LHP linked-WO hanya boleh dicatat untuk WO yang sudah benar-benar dimulai
  // dan masih memiliki sisa target produksi.
  const activeWorkOrders = workOrderList.filter((wo) => {
    const target = Number(wo.targetQty || 0);
    const completed = Number(wo.completedQty || 0);

    return wo.status === "In Progress" && target > completed;
  });

  const [newReport, setNewReport] = useState<
    Partial<ProductionReport & { woId?: string; selectedItem?: string }>
  >({
  ...
  const availableAssets = useMemo(() => {
    ...
    return assetList.filter(
      (a) =>
        ...
        (a.status === "Available" ||
          a.id === selectedWO?.machineId ||
          a.id === newReport.machineId),
    );
  }, [assetList, newReport.woId, newReport.machineId, workOrderList]);

  const filteredReports = productionReportList
    .filter(
      (report) =>
  ...
  const handleWOSelect = (woId: string) => {
    ...
    if (wo) {
      ...
      const fromRows =
        (wo.teknisiRows as any[])?.map((r: any) => r.nama).filter(Boolean) ||
        [];
  ...
  const handleAddReport = async () => {
    ...
    if (
      ...
      toast.error(
        "Pilih Work Order, lalu lengkapi teknisi, aktivitas, dan qty",
      );
      return;
    }
    ...
    if (selectedWO.status !== "In Progress") {
      toast.error(
        `Work Order ${selectedWO.woNumber} tidak dalam status In Progress`,
      );
      return;
    }

    const targetQty = Number(selectedWO.targetQty || 0);
    const completedQty = Number(selectedWO.completedQty || 0);
    const remainingQty = Math.max(targetQty - completedQty, 0);
    const outputQty = Number(newReport.outputQty);

    if (!Number.isFinite(outputQty) || outputQty <= 0) {
      toast.error("Qty output harus lebih besar dari 0");
      return;
    }

    if (remainingQty <= 0) {
      toast.error(`Target Work Order ${selectedWO.woNumber} sudah terpenuhi`);
      return;
    }

    if (outputQty > remainingQty) {
      toast.error(
        `Qty output melebihi sisa target. Maksimal ${remainingQty} ${selectedWO.targetUnit || ""}`,
      );
      return;
    }
    ...
    const finalReport = {
      ...
      nomorSPK:
        (newReport as any).nomorSPK || selectedWO?.nomorSPK || undefined,
    ...
    if (!newReport.woId) {
      const deductItems = manualMaterials
        ...
        .map((m) => {
          ...
          return si
            ? { kode: si.kode, nama: si.nama, qty: m.qty, satuan: si.satuan }
            : null;
        })
        .filter(Boolean) as {
        kode: string;
      ...
      if (deductItems.length > 0) {
        try {
          ...
          toast.error(
            "LHP tersimpan, tetapi potong stok gagal: " +
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">
              Workshop Progress & Productivity Logs
            </p>
          </div>
        </div>
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-2xl px-5 py-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          ...
          <p className="text-sm font-black text-slate-800">
            LHP = catat hasil harian dari Work Order yang dipilih
          </p>
          <p className="text-xs text-slate-500 mt-1">
            Jangan membuat rencana baru di sini. Pilih WO, isi output hari ini,
            lalu simpan.
          </p>
        </div>
        <span className="text-[10px] font-black uppercase text-blue-700 bg-white border border-blue-200 px-3 py-2 rounded-xl">
          LHP → potong bahan & tambah progress
        </span>
      </div>
                        ...
                        <div className="flex items-center gap-2 mt-1 flex-wrap">
                          ...
                          {report.machineNo && (
                            <span className="text-[10px] bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded-md font-black">
                              {assetList.find(
                                (a) =>
                                  a.id === report.machineNo ||
                                  a.assetCode === report.machineNo,
                              )?.name || report.machineNo}
                            </span>
                          )}
                        </div>
                      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white md:col-span-2 shadow-xl shadow-slate-900/10">
          <div className="flex items-center gap-4 mb-6">
            ...
            <h3 className="text-lg font-black uppercase italic italic tracking-tight">
          ...
          <div className="grid grid-cols-3 gap-8">
            <div>
              ...
              <p className="text-2xl font-black">
                {Object.entries(
                  productionReportList.reduce<Record<string, number>>(
                    (totals, report) => {
                      const unit = report.unit || "Tanpa satuan";
                      totals[unit] = (totals[unit] || 0) + report.outputQty;
                      return totals;
                    },
                    {},
                  ),
            ...
            <div>
              ...
              <p className="text-2xl font-black">
                {new Set(productionReportList.map((r) => r.workerName)).size}{" "}
                <span className="text-xs text-slate-500 font-bold uppercase">
                  Orang
                </span>
              </p>
        ...
        <div className="bg-rose-50 rounded-[2.5rem] p-8 border border-rose-100 flex flex-col justify-between">
          <div>
            <h3 className="text-sm font-black text-rose-900 uppercase tracking-widest mb-2">
              Automated Progress
            </h3>
            <p className="text-xs text-rose-700/70 font-medium leading-relaxed">
              Setiap laporan yang Anda input akan langsung memproses persentase
              di Timeline & Tracker secara otomatis tanpa intervensi manual.
            </p>
          </div>
          <Link
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-[2.5rem] w-full max-w-2xl relative z-10 overflow-hidden shadow-2xl flex flex-col max-h-[90vh] border border-slate-200">
            ...
            <div className="p-8 overflow-y-auto space-y-7">
              ...
              <div>
                <p className="text-[9px] font-black text-rose-500 uppercase tracking-[0.2em] mb-3">
                  ① Identitas Pekerjaan
                </p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      return (
                        <div className="md:col-span-2 grid grid-cols-2 md:grid-cols-4 gap-3 p-4 bg-blue-50 border border-blue-100 rounded-2xl">
                          ...
                          <div>
                            ...
                            <p className="text-sm font-black text-slate-800">
                              {target} / {completed} {wo.targetUnit || ""}
                            </p>
                          </div>
                          <div>
                            <p className="text-[9px] font-black text-blue-400 uppercase">
                  ...
                  <div className="space-y-1.5 md:col-span-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                      Nama Barang / Item yang Dikerjakan{" "}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="space-y-1.5 md:col-span-4">
                    ...
                    {lhpTeknisiList.length > 0 && (
                      <div className="flex flex-wrap gap-2 mb-2">
                        {lhpTeknisiList.map((nama, idx) => (
                          <span
                            key={idx}
                            className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 border border-blue-200 rounded-xl text-xs font-black text-blue-700 uppercase tracking-wide"
                          >
                            <User size={11} />
                            {nama}
                            <button
                    ...
                    <p className="text-[10px] text-slate-400 ml-1 mt-1">
                      Tekan Enter atau klik + Add. Jika ada WO, nama teknisi
                      otomatis terisi dari WO.
                    </p>
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNewReport({ ...newReport, tanggal: e.target.value })
                      }
                    />
                  </div>
                  <div className="space-y-1.5 md:col-span-2">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setNewReport({ ...newReport, workshop: e.target.value })
                      }
                    >
                      <option value="Gema Teknik Workshop">
                        Gema Teknik Workshop
                      </option>
                      <option value="Project Site">Project Site</option>
                      <option value="External Workshop">
                        External Workshop
                      </option>
                    </select>
                  </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setNewReport({ ...newReport, activity: e.target.value })
                      }
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                        Output Qty <span className="text-rose-400">*</span>
                      </label>
              ...
              <div>
                <p className="text-[9px] font-black text-rose-500 uppercase tracking-[0.2em] mb-3">
                  ...
                  <span className="text-slate-300 normal-case font-medium">
                    (stok berkurang sesuai output LHP)
                  </span>
                </p>
                ...
                {newReport.woId ? (
                  ...
                  <div className="space-y-1.5">
                    ...
                    <p className="text-[9px] text-slate-400 ml-1">
                      Auto memakai seluruh BOM secara proporsional terhadap
                      output LHP. Pilih item tertentu hanya jika pemakaian hari
                      ini memang terbatas pada item tersebut.
                    </p>
                  </div>
                ) : (
                  ...
                  <div className="space-y-3">
                    {manualMaterials.map((row, idx) => {
                      const selectedStock = stockItemList.find(
                        (s) => s.id === row.itemId,
                      );
                      return (
                        <div
                    ...
                    <p className="text-[9px] text-slate-400">
                      Pilih item dari gudang yang dipakai. Stok akan terpotong
                      otomatis saat LHP disimpan.
                    </p>
                  </div>
                )}
              </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="text-[10px] font-black text-slate-900 uppercase tracking-widest">
                        Evidence Foto Hasil Kerja
                      </h4>
                      <p className="text-[9px] text-slate-400 font-bold">
                        Opsional — untuk verifikasi QC
                      </p>
                    </div>
                  </div>
                  <button
            ...
            <div className="p-8 border-t border-slate-100 bg-slate-50 flex gap-4 sticky bottom-0">
              ...
              <button
                ...
                className="flex-1 py-4 bg-rose-600 text-white rounded-2xl text-xs font-black uppercase hover:bg-rose-700 shadow-xl shadow-rose-200 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <ClipboardList size={16} />
                {isSubmitting ? "MENYIMPAN..." : "SIMPAN & UPDATE PROGRESS"}
              </button>
            </div>
          </div>
        </div>
      ...
      {selectedPhoto && (
        <div
          className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-8"
        ...
        >
          ...
          >
            <button
              onClick={() => setSelectedPhoto(null)}
              className="absolute -top-12 right-0 text-white flex items-center gap-2 font-black uppercase text-xs"
            >
              Close <X size={20} />
            </button>
