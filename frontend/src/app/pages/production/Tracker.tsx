...
export const ProductionTrackerPage: React.FC = () => {
  const { workOrderList, assetList } = useApp();
  const [searchTerm, setSearchTerm] = useState("");
  const [filterStatus, setFilterStatus] = useState<string>("All");

  // Work Order adalah sumber authoritative untuk Production Tracker.
  // Legacy productionTrackerList tidak lagi digabung karena tidak memiliki
  // active writer dan dapat membawa data tanggal lama/stale dari localStorage.
  const woAsTrackers: ProductionTracker[] = useMemo(() => {
    return (workOrderList || [])
      .filter((wo: WorkOrder) => wo.status !== "Draft")
      .map(
        (wo: WorkOrder) =>
          ({
            id: `wo-${wo.id}`,
            ...
            finishDate:
              wo.status === "Completed" && wo.endDate
                ? wo.endDate
                : wo.deadline || "",
            status:
              wo.status === "Completed"
  ...
  const allTrackers = woAsTrackers;
  ...
  const formatTrackerDate = (date: Date | null) =>
    date
  ...
  // Bar hanya dirender jika start + finish valid dan urutannya masuk akal.
  const getTimelinePosition = (startDate: Date, finishDate: Date) => {
    ...
    const width = Math.min(100 - left, (duration / 31) * 100);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
        <div>
          ...
          <p className="text-slate-500 font-bold uppercase text-[10px] tracking-widest flex items-center gap-2">
            <Clock size={12} className="text-blue-600" /> Real-time Workshop
            Monitoring & Delivery Timeline
          </p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-900 text-white">
                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest border-r border-slate-800">
                  Customer
                </th>
                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest border-r border-slate-800">
                  Item Type
                </th>
                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-center border-r border-slate-800">
                  Qty
                </th>
                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-center border-r border-slate-800">
                  Start
                  ...
                  <div className="w-full h-full flex flex-col">
                    ...
                    <div className="flex h-6">
                      {Array.from({ length: 4 }).map((_, i) => (
                        <div
                          key={i}
                          className="flex-1 text-[8px] font-black text-slate-500 border-r border-slate-800 flex items-center justify-center"
                        >
                          W{i + 1}
                        </div>
                      ))}
            ...
            <tbody className="divide-y divide-slate-100">
              {(filteredTracker || []).map((item) => {
                ...
                const finishDate = parseValidDate(item.finishDate);
                const hasValidTimeline =
                  startDate !== null &&
                  finishDate !== null &&
                  finishDate.getTime() >= startDate.getTime();

                const duration = hasValidTimeline
                  ? Math.max(
                      1,
                      Math.ceil(
                        (finishDate.getTime() - startDate.getTime()) /
                ...
                const timeline =
                  hasValidTimeline && startDate && finishDate
                    ? getTimelinePosition(startDate, finishDate)
                    : null;

                const asset = (assetList || []).find(
                  (a) => a.id === item.machineId,
                );
                  ...
                  >
                    ...
                    <td className="px-6 py-6 text-center border-r border-slate-50">
                      <span className="text-[10px] font-bold text-slate-500">
                        {formatTrackerDate(startDate)}
                      </span>
                    </td>
                    <td className="px-6 py-6 text-center border-r border-slate-50">
                      <span className="text-[10px] font-black text-slate-900">
                        {formatTrackerDate(finishDate)}
                      </span>
                    </td>
                    <td className="px-6 py-6 text-center border-r border-slate-50">
                      <span
                        className={`inline-block px-3 py-1 rounded-md text-[9px] font-black uppercase tracking-tighter ${getStatusStyle(item.status)}`}
                      >
                        {item.status}
                      </span>
                    </td>
                    ...
                    <td className="px-0 py-0 relative">
                      ...
                      <div className="relative h-16 w-full flex items-center px-4">
                        {timeline && duration !== null ? (
                          <div
                            className={`h-4 rounded-full relative group/bar transition-all duration-700 hover:h-6 ${
                          ...
                          <span className="mx-auto text-[9px] font-black uppercase tracking-widest text-slate-300">
                            Belum ada timeline
                          </span>
                        )}
                      </div>
      ...
      {(() => {
        ...
        const inProgress = allTrackers.filter(
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden group">
              ...
              <div className="flex items-end gap-3">
                ...
                <span className="text-emerald-400 text-xs font-bold mb-1">
                  {completed} / {total} WO
                </span>
              </div>
            ...
            <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm relative overflow-hidden group">
              ...
              <div className="flex items-end gap-3">
                ...
                <span className="text-slate-400 text-xs font-bold mb-1 uppercase">
                  In Progress
                </span>
              </div>
              <p className="mt-4 text-[10px] font-bold text-slate-500 uppercase italic">
                {total} WO total terdaftar
              </p>
            </div>
            ...
            <div className="bg-blue-600 rounded-[2.5rem] p-8 text-white relative overflow-hidden group">
              ...
              <div className="flex items-end gap-3">
                ...
                <span className="text-blue-100 text-[10px] font-bold mb-1 uppercase tracking-tighter">
                  Unit
                </span>
              </div>
