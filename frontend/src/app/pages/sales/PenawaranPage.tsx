...
export default function PenawaranPage() {
  ...
  const handleSelectDataCollection = (dcId: string) => {
    ...
    if (dc) {
      const matchedCustomer = customerList.find((c: any) =>
        c.namaCustomer?.toLowerCase() === (dc.namaResponden || '').toLowerCase() ||
        c.namaCustomer?.toLowerCase() === (dc.perusahaan || '').toLowerCase()
      );
      setFormData({
        ...formData,
  ...
  const handleExportExcel = () => {
    const exportData = quotationList.map((q: any) => ({
      'No. Penawaran': q.nomorQuotation,
      'Tanggal': q.tanggal,
      'Customer': q.customer?.nama || 'N/A',
      'Perihal': q.perihal || '-',
      'Nilai': q.grandTotal || 0,
      'Status': q.status || 'Draft'
    }));
    exportToExcel(exportData, `Quotation_Registry_${new Date().toISOString().split('T')[0]}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newQuo: Quotation = {
      id: `quo-${Date.now()}`,
      nomorQuotation: formData.noPenawaran || `${String(quotationList.length + 1).padStart(3, '0')}/QO/GTP/${new Date().getFullYear()}`,
  ...
  const filteredData = quotationList.filter((item: any) => {
    const matchesSearch = 
                ...
                <tr key={item.id} className="hover:bg-blue-50/30 transition-all group cursor-pointer">
                  ...
                  <td className="px-8 py-5 text-center">
                    <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase italic border ${
                      ...
                      item.status === 'Complete'        ? 'bg-teal-50 text-teal-600 border-teal-100' :
                      item.status === 'In Progress'     ? 'bg-blue-50 text-blue-600 border-blue-100' :
                      item.status === 'Pending Approval'? 'bg-yellow-50 text-yellow-600 border-yellow-100' :
                      item.status === 'Pending Review'  ? 'bg-orange-50 text-orange-600 border-orange-100' :
                      item.status === 'On Hold'         ? 'bg-purple-50 text-purple-600 border-purple-100' :
                      item.status === 'Rejected'        ? 'bg-rose-50 text-rose-600 border-rose-100' :
                      item.status === 'Planning'        ? 'bg-slate-50 text-slate-500 border-slate-100' :
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            >
                        ...
                        <div className="space-y-3">
                          ...
                          <select
                            ...
                            onChange={e => {
                              ...
                              setFormData({ ...formData, customerId: e.target.value, customer: cust?.namaCustomer || formData.customer });
                            }}
                            className="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl text-sm font-bold focus:border-blue-500 outline-none"
                          >
                          ...
                          <input type="text" placeholder="Atau ketik manual jika belum terdaftar" value={formData.customer} onChange={e => setFormData({...formData, customer: e.target.value, customerId: ''})} className="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl text-sm font-bold focus:border-blue-500 outline-none" />
                        </div>
                      </div>
              ...
              <div className="p-8 bg-slate-50 border-t-2 border-slate-100 flex justify-end gap-4 shrink-0">
                <button type="button" onClick={() => setShowModal(false)} disabled={isSubmitting} className="px-8 py-3 bg-white border-2 border-slate-100 text-slate-600 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-100 transition-all cursor-pointer disabled:opacity-50">Batal</button>
                <button type="submit" form="quoForm" disabled={isSubmitting} className="px-10 py-3 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-black transition-all shadow-xl shadow-slate-200 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">{isSubmitting ? 'Menyimpan...' : 'Simpan & Generate Quotation'}</button>
              </div>
            </motion.div>
          </div>
        )}
