...
import { useApp, type Quotation, type QuotationItem } from '../contexts/AppContext';
...
// Satuan adalah unit pengukuran barang/pekerjaan, bukan label generik "Qty".
const MATERIAL_UNITS = ['Kg', 'Sack', 'Ton', 'Liter', 'Meter', 'M²', 'M³', 'Pcs', 'Unit', 'Set', 'Job', 'Lot'];
...
export default function ProjectQuotationPage() {
  ...
  useEffect(() => {
    if (location.state?.fromDataCollection) {
      ...
      setTimeout(() => {
        alert(`📋 Data dari Data Collection (${noKoleksi}) berhasil dimuat!\\n\\n✅ Customer: ${customerName}\\n✅ ${materials?.length || 0} materials\\n✅ ${manpower?.length || 0} manpower\\n✅ ${schedule?.length || 0} schedule items\\n✅ ${consumables?.length || 0} consumables\\n✅ ${equipment?.length || 0} equipment\\n\\nSilakan lengkapi data quotation dan simpan.`);
      }, 500);
    }
  }, [location.state]);
  ...
  const { handleApprove, handleReject, handleSendToCustomer } = useQuotationActions({
    updateQuotation,
    formatCurrency,
    calculateTotalValue,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const quotationData = {
      ...
      nomorQuotation: formData.nomorQuotation || `QT-${new Date().getFullYear()}-${String(quotationList.length + 1).padStart(3, '0')}`,
    ...
    try {
      if (editMode && editingId) {
        // Update existing
        const saved = await updateQuotation(editingId, quotationData);
        if (saved === undefined) {
          alert('❌ Perubahan quotation gagal disimpan ke server.');
          return;
        }
        alert('✅ Quotation berhasil diupdate!');
      } else {
        // Create new
      ...
      alert('❌ Quotation gagal disimpan: ' + (err instanceof Error ? err.message : 'Error'));
    } finally {
  ...
  const handleExportWord = (quotation: Quotation) => {
    const quotationData = {
      ...quotation,
      items: quotation.materials?.map((m: Material) => ({
        description: m.materialName,
        specification: m.specifications?.join(', ') || '',
        quantity: m.quantity,
        unit: m.unit,
        unitPrice: m.unitPrice || 0,
        totalPrice: (m.quantity * (m.unitPrice || 0))
      })) || [],
      subtotal: calculateSubtotal(quotation.materials || []),
      discount: 0,
      discountPercent: 0,
      tax: calculatePPN(calculateSubtotal(quotation.materials || []), quotation.ppn),
      taxPercent: quotation.ppn,
      grandTotal: calculateGrandTotal(calculateSubtotal(quotation.materials || []), calculatePPN(calculateSubtotal(quotation.materials || []), quotation.ppn)),
      paymentTerms: quotation.paymentTerms || 'DP 50%, Pelunasan setelah pekerjaan selesai',
      timeline: quotation.timeline || '30 hari kerja',
      warranty: quotation.warranty || '6 bulan'
    };
    
    const filename = `Quotation_${(quotation.nomorQuotation || quotation.noPenawaran || 'draft').replace(/\//g, '_')}.doc`;
    downloadQuotationWordDocument(quotationData, filename);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          ...
          <div className="text-gray-900">{quotationList.length}</div>
        </div>
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          <div className="text-gray-600 mb-2">Draft</div>
          <div className="text-gray-600">{quotationList.filter(q => q.status === 'Draft').length}</div>
        </div>
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          <div className="text-gray-600 mb-2">Sent</div>
          <div className="text-blue-600">{quotationList.filter(q => q.status === 'Sent').length}</div>
        </div>
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          <div className="text-gray-600 mb-2">Approved</div>
          <div className="text-green-600">{quotationList.filter(q => q.status === 'Approved').length}</div>
        </div>
      </div>
                  ...
                  <tr key={quotation.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button
                          onClick={() => handleExportWord(quotation)}
                          className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"
          ...
          <div className="bg-white rounded-lg w-full max-w-6xl my-8">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="p-6 border-t border-gray-200 flex justify-end gap-3">
                ...
                >
                  <Save size={18} />
                  {isSubmitting ? 'Menyimpan...' : editMode ? 'Update Quotation' : 'Simpan Quotation'}
                </button>
              </div>
            </form>
          </div>
            ...
            <form onSubmit={(e) => {
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    list="quotation-material-units"
                    value={materialForm.unit}
                    onChange={(e) => setMaterialForm({ ...materialForm, unit: e.target.value })}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg"
                    placeholder="Kg, Sack, Ton, Pcs, dll"
                    required
                  />
                  <datalist id="quotation-material-units">
          ...
          <div className="bg-white rounded-lg w-full max-w-5xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 border-t border-gray-200 flex justify-between items-center gap-3 print:hidden">
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => handleExportWord(selectedQuotation)}
                  className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center gap-2"
                >
                  <Download size={18} />
                  Export to Word
