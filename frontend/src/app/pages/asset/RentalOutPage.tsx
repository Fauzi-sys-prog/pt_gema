...
export default function RentalOutPage() {
  ...
  useEscapeKey([
    { condition: showRentalModal, close: () => setShowRentalModal(false) },
  ]);


  const rentableAssets = useMemo(() => {
    return assetList.filter(a => a.status === 'Rented Out' || a.status === 'Available');
  }, [assetList]);
  ...
  const activeRentals = useMemo(() => assetList.filter(a => a.status === 'Rented Out'), [assetList]);
  ...
  const handleRentalOut = (e: React.FormEvent) => {
    ...
    const asset = assetList.find(a => a.id === rentalForm.assetId);
    if (!asset) return;
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm flex items-center gap-6">
           ...
           <div>
              ...
              <p className="text-3xl font-black text-slate-900 italic">{Math.round((activeRentals.length / (assetList.length || 1)) * 100)}% <span className="text-xs text-slate-300 font-bold uppercase not-italic">Rented</span></p>
           </div>
        </div>
      </div>
