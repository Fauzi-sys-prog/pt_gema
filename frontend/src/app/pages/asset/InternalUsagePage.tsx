...
export default function InternalUsagePage() {
  ...
  useEscapeKey([
    { condition: showAssignModal, close: () => setShowAssignModal(false) },
  ]);


  const internalAssets = useMemo(() => {
    return assetList.filter(a => a.status === 'In Use' || a.status === 'Available');
  }, [assetList]);
  ...
  const activeUsage = useMemo(() => assetList.filter(a => a.status === 'In Use'), [assetList]);
  ...
  const handleAssignUsage = (e: React.FormEvent) => {
    ...
    const asset = assetList.find(a => a.id === assignForm.assetId);
    const operator = employeeList.find(e => e.id === assignForm.operatorId);
    const project = projectList.find(p => p.id === assignForm.projectId);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex items-center gap-6">
           ...
           <div>
              ...
              <p className="text-3xl font-black text-slate-900 italic">{employeeList.length} <span className="text-xs text-slate-300 font-bold uppercase not-italic">Personnels</span></p>
           </div>
        </div>
      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      {assetList.filter(a => a.status === 'Available').map(a => (
