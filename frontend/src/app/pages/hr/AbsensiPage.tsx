...
import { useApp, type Attendance } from '../../contexts/AppContext';
...
export default function AbsensiPage() {
  ...
  const handleCheckIn = () => {
    ...
    alert(`Check-in berhasil pada ${time}!`);
  };

  const handleCheckOut = () => {
    const now = new Date();
    const time = now.toTimeString().split(' ')[0].substring(0, 5);
    alert(`Check-out berhasil pada ${time}!`);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!employee) {
      alert('Pilih karyawan dari daftar yang tersedia.');
      return;
    }
    if (attendanceList.some(item => item.employeeId === employee.id && item.date === formData.date)) {
      alert('Absensi karyawan pada tanggal tersebut sudah ada. Perbarui catatan yang ada, jangan membuat duplikat.');
      return;
    }
    ...
    try {
      ...
      alert('Gagal menyimpan absensi: ' + (err instanceof Error ? err.message : 'Error'));
    } finally {
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      const employee = employeeList.find(item => item.id === e.target.value);
                      setFormData({ ...formData, employeeId: e.target.value, employeeName: employee?.name ?? '' });
                    }}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg"
                    required
                  >
                    <option value="">Pilih karyawan</option>
                    {employeeList.filter(item => item.status === 'Active').map(item => <option key={item.id} value={item.id}>{item.name} — {item.employeeId}</option>)}
                  </select>
                </div>
                <div>
