...
export default function AttendanceRecapPage() {
  const { attendanceList, employeeList } = useApp();
  ...
  const processedData = useMemo(() => {
    return (employeeList || []).map(emp => {
      const yearlyStats = months.map((monthName, idx) => {
        ...
        const monthAttendances = (attendanceList || []).filter(a => 
  ...
  }, [attendanceList, employeeList, selectedYear]);
  ...
  const filteredEmployees = processedData.filter(e => (e.name || '').toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-lg text-gray-700 hover:bg-gray-50 transition-all shadow-sm">
            <Download size={18} />
