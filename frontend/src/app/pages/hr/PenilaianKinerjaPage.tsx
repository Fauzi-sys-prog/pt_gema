...
export default function PenilaianKinerjaPage() {
  const { employeeList = [], penilaianKinerjaList = [], addPenilaian, updatePenilaian } = useApp();
  const penilaianList: PenilaianKinerja[] = penilaianKinerjaList as PenilaianKinerja[];
  ...
  useEscapeKey([
    { condition: showFormModal, close: () => setShowFormModal(false) },
  ...
  const handleSubmit = () => {
    if (isEdit && selectedPenilaian) {
      updatePenilaian(selectedPenilaian.id, formData);
    } else {
      const newPenilaian: PenilaianKinerja = {
        id: `PKK${Date.now()}`,
        ...formData
      } as PenilaianKinerja;
      addPenilaian(newPenilaian);
    }
  ...
  // Statistics
  const avgScore = Math.round((penilaianList.reduce((sum, p) => sum + p.averageScore, 0) / penilaianList.length) * 10) / 10;
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-5xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 border-t border-gray-200 flex justify-end gap-3">
              ...
              <button
                onClick={() => {
                  handleFormChange('status', 'Submitted');
                  handleSubmit();
                }}
