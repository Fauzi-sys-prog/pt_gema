...
export default function TraceabilityPage() {
  const { stockMovementList, stockItemList } = useApp();
  ...
  const batchLifecycle = useMemo(() => {
    ...
    const item = stockItemList.find(s => s.kode === firstMovement.itemKode);
  ...
  }, [selectedBatch, stockMovementList, stockItemList]);
                             ...
                             <div className="flex-1">
                                ...
                                <div className="mt-4 flex flex-wrap gap-4">
                                   <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-50 rounded-lg text-[10px] font-black text-slate-500 border border-slate-100 uppercase italic">
                                      <Clock size={12} /> {m.tanggal}
                                   </div>
                                   <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-50 rounded-lg text-[10px] font-black text-slate-500 border border-slate-100 uppercase italic">
                                      <User size={12} /> {m.createdBy || 'Sistem'}
                                   </div>
                                   <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-50 rounded-lg text-[10px] font-black text-slate-500 border border-slate-100 uppercase italic">
