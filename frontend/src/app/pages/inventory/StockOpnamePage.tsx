...
import { useApp, type StockOpname } from '../../contexts/AppContext';
import { 
  ClipboardList, 
  Search, 
  Plus, 
...
export default function StockOpnamePage() {
  ...
  const handleSubmit = () => {
    ...
    try {
      ...
      toast.error('Gagal menyimpan draft opname: ' + (err instanceof Error ? err.message : 'Error'));
    } finally {
      setIsSubmitting(false);
    }
  };

  const handleConfirm = (id: string): boolean => {
    if (processingId) return false;
    if (!window.confirm('Konfirmasi Stock Opname? Perubahan stok akan langsung diterapkan ke gudang.')) return false;
    setProcessingId(id);
    try {
      confirmStockOpname(id, currentUser?.fullName || 'Admin');
      toast.success('Stock Opname berhasil dikonfirmasi dan stok diperbarui');
      return true;
    } catch (err) {
      toast.error('Gagal konfirmasi opname: ' + (err instanceof Error ? err.message : 'Error'));
      return false;
    } finally {
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
              ...
              <h3 className="text-3xl font-black italic text-slate-900">{stockOpnameList.length} <span className="text-sm text-slate-400 font-bold not-italic">Records</span></h3>
            </div>
            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm border-l-4 border-l-amber-500">
              ...
              <h3 className="text-3xl font-black italic text-slate-900">{stockOpnameList.filter(o => o.status === 'Draft').length} <span className="text-sm text-slate-400 font-bold not-italic">Items</span></h3>
            </div>
            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm border-l-4 border-l-emerald-500">
              ...
              <h3 className="text-3xl font-black italic text-slate-900">{stockOpnameList.filter(o => o.status === 'Completed').length} <span className="text-sm text-slate-400 font-bold not-italic">Selesai</span></h3>
            </div>
          </div>
                        ...
                        <div className="flex justify-end gap-2">
                           {o.status === 'Draft' && (
                              <button 
                                ...
                                className="px-6 py-3 bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-200 flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                              >
                                {processingId === o.id ? 'Memproses...' : 'Confirm'}
                              </button>
                           )}
                           <button 
            ...
            <div className="pt-10 border-t border-slate-100 flex justify-end gap-4">
               ...
               >
                ...
                <button 
                 ...
                 disabled={newOpname.items.length === 0 || isSubmitting}
                ...
                >
                  <Save size={18} /> {isSubmitting ? 'Menyimpan...' : 'Simpan Draft Opname'}
                </button>
            </div>
          </div>
        </motion.div>
                ...
                <div className="p-10 overflow-y-auto space-y-8">
                  ...
                  {selectedOpname.status === 'Draft' && (
                    <div className="pt-6 border-t border-slate-100">
                      <button 
                        onClick={() => {
                          if (handleConfirm(selectedOpname.id)) {
                            setSelectedOpname(null);
                          }
                        }}
                        disabled={processingId !== null}
                        className="w-full py-5 bg-emerald-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-700 transition-all flex items-center justify-center gap-3 shadow-xl shadow-emerald-200 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        <CheckCircle2 size={20} /> {processingId === selectedOpname.id ? 'Memproses...' : 'Konfirmasi & Perbarui Stok'}
                      </button>
                    </div>
                  )}
                </div>
