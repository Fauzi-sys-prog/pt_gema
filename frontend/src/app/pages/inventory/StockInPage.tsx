...
export default function StockInPage() {
  ...
  const filteredStockIn = stockInList.filter((si) => {
    ...
    const matchSearch =
      noStockIn.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const stockIn: StockIn = {
        ...
        type: formData.type,
        status: formData.receivingId ? "Draft" : "Posted",
      ...
      if (!stockIn.items.length)
        throw new Error("Isi minimal satu SKU dengan qty positif.");
      await createStockIn(stockIn);

      toast.success(
        stockIn.status === "Draft"
          ? "Draft Stock In Receiving berhasil dibuat. Verifikasi lalu Posting untuk menambah saldo stok."
          : "Jurnal stok masuk berhasil diposting.",
      );
      setShowModal(false);
      resetForm();
    } catch (err) {
      toast.error(
        "Gagal posting stok masuk: " +
  ...
  const canImportReceiving = (rcv: Receiving) =>
    ...
    !stockInList.some(
      (si) =>
        si.receivingId === rcv.id ||
        (si.noSuratJalan === (rcv.noSuratJalan || rcv.noPO) &&
          ["Receiving", "Purchase Receiving"].includes(si.type)),
    );
  ...
  const importFromReceiving = (rcv: Receiving) => {
    ...
    if (alreadyImported) {
      toast.warning(
        `Receiving ${rcv.noReceiving} sudah pernah di-import ke Stok Masuk.`,
      );
      return;
    }
    setFormData({
      ...
      items: rcv.items
        .filter((i) => (i.qtyGood ?? i.qtyReceived ?? 0) > 0)
        .map((i) => ({
          kode: i.itemKode || "",
          nama: i.itemName,
          kategori: "",
  ...
  const handleFotoKondisiUpload = (
    ...
    reader.onload = (ev) =>
      updateItem(index, "fotoKondisi", ev.target?.result as string);
  ...
  const updateItem = (index: number, field: string, value: any) => {
    ...
    if (field === "kode") {
      const stockItem = stockItemList.find(
        (s) => s.kode.toLowerCase() === value.toLowerCase(),
      );
      if (stockItem) {
  ...
  const postDraftStockIn = async (stockIn: StockIn) => {
    if (isPosting) return;
    setIsPosting(true);

    const createdBy =
      currentUser?.fullName || stockIn.createdBy || "Petugas Gudang";

    try {
      await updateStockIn(stockIn.id, {
        status: "Posted",
        createdBy,
      });
      ...
      toast.success(
        stockIn.type === "Production Output"
          ? `${stockIn.noStockIn} sudah diverifikasi dan diposting. Stok barang jadi bertambah.`
          : `${stockIn.noStockIn} sudah diverifikasi dan diposting. Stok gudang bertambah.`,
      );
    } catch (err) {
      toast.error(
        "Gagal memposting: " + (err instanceof Error ? err.message : "Error"),
      );
    } finally {
      setIsPosting(false);
    }
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredStockIn.map((si) => (
                <tr key={si.id} className="hover:bg-slate-50/80 transition-all">
                  <td className="px-8 py-5 font-black text-slate-900 italic tracking-tight">
                    {si.noStockIn}
                  </td>
                  ...
                  <td className="px-8 py-5">
                    <span
                      className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider border ${si.status === "Posted" ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-amber-50 text-amber-700 border-amber-200"}`}
                    >
                      {si.status === "Posted"
                        ? "Posted"
                        : "Menunggu Verifikasi"}
                  ...
                  <td className="px-8 py-5 text-center">
                    <div className="flex items-center justify-center gap-2">
                      <button
                        onClick={() => {
                          setSelectedStockIn(si);
                          setShowDetailModal(true);
                        }}
                        className="w-9 h-9 bg-slate-100 text-slate-400 rounded-xl flex items-center justify-center hover:bg-slate-900 hover:text-white transition-all"
                      >
                        <Eye size={18} />
                      </button>
                      {si.status === "Draft" &&
                        ["Production Output", "Receiving"].includes(
                          si.type,
                        ) && (
                          <button
                            onClick={() => postDraftStockIn(si)}
                            disabled={isPosting}
                            className="px-3 h-9 bg-emerald-600 text-white rounded-xl text-[9px] font-black uppercase hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            Posting
                          </button>
                        )}
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-8">
              ...
              <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center gap-4">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-indigo-500 font-medium">
                    Pilih receiving untuk auto-isi form otomatis.
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100">
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">
                    No. Surat Jalan / Dokumen
                  </label>
                  <input
                    type="text"
                    value={formData.noSuratJalan}
                    onChange={(e) =>
                      setFormData({ ...formData, noSuratJalan: e.target.value })
                    }
                ...
                <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        type: e.target.value as StockIn["type"],
                      })
                    }
                    className="w-full px-4 py-3 bg-white border-transparent rounded-xl text-sm font-bold text-black outline-none"
                    required
              ...
              <div>
                ...
                <div className="space-y-3">
                  {formData.items.map((item, index) => (
                    <div
                      key={index}
                      className="grid grid-cols-12 gap-3 items-end bg-slate-50 p-5 rounded-3xl border border-slate-100"
                    >
                      <div className="col-span-2">
                        <label className="block text-[9px] font-black text-slate-400 uppercase mb-1.5 ml-1">
                          SKU
                        </label>
                        <input
                          ...
                          value={item.kode || ""}
                          onChange={(e) =>
                            updateItem(index, "kode", e.target.value)
                          }
                      ...
                      <div className="col-span-2">
                        ...
                        <select
                          value={item.kategori || ""}
                          onChange={(e) =>
                            updateItem(index, "kategori", e.target.value)
                          }
                      ...
                      <div className="col-span-1">
                        ...
                        <input
                          ...
                          className="w-full px-4 py-2.5 bg-white border-transparent rounded-xl text-xs font-black text-black outline-none"
                          required
                        />
                      </div>
                      <div className="col-span-1">
                      ...
                      <div className="col-span-1">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            updateItem(index, "kondisi", e.target.value)
                          }
                      ...
                      <div className="col-span-1">
                        <button
                          ...
                          className="w-full h-10 bg-white text-red-400 rounded-xl flex items-center justify-center hover:bg-red-50 hover:text-red-600 transition-all active:scale-90 disabled:opacity-30 mt-5"
                      ...
                      {item.kondisi !== "Baik" && (
                        <div className="col-span-12 mt-1 pt-3 border-t border-rose-100 flex items-center gap-3">
                          ...
                          <input
                            ref={(el) => {
                              fotoKondisiRefs.current[index] = el;
                            }}
                            type="file"
                            accept="image/*"
                            className="hidden"
                          ...
                          {item.fotoKondisi ? (
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                fotoKondisiRefs.current[index]?.click()
                              }
              ...
              <div className="flex gap-4 pt-4">
                <button
                  type="button"
                  onClick={() => setShowModal(false)}
                  className="flex-1 py-4 text-[11px] font-black uppercase tracking-widest text-slate-400"
                >
                  Batal
                </button>
                <button
                  ...
                  className="flex-[2] py-4 bg-emerald-600 text-white rounded-2xl text-[11px] font-black uppercase tracking-widest shadow-lg shadow-emerald-100 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isSubmitting ? "Memproses..." : "Posting ke Buku Besar"}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-3xl overflow-hidden">
            ...
            <div className="p-8 space-y-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                <div>
                  ...
                  <p className="font-black text-slate-900 italic">
                    {new Date(selectedStockIn.tanggal).toLocaleDateString(
                      "id-ID",
                    )}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <span
                    className={`text-[10px] font-black px-2 py-0.5 rounded ${selectedStockIn.status === "Posted" ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-700"}`}
                  >
                    {selectedStockIn.status === "Posted"
                      ? "POSTED"
                      : "DRAFT - MENUNGGU GUDANG"}
                  </span>
                </div>
              </div>
              ...
              <div className="border border-slate-100 rounded-3xl overflow-hidden">
                <table className="w-full text-left">
                  <thead className="bg-slate-50">
                    <tr>
                      <th className="px-6 py-4 text-[9px] font-black text-slate-500 uppercase">
                        SKU / Nama
                      </th>
                      <th className="px-6 py-4 text-[9px] font-black text-slate-500 uppercase text-center">
                        Batch/Lot
                      </th>
                      <th className="px-6 py-4 text-[9px] font-black text-slate-500 uppercase text-right">
                        Qty
                      </th>
                    </tr>
                  </thead>
                    ...
                    {selectedStockIn.items?.map((item, i) => (
                      <tr key={i}>
                        ...
                        <td className="px-6 py-4 text-right">
                          ...
                          <span className="ml-1 text-[9px] font-black text-slate-300 uppercase">
                            {item.unit}
                          </span>
                        </td>
                      </tr>
              ...
              {selectedStockIn.status === "Draft" &&
                ["Production Output", "Receiving"].includes(
                  selectedStockIn.type,
                ) && (
                  <button
                    onClick={() => postDraftStockIn(selectedStockIn)}
                    disabled={isPosting}
                    className="w-full py-4 bg-emerald-600 text-white rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {selectedStockIn.type === "Production Output"
                      ? "Verifikasi & Posting ke Stok Barang Jadi"
                      : "Verifikasi & Posting ke Stok Gudang"}
                  </button>
                )}
            </div>
          </div>
