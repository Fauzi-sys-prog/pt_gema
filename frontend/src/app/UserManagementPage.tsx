...
export default function UserManagementPage() {
  const { userList, addUser, updateUser } = useApp();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingUser) {
      ...
      if (password.length < 10 || !/[A-Za-z]/.test(password) || !/[0-9]/.test(password)) {
        toast.error('Password minimal 10 karakter serta harus mengandung huruf dan angka.');
        return;
      }
      // Add new user
      const newUser: User = {
      ...
      try {
        ...
        toast.error(`User gagal ditambahkan: ${error instanceof Error ? error.message : 'Kesalahan server'}`);
        return;
      }
    }
  ...
  const handleDelete = (user: User) => {
    if (confirm(`Hapus user ${user.fullName}?`)) {
