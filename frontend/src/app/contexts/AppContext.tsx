...
export interface SuratJalan {
  ...
  pengirim?: string;
  deliveryStatus?:
    "Pending" | "On Delivery" | "Delivered" | "In Transit" | "Returned";
  deliveredAt?: string;
  podName?: string;
  podTime?: string;
  items: Array<{
...
export interface PayrollPolicy {
  ...
  incentiveDeductionRules: Array<{
    minAttendancePct: number;
    maxAttendancePct: number;
    deductionPct: number;
  }>;
...
export interface EmployeeCompensation {
  ...
  overtimeRate: number;
  effectiveDate?: string;
  bpjsKetEmployerPct?: number;
  bpjsKetEmployeePct?: number;
  bpjsKesPct?: number;
  bpjsKetEmployerAmount?: number;
  bpjsKetEmployeeAmount?: number;
  bpjsKesEmployeeAmount?: number;
  pph21Amount?: number;
}
...
export interface PayrollSlip {
  ...
  bpjsKetEmployee: number;
  bpjsKesEmployee: number;
  pph21: number;
  incentiveDeductionAmount: number;
  ...
  leaveDays?: number;
  holidayDays?: number;
  bpjsKetEmployer: number;
  bpjsKesEmployer: number;
  status: "Calculated" | "Approved" | "Disbursed" | "Closed";
  disbursedAt?: string;
...
export interface AppContextType {
  ...
  updateKoperasiMember: (
    id: string,
    updates: Partial<KoperasiMember>,
  ) => Promise<KoperasiMember>;
  koperasiSimpananList: KoperasiSimpanan[];
  addKoperasiSimpanan: (
    simpanan: KoperasiSimpanan,
  ) => Promise<KoperasiSimpanan>;
  ...
  topUpKoperasi: (topUp: {
    ...
    bankAccount: string;
    notes?: string;
  }) => Promise<KoperasiCashTransaction>;
  ...
  updateWorkingExpenseSheet: (
    id: string,
    updates: Partial<WorkingExpenseSheet>,
  ) => void;
  ...
  updateTHLTimesheet: (
    id: string,
    updates: Partial<THLTimesheetRecord>,
  ) => void;
  deleteTHLTimesheet: (id: string) => void;
  saveTHLTimesheetBatch: (
    thlId: string,
    periode: string,
    records: THLTimesheetRecord[],
  ) => void;
  ...
  updateProductionReport: (
    id: string,
    updates: Partial<ProductionReport>,
  ) => void;
  ...
  updatePettyCashGudangEntry: (
    id: string,
    updates: Partial<PettyCashEntry>,
  ) => void;
  ...
  updateStockIn: (id: string, updates: Partial<StockIn>) => Promise<void>;
  ...
  setBankSaldoAwal: (
    bank: string,
    amount: number,
    options?: BankOpeningBalanceOptions,
  ) => Promise<void>;
...
export const bankOpeningBalanceKey = (bank: string, period: string) =>
  `${bank}::${period}`;
...
const unwrapDataEntity = <T,>(entry: unknown): T => {
  ...
  const payload =
    carrier?.payload && typeof carrier.payload === "object"
...
const toInvoiceApiPayload = (invoice: Invoice, customerId?: string) => ({
  ...
  outstandingAmount: Math.max(
    0,
    invoice.totalBayar - (invoice.paidAmount ?? 0),
  ),
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const [materialRequestList, setMaterialRequestList] = useStoredState<
    MaterialRequest[]
  >("materialRequests", []);
  const [maintenanceList, setMaintenanceList] = useStoredState<
    MaintenanceRecord[]
  >("maintenanceList", []);
  const [invoiceList, setInvoiceList] = useStoredState<Invoice[]>(
    "invoiceList",
    [],
  );
  ...
  const [koperasiMembers, setKoperasiMembers] = useStoredState<
    KoperasiMember[]
  >("koperasiMembers", []);
  const [koperasiSimpananList, setKoperasiSimpananList] = useStoredState<
    KoperasiSimpanan[]
  >("koperasiSimpananList", []);
  const [koperasiPinjamanList, setKoperasiPinjamanList] = useStoredState<
    KoperasiPinjaman[]
  >("koperasiPinjamanList", []);
  const [koperasiBalance, setKoperasiBalance] = useStoredState<number>(
    "koperasiBalance",
    0,
  );
  const [koperasiTransactions, setKoperasiTransactions] = useStoredState<
    KoperasiCashTransaction[]
  >("koperasiTransactions", []);
  const [attendanceList, setAttendanceList] = useStoredState<Attendance[]>(
    "attendanceList",
    [],
  );
  const [workOrderList, setWorkOrderList] = useStoredState<WorkOrder[]>(
    "workOrderList",
    [],
  );
  const [productionReportList, setProductionReportList] = useStoredState<
    ProductionReport[]
  >("productionReportList", []);
  const [productionTrackerList, setProductionTrackerList] = useStoredState<
    ProductionTracker[]
  >("productionTrackerList", []);
  const [qcInspectionList, setQcInspectionList] = useStoredState<
    QCInspection[]
  >("qcInspectionList", []);
  const [stockInList, setStockInList] = useStoredState<StockIn[]>(
    "stockInList",
    [],
  );
  const [stockOutList, setStockOutList] = useStoredState<StockOut[]>(
    "stockOutList",
    [],
  );
  ...
  const [suratJalanList, setSuratJalanList] = useStoredState<SuratJalan[]>(
    "suratJalanList",
    [
      {
        id: "SJ-DEMO-1",
        ...
        items: [
          {
            namaItem: "Pipa Galvanis 2 Inch",
            itemKode: "GTP-MTR-PIP-001",
            jumlah: 50,
            satuan: "Batang",
            batchNo: "GTP/20260210/A12",
          },
          {
            namaItem: "Valve 2 Inch",
            itemKode: "GTP-MTR-VAL-002",
            jumlah: 12,
            satuan: "Pcs",
            batchNo: "GTP/20260210/B34",
          },
        ],
      ...
      },
      {
        ...
        alamat:
          "Jalan Malig VIII Lot T2 Kawasan Industri KIIC, Teluk Jambe Barat, Karawang",
        upPerson: "Ir. Wijaya",
        sopir: "Dewi",
        ...
        items: [
          {
            ...
            satuan: "Unit",
            keterangan:
              "Alat dipinjamkan untuk pekerjaan cor beton area plant 2",
          },
          {
            namaItem: "Vibrator",
            jumlah: 2,
            satuan: "Unit",
            keterangan:
              "Setelah pekerjaan sudah selesai alat harus dikembalikan",
          },
          {
            namaItem: "Trafo Las",
            jumlah: 1,
            satuan: "Unit",
            keterangan: "Untuk fabrikasi struktur baja",
          },
          {
            namaItem: "Gerinda Tangan",
            jumlah: 3,
            satuan: "Unit",
            keterangan: "Grinding & polishing work",
          },
        ],
  ...
  const [quotationList, setQuotationList] = useStoredState<Quotation[]>(
    "quotationList",
    [],
  );
  ...
  useEffect(() => {
    ...
    let active = true;
    const sync = async <T extends { id: string }>(
      path: string,
      cached: T[],
      apply: (rows: T[]) => void,
    ) => {
    ...
    Promise.all([
      ...
      console.warn(
        "[Assets] Memakai cache lokal karena API belum tersedia:",
        error,
      );
    });
  ...
  useEffect(() => {
    ...
    const syncFinanceProject = async () => {
      try {
        const [projects, invoices, vendorInvoices, customerInvoices] =
          await Promise.all([
        ...
        console.warn("[Finance/Project] Sync realtime gagal:", error);
      }
    };
    window.addEventListener("focus", syncFinanceProject);
    return () => {
      active = false;
      window.removeEventListener("focus", syncFinanceProject);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const syncQuotations = () => {
      api
        ...
        .catch((error) => {
          console.warn(
            "[Quotation] Memakai cache lokal karena API belum tersedia:",
            error,
          );
        });
    };
  ...
  const [vendorInvoiceList, setVendorInvoiceList] = useStoredState<
    VendorInvoice[]
  >(
    "vendorInvoiceList",
    DEMO_MODE_ENABLED ? (seedVendorInvoiceList as any) : [],
  );
  const [userList, setUserList] = useStoredState<User[]>("userList", []);
  const [projectList, setProjectList] = useStoredState<Project[]>(
    "projectList",
    DEMO_MODE_ENABLED ? (seedProjectList as any) : [],
  );
  const [dataCollectionList, setDataCollectionList] = useStoredState<
    DataCollection[]
  >(
    "dataCollectionList",
    DEMO_MODE_ENABLED ? (seedDataCollectionList as any) : [],
  );
  ...
  React.useEffect(() => {
    refreshHumanCapital().catch((error) => {
      console.warn(
        "[Human Capital] Memakai cache lokal karena API belum tersedia:",
        error,
      );
    });
  }, [refreshHumanCapital]);
  ...
  React.useEffect(() => {
    refreshProcurementInventory().catch((error) => {
      console.warn(
        "[Procurement/Inventory] Memakai cache lokal karena API belum tersedia:",
        error,
      );
    });
    const syncOnFocus = () => {
  ...
  const refreshProductionFlow = React.useCallback(async () => {
    ...
    const cachedWorkOrders = (() => {
      try {
        return JSON.parse(
          localStorage.getItem("gtp_erp_v1_workOrderList") || "[]",
        ) as WorkOrder[];
      } catch {
        return [];
      }
    ...
    const mergedWorkOrders = [
      ...workOrders,
      ...cachedWorkOrders.filter(
        (local) => !workOrders.some((server) => server.id === local.id),
      ),
    ];
  ...
  React.useEffect(() => {
    refreshProductionFlow().catch((error) => {
      console.warn(
        "[WO/LHP] Memakai cache lokal karena API belum tersedia:",
        error,
      );
    });
    const syncOnFocus = () => {
      refreshProductionFlow().catch(() => undefined);
    };
    window.addEventListener("focus", syncOnFocus);
    return () => {
      window.removeEventListener("focus", syncOnFocus);
  ...
  useEffect(() => {
    if (!localStorage.getItem("authToken")) return;
    let active = true;
    const syncDataCollections = () =>
      api
        .request<DataCollection[]>("/data-collections")
        .then((items) => {
          if (active) setDataCollectionList(items);
        })
        .catch((error) =>
          console.warn(
            "[Data Collection] Memakai cache lokal karena API belum tersedia:",
            error,
          ),
        );
    syncDataCollections();
  ...
  const [payrollList, setPayrollList] = useStoredState<Payroll[]>(
    "payrollList",
    [],
  );
  const [workingExpenseSheets, setWorkingExpenseSheets] = useStoredState<
    WorkingExpenseSheet[]
  >("workingExpenseSheets", []);
  const [kasbonList, setKasbonList] = useStoredState<KasbonEntry[]>(
    "kasbonList",
    [],
  );
  const [kasbonTHLList, setKasbonTHLList] = useStoredState<KasbonTHLEntry[]>(
    "kasbonTHLList",
    [],
  );
  const [thlTimesheetList, setTHLTimesheetList] = useStoredState<
    THLTimesheetRecord[]
  >("thlTimesheetList", []);
  const [thlSettlementList, setTHLSettlementList] = useStoredState<
    THLKasbonSettlement[]
  >("thlSettlementList", []);
  const [suratMasukList, setSuratMasukList] = useStoredState<SuratMasuk[]>(
    "suratMasukList",
    [],
  );
  const [suratKeluarList, setSuratKeluarList] = useStoredState<SuratKeluar[]>(
    "suratKeluarList",
    [],
  );
  const [beritaAcaraList, setBeritaAcaraList] = useStoredState<BeritaAcara[]>(
    "beritaAcaraList",
    DEMO_MODE_ENABLED ? (seedBeritaAcaraList as any) : [],
  );
  const [templateSuratList, setTemplateSuratList] = useStoredState<
    TemplateSurat[]
  >("templateSuratList", []);
  const [leaveList, setLeaveList] = useStoredState<Leave[]>("leaveList", []);
  const [overtimeList, setOvertimeList] = useStoredState<OvertimeRequest[]>(
    "overtimeList",
    [],
  );
  const [onlineEmployeeList, setOnlineEmployeeList] = useStoredState<
    OnlineEmployee[]
  >("onlineEmployeeList", []);
  const [penilaianKinerjaList, setPenilaianKinerjaList] = useStoredState<any[]>(
    "penilaianKinerjaList",
    [],
  );
  const [pettyCashList, setPettyCashList] = useStoredState<PettyCashEntry[]>(
    "pettyCashList",
    DEMO_MODE_ENABLED ? (seedPettyCashList as any) : [],
  );
  const [topUpRequestList, setTopUpRequestList] = useStoredState<
    TopUpRequest[]
  >("topUpRequestList", []);
  const [pettyCashGudangList, setPettyCashGudangList] = useStoredState<
    PettyCashEntry[]
  >("pettyCashGudangList", []);
  const [topUpGudangRequestList, setTopUpGudangRequestList] = useStoredState<
    TopUpRequest[]
  >("topUpGudangRequestList", []);
  const [archiveRegistry, setArchiveRegistry] = useStoredState<ArchiveEntry[]>(
    "archiveRegistry",
    [],
  );
  const [payrollRecords, setPayrollRecords] = useStoredState<PayrollRecord[]>(
    "payrollRecords",
    [],
  );
  const [auditLogs, setAuditLogs] = useStoredState<AuditLog[]>(
    "auditLogs",
    DEMO_MODE_ENABLED ? (seedAuditLogs as any) : [],
  );
  ...
  useEffect(() => {
    ...
    let active = true;
    api
      .request<Payroll[]>("/payrolls")
      .then((items) => {
        if (active) setPayrollList(items);
      })
      .catch((error) =>
        console.warn(
          "[Finance Payroll] Memakai cache lokal karena API belum tersedia:",
          error,
        ),
      );
    return () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((error) => {
        console.warn(
          "[Settings] Memakai cache lokal karena API belum tersedia:",
          error,
        );
      });
  ...
  const [vendorList, setVendorList] = useStoredState<Vendor[]>(
    "vendorList",
    DEMO_MODE_ENABLED ? (seedVendorList as any) : [],
  );

  const [expenseList, setExpenseList] = useStoredState<VendorExpense[]>(
    "expenseList",
    DEMO_MODE_ENABLED ? (seedExpenseList as any) : [],
  );

  // NEW: Customer & AR Management
  const [customerList, setCustomerList] = useStoredState<Customer[]>(
    "customerList",
    DEMO_MODE_ENABLED ? (seedCustomerList as any) : [],
  );

  const [customerInvoiceList, setCustomerInvoiceList] = useStoredState<
    CustomerInvoice[]
  >(
    "customerInvoiceList",
    DEMO_MODE_ENABLED ? (seedCustomerInvoiceList as any) : [],
  );
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((error) => {
        console.warn(
          "[Finance Approval] Memakai cache lokal karena API belum tersedia:",
          error,
        );
      });
    return () => {
    ...
    Promise.all([
      ...
      .then(
        ([
          ...
          setCustomerList(
            customers.map((row) => unwrapDataEntity<Customer>(row)),
          );
          setSuratJalanList(
            suratJalan.map((row) => unwrapDataEntity<SuratJalan>(row)),
          );
          setBeritaAcaraList(
            beritaAcara.map((row) => unwrapDataEntity<BeritaAcara>(row)),
          );
          setInvoiceList(invoices);
      ...
      .catch((error) => {
        console.warn(
          "[Core Business Data] Memakai cache lokal karena API belum tersedia:",
          error,
        );
      });
  ...
  const [stockOpnameList, setStockOpnameList] = useStoredState<StockOpname[]>(
    "stockOpnameList",
    [],
  );
  const [thlList, setThlList] = useStoredState<THL[]>(
    "thlList",
    DEMO_MODE_ENABLED ? (seedThlList as any) : [],
  );
  const [resignationList, setResignationList] = useStoredState<Resignation[]>(
    "resignationList",
    [],
  );
  const [shiftList, setShiftList] = useStoredState<Shift[]>(
    "shiftList",
    DEMO_MODE_ENABLED ? (seedShiftList as any) : [],
  );
  const [shiftScheduleList, setShiftScheduleList] = useStoredState<
    ShiftSchedule[]
  >("shiftScheduleList", []);

  const [salaryHistoryList, setSalaryHistoryList] = useStoredState<
    SalaryHistory[]
  >("salaryHistoryList", []);
  const [payrollPolicyState, setPayrollPolicyState] =
    useStoredState<PayrollPolicy | null>("payrollPolicy", {
      id: "policy-gtp-2024",
      policyName: "GTP Payroll Policy 2024",
      ...
      bpjsKesEmployer: 4,
      bpjsKesEmployee: 1,
      roundingRule: "nearest1000",
      signatoryPrepared: "Syamsudin",
      signatoryChecked: "Sri Rahayu",
      signatoryApproved: "Syamsudin",
      companyName: "PT Gema Teknik Perkasa",
      holidayDates: [],
    });
  const [employeeCompensationsState, setEmployeeCompensationsState] =
    useStoredState<EmployeeCompensation[]>("employeeCompensations", []);
  const [employeeAdvanceList, setEmployeeAdvanceList] = useStoredState<
    EmployeeAdvance[]
  >("employeeAdvanceList", []);
  const [payrollRunList, setPayrollRunList] = useStoredState<PayrollRun[]>(
    "payrollRunList",
    [],
  );
  ...
  useEffect(() => {
    ...
    Promise.all([
      syncList(
        "/hr-payroll-policies",
        payrollPolicyState ? [payrollPolicyState] : [],
        (rows) => setPayrollPolicyState(rows[0] ?? null),
      ),
      syncList(
        "/hr-employee-compensations",
        employeeCompensationsState,
        setEmployeeCompensationsState,
      ),
      syncList(
        "/hr-employee-advances",
        employeeAdvanceList,
        setEmployeeAdvanceList,
      ),
      ...
      syncList(
        ...
        workingExpenseSheets,
        setWorkingExpenseSheets,
      ),
    ]).catch((error) =>
      console.warn("[Human Capital Payroll] Memakai cache lokal:", error),
    );
    return () => {
      active = false;
    };
  ...
  const upsertHrAlias = <T extends { id: string }>(path: string, value: T) => {
    return api
      ...
      .catch((error) =>
        console.warn(`[Human Capital] Gagal menyimpan ${path}:`, error),
      );
  };
  const persistHrAlias = <T extends { id: string }>(
    path: string,
    value: T,
  ): Promise<T> =>
    api
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      syncList(
        ...
        pettyCashGudangList,
        setPettyCashGudangList,
      ),
      syncList(
        "/finance-warehouse-petty-cash-topups",
        topUpGudangRequestList,
        setTopUpGudangRequestList,
      ),
    ]).catch((error) =>
      console.warn(
        "[Kas Kecil] Memakai cache lokal karena API belum tersedia:",
        error,
      ),
    );
  ...
  useEffect(() => {
    ...
    const syncHumanCapitalLists = () =>
      Promise.all([
        ...
        console.warn("[Human Capital Attendance] Memakai cache lokal:", error),
      );
  ...
  const setEmployeeCompensation = (comp: EmployeeCompensation) => {
    ...
    setEmployeeCompensationsState((prev) => {
      return prev.find((c) => c.employeeId === comp.employeeId)
        ? prev.map((c) => (c.employeeId === comp.employeeId ? comp : c))
        : [...prev, comp];
    });
  ...
  const addPayrollRun = (run: PayrollRun) => {
    ...
    void persistHrAlias("/hr-payroll-runs", run).catch(() => {
      setPayrollRunList((prev) =>
        prev.filter((existing) => existing.id !== run.id),
      );
    });
  };
  ...
  const disbursePayrollRun = async (
    ...
    try {
      const posted = await api.request<{
        ...
        employeeAdvances: Array<{ id: string; payload: EmployeeAdvance }>;
      }>(`/koperasi/payroll-runs/${encodeURIComponent(id)}/post`, {
        method: "POST",
        body: JSON.stringify({ period: next.period, run: next }),
      });
      if (posted.employeeAdvances.length > 0) {
        const updatedById = new Map(
          posted.employeeAdvances.map((row) => [row.id, row.payload]),
        );
        setEmployeeAdvanceList((prev) =>
          prev.map((advance) => updatedById.get(advance.id) || advance),
        );
      }
      if (
  ...
  const deletePayrollRun = (id: string) => {
    ...
    void api
      .request(`/hr-payroll-runs/${encodeURIComponent(id)}`, {
        method: "DELETE",
      })
      .catch((error) =>
        console.warn("[Human Capital] Gagal menghapus payroll run:", error),
      );
  };
  const [extraCategories, setExtraCategories] = useStoredState<string[]>(
    "extraCategories",
    [],
  );
  useEffect(() => {
    ...
    api
      ...
      .then((rows) => {
        if (!active) return;
        setExtraCategories(
          rows.map((row) => row.name || row.id).filter(Boolean),
        );
      })
      .catch((error) =>
        console.warn(
          "[Kategori Gudang] Memakai cache lokal karena API belum tersedia:",
          error,
        ),
      );
    return () => {
  ...
  const addExtraCategory = (cat: string) => {
    ...
    api
      .request("/inventory-stock-categories", {
        method: "POST",
        body: JSON.stringify({ id, name: trimmed }),
      })
      .catch((error) => {
        setExtraCategories((prev) =>
          prev.filter((category) => category !== trimmed),
        );
        toast.error(`Kategori gudang gagal disimpan: ${error.message}`);
      });
  };

  const [bankSaldoAwal, setBankSaldoAwalMap] = useStoredState<
    Record<string, number>
  >("bankSaldoAwal", {});
  const setBankSaldoAwal = async (
    bank: string,
    amount: number,
    options: BankOpeningBalanceOptions = {},
  ) => {
    ...
    const payload = {
      ...
      balance: amount,
      note: [
        options.note || "Saldo awal rekening perusahaan",
        `Prioritas: ${options.priority || "Normal"}`,
      ].join(" · "),
    };
    try {
      await api
        .request(`/finance/bank-reconciliations/${encodeURIComponent(id)}`, {
          method: "PATCH",
          body: JSON.stringify(payload),
        })
        .catch(() =>
          api.request("/finance/bank-reconciliations", {
            method: "POST",
            body: JSON.stringify(payload),
          }),
        );
  ...
  const [bankManualEntryList, setBankManualEntryList] = useStoredState<
    BankManualEntry[]
  >("bankManualEntryList", []);
  ...
  React.useEffect(() => {
    ...
    api
      ...
      .then((rows) => {
        ...
        rows.forEach((row) => {
          if (row.periodLabel?.startsWith("OPENING_BALANCE") && row.account) {
            const period =
              row.periodLabel.split(":")[1] || row.date.slice(0, 7);
            balances[bankOpeningBalanceKey(row.account, period)] =
              row.balance || 0;
            if (typeof balances[row.account] === "undefined")
              balances[row.account] = row.balance || 0;
      ...
      .catch((error) =>
        console.warn("[Bank Reconciliation] Memakai cache lokal:", error),
      );
  }, []);

  const [closedYearsList, setClosedYearsList] = useStoredState<ClosedYear[]>(
    "closedYearsList",
    [],
  );
  ...
  React.useEffect(() => {
    api
      .request<ClosedYear[]>("/finance/closed-years")
      .then((rows) => setClosedYearsList(rows))
      .catch((error) =>
        console.warn("[Year-End Closing] Memakai cache lokal:", error),
      );
  }, []);
  ...
  const addClosedYear = (entry: Omit<ClosedYear, "id">) => {
    ...
    setClosedYearsList((prev) => [
      ...prev.filter((y) => y.year !== entry.year),
      saved,
    ]);
    api
      .request("/finance/closed-years", {
        method: "POST",
      ...
      .catch(async (error) => {
        try {
          await api.request(
            `/finance/closed-years/${encodeURIComponent(saved.id)}`,
            {
              method: "PATCH",
              body: JSON.stringify(saved),
            },
          ...
          console.error(
            "[Year-End Closing] Gagal menyimpan tutup buku:",
            error,
          );
  ...
  const isYearClosed = (year: number) =>
    closedYearsList.some((y) => y.year === year);

  const clearAllPayrollRuns = () => {
    const previous = payrollRunList;
    setPayrollRunList([]);
    void api
      .request("/hr-payroll-runs/bulk", {
        method: "PUT",
        body: JSON.stringify([]),
      })
      .catch((error) => {
  ...
  const addAuditLog = (
    ...
    api
      ...
      .then((saved) => {
        setAuditLogs((prev) =>
          prev.map((item) => (item.id === newLog.id ? saved : item)),
        );
      })
      .catch(() => {
  ...
  const addPettyCashEntry = async (entry: Omit<PettyCashEntry, "id">) => {
    ...
    if (localStorage.getItem("authToken")) {
      try {
        const saved = await persistHrAlias("/finance-petty-cash", next);
        setPettyCashList((prev) => [...prev, saved]);
      } catch {
        toast.error("Transaksi kas kecil gagal disimpan");
        return;
      }
  ...
  const addPettyCashGudangEntry = async (entry: Omit<PettyCashEntry, "id">) => {
    ...
    if (localStorage.getItem("authToken")) {
      try {
        const saved = await persistHrAlias(
          "/finance-warehouse-petty-cash",
          next,
        );
  ...
  const updatePettyCashGudangEntry = (
    id: string,
    updates: Partial<PettyCashEntry>,
  ) => {
  ...
  const addTopUpGudangRequest = (req: Omit<TopUpRequest, "id" | "status">) => {
    ...
    void persistHrAlias("/finance-warehouse-petty-cash-topups", next).catch(
      () => {
        setTopUpGudangRequestList((prev) =>
          prev.filter((item) => item.id !== next.id),
        );
  ...
  const approveTopUpGudangRequest = async (
    id: string,
    approverName: string,
  ) => {
    ...
    const lastBalance =
      pettyCashGudangList.length > 0
        ? pettyCashGudangList[pettyCashGudangList.length - 1].balance
        : 0;
  ...
  const addArchiveEntry = (entry: Omit<ArchiveEntry, "id">) => {
    ...
    api
      .request<ArchiveEntry>("/archive-registry", {
        method: "POST",
      ...
      .catch((error) => {
        ...
        toast.error(
          `Register arsip gagal disimpan ke database: ${error.message}`,
        );
      });
  };
  ...
  const addPayroll = (p: Payroll) => {
    ...
    api
      ...
      .then((saved) =>
        setPayrollList((prev) =>
          prev.map((item) => (item.id === p.id ? saved : item)),
        ),
      )
      .catch((error) => {
  ...
  const updateKasbon = (id: string, updates: Partial<KasbonEntry>) =>
    setKasbonList((prev) =>
      prev.map((k) => {
        if (k.id !== id) return k;
  ...
  const deleteKasbon = (id: string) => {
    ...
    void api
      .request(`/hr-kasbon-karyawan/${encodeURIComponent(id)}`, {
        method: "DELETE",
      })
      .catch(() => {});
  };
  ...
  const updateKasbonTHL = (id: string, updates: Partial<KasbonTHLEntry>) =>
    setKasbonTHLList((prev) =>
      prev.map((k) => {
        if (k.id !== id) return k;
        const next = { ...k, ...updates };
  ...
  const deleteKasbonTHL = (id: string) => {
    ...
    void api
      .request(`/hr-kasbon-thl/${encodeURIComponent(id)}`, { method: "DELETE" })
      .catch(() => {});
  };
  ...
  const deleteTHLTimesheet = (id: string) => {
    ...
    void api
      .request(`/hr-thl-timesheets/${encodeURIComponent(id)}`, {
        method: "DELETE",
      })
      .catch(() => {});
  };
  ...
  const deleteTHLSettlement = (id: string) => {
    ...
    void api
      .request(`/hr-thl-settlements/${encodeURIComponent(id)}`, {
        method: "DELETE",
      })
      .catch(() => {});
  };

  const [thlPayrollRunList, setTHLPayrollRunList] = useStoredState<
    THLPayrollRun[]
  >("thlPayrollRunList", []);
  useEffect(() => {
    if (!localStorage.getItem("authToken")) return;
    ...
    Promise.all([
      ...
      console.warn("[Human Capital THL] Memakai cache lokal:", error),
    );
    return () => {
      active = false;
    };
  ...
  const deleteTHLPayrollRun = (id: string) => {
    ...
    void api
      .request(`/hr-thl-payroll-runs/${encodeURIComponent(id)}`, {
        method: "DELETE",
      })
      .catch(() => {});
  };
  const saveTHLTimesheetBatch = (
    thlId: string,
    periode: string,
    records: THLTimesheetRecord[],
  ) => {
    ...
    void api
      .request("/hr-thl-timesheets/bulk", {
        method: "PUT",
        body: JSON.stringify(records),
      })
      .catch((error) =>
        console.warn(
          "[Human Capital THL] Gagal menyimpan batch timesheet:",
          error,
        ),
      );
  };
  ...
  const updatePayrollRecord = (id: string, updates: Partial<PayrollRecord>) => {
    ...
    setPayrollRecords((prev) =>
      prev.map((record) => (record.id === id ? next : record)),
    );
    void persistHrAlias("/finance-payroll-records", next).catch(() => {
      setPayrollRecords((prev) =>
        prev.map((record) => (record.id === id ? previous : record)),
      );
  ...
  const deleteUser = async (id: string): Promise<void> => {
    ...
    try {
      await api.request<void>(`/users/${encodeURIComponent(id)}`, {
        method: "DELETE",
      });
  ...
  const addLeave = (leave: Leave) => {
    ...
    return api
      ...
      .then((saved) => {
        ...
        setLeaveList((prev) =>
          prev.map((item) => (item.id === leave.id ? saved : item)),
        );
        return saved;
      })
      .catch((error) => {
  ...
  const updateLeave = (id: string, updates: Partial<Leave>) =>
    setLeaveList((prev) =>
      prev.map((l) => {
        ...
        void api
          .request(`/hr-leaves/${encodeURIComponent(id)}`, {
            method: "PATCH",
            body: JSON.stringify(next),
          })
          .catch((error) =>
            console.warn("[Human Capital] Gagal memperbarui cuti:", error),
          );
        return next;
      }),
    );
  ...
  const updateOvertime = (id: string, updates: Partial<OvertimeRequest>) =>
  ...
  const updatePenilaian = (id: string, updates: any) =>
    setPenilaianKinerjaList((prev) =>
      prev.map((p) => {
        if (p.id !== id) return p;
  ...
  const deletePenilaian = (id: string) => {
    ...
    void api
      .request(`/hr-performance-reviews/${encodeURIComponent(id)}`, {
        method: "DELETE",
      })
      .catch(() => {});
  };
  const addOnlineEmployee = (emp: OnlineEmployee) => {
    ...
    void api
      .request("/hr-online-status", {
        method: "POST",
        body: JSON.stringify(emp),
      })
      .catch(() => {});
  };
  const updateOnlineEmployee = (id: string, updates: Partial<OnlineEmployee>) =>
    setOnlineEmployeeList((prev) =>
      prev.map((e) => {
        if (e.id !== id) return e;
        const next = { ...e, ...updates };
        void api
          .request(`/hr-online-status/${encodeURIComponent(id)}`, {
            method: "PATCH",
            body: JSON.stringify(next),
          })
          .catch(() => {});
  ...
  const updateProject = async (
    ...
    try {
      const saved = await api.request<Project>(
      ...
      setProjectList((prev) =>
        prev.map((project) => (project.id === id ? saved : project)),
      );
      return true;
    } catch (error) {
      toast.error(
        `Project gagal diperbarui di database: ${error instanceof Error ? error.message : "Terjadi kesalahan"}`,
      );
      return false;
    }
  };
  const createProjectSpkWithWorkOrder = async (
    projectId: string,
    spk: Record<string, unknown>,
    workOrder: WorkOrder,
  ) => {
    const saved = await api.request<Project>(
      `/projects/${encodeURIComponent(projectId)}/spk-work-order`,
      {
        method: "POST",
        body: JSON.stringify({ spk, workOrder }),
      },
    );
    setProjectList((prev) =>
  ...
  const deleteEmployee = (id: string) => {
    ...
    api
      .request<void>(`/employees/${encodeURIComponent(id)}`, {
        method: "DELETE",
      })
      .catch((error) => {
  ...
  const updateAttendance = (id: string, updates: Partial<Attendance>) => {
    ...
    api
      .request<Attendance>(`/attendances/${encodeURIComponent(id)}`, {
        method: "PATCH",
        body: JSON.stringify(updates),
      })
      .then((saved) =>
  ...
  const deleteAttendance = (id: string) => {
    ...
    api
      .request<void>(`/attendances/${encodeURIComponent(id)}`, {
        method: "DELETE",
      })
      .catch((error) => {
  ...
  const addQCInspection = async (inspection: QCInspection) => {
    ...
    try {
      await api.request("/production/submit-qc", {
        method: "POST",
      ...
      toast.error(
        `QC gagal disimpan: ${error instanceof Error ? error.message : "kesalahan server"}`,
      );
      await Promise.all([
        refreshProductionFlow().catch(() => undefined),
        refreshProcurementInventory().catch(() => undefined),
  ...
  const addWorkOrder = (wo: WorkOrder) => {
    ...
    api
      ...
      .catch((error) => {
        toast.error(`Work Order gagal disimpan: ${error.message}`);
        refreshProductionFlow().catch(() => undefined);
      });
  };
  ...
  const startWorkOrder = async (id: string) => {
    try {
      await api.request(
        `/production/work-orders/${encodeURIComponent(id)}/start`,
        {
          method: "POST",
        },
      );
  ...
  const updateWorkOrder = (id: string, updates: Partial<WorkOrder>) => {
    ...
    api
      ...
      .then(() => refreshProductionFlow())
      .catch((error) => {
        toast.error(`Work Order gagal diperbarui: ${error.message}`);
        refreshProductionFlow().catch(() => undefined);
      });
  };
  const deleteWorkOrder = (id: string) => {
    ...
    api
      .request<void>(`/work-orders/${id}`, { method: "DELETE" })
      .then(() => refreshProductionFlow())
      .catch((error) => {
        toast.error(`Work Order gagal dihapus: ${error.message}`);
        refreshProductionFlow().catch(() => undefined);
      });
  };
  ...
  const createStockOut = (so: StockOut) => {
    ...
    so.items.forEach((item) => {
      const idx = updatedInventory.findIndex(
        (i) => i.kode === item.kode || i.nama === item.nama,
      );
      if (idx !== -1) {
        const stockBefore = updatedInventory[idx].stok;
        updatedInventory[idx] = {
          ...updatedInventory[idx],
          stok: stockBefore - item.qty,
        ...
        newMovements.push({
          ...
          notes:
            so.notes || (so.noWorkOrder ? `WO: ${so.noWorkOrder}` : undefined),
        });
      }
    });
    ...
    if (localStorage.getItem("authToken")) {
      api
        .request<StockOut>("/inventory/stock-outs", {
          method: "POST",
          ...
          toast.error(`Stock Out ${so.noStockOut} gagal disimpan`, {
            description:
              error instanceof Error
                ? error.message
                : "Periksa stok dan koneksi backend.",
          });
          refreshProcurementInventory().catch(() => undefined);
        });
    }
  ...
  const createStockIn = async (si: StockIn) => {
    ...
    await refreshProcurementInventory().catch(() => {
      toast.warning(
        "Stok masuk tersimpan. Muat ulang untuk memperbarui saldo stok.",
      );
    });
  };
  ...
  const addStockItem = (item: StockItem) => {
    ...
    api
      .request<StockItem>("/inventory/items", {
        method: "POST",
  ...
  const updateStockIn = async (
    id: string,
    updates: Partial<StockIn>,
  ): Promise<void> => {
    const current = stockInList.find((item) => item.id === id);
    if (!current) {
      throw new Error("Stock In tidak ditemukan.");
    }

    const merged = { ...current, ...updates };
    const saved = await api.request<StockIn>(`/inventory/stock-ins/${id}`, {
      method: "PATCH",
      body: JSON.stringify(merged),
    });
    ...
    await refreshProcurementInventory().catch(() => {
      toast.warning(
        "Stock In diperbarui. Muat ulang untuk memperbarui saldo stok.",
      );
    });
  };
  ...
  const addReceiving = async (rcv: Receiving) => {
    ...
    await refreshProcurementInventory().catch(() => {
      toast.warning(
        "Receiving tersimpan. Muat ulang untuk memperbarui daftar PO dan stok.",
      );
    });
  };

  const handleProductionOutput = (
    woId: string,
    qty: number,
    _workerName: string,
    ...
    if (wo.projectId) {
      ...
      const projectWOs = workOrderList
        .map((item) =>
          item.id === woId ? { ...item, completedQty: newCompleted } : item,
        )
        .filter((item) => item.projectId === wo.projectId);
      const totalTarget = projectWOs.reduce(
        (sum, item) => sum + item.targetQty,
        0,
      );
      const totalCompleted = projectWOs.reduce(
        (sum, item) => sum + (item.completedQty || 0),
        0,
      );
      const progress =
        totalTarget > 0
      ...
      if (typeof progress === "number")
        updateProject(wo.projectId, { progress });
    }
  };
  ...
  const addProductionReport = async (report: ProductionReport) => {
    ...
    try {
      ...
      await api.request<ProductionReport>("/production-reports", {
        method: "POST",
      ...
      toast.error(
        `LHP gagal disimpan: ${error instanceof Error ? error.message : "kesalahan server"}`,
      );
  ...
  const updateProductionReport = (
    ...
    const merged = { ...current, ...updates };
    setProductionReportList((prev) =>
      prev.map((report) => (report.id === id ? merged : report)),
    );
    if (merged.projectId) {
      api
        .request<ProductionReport>(`/production-reports/${id}`, {
          method: "PATCH",
          body: JSON.stringify(merged),
        })
        .then(() => refreshProductionFlow())
        .catch((error) => {
          toast.error(`LHP gagal diperbarui: ${error.message}`);
          refreshProductionFlow().catch(() => undefined);
        });
    }
  };
  const addBeritaAcara = (ba: BeritaAcara) => {
    setBeritaAcaraList((prev) => [
      ...prev.filter((item) => item.id !== ba.id),
      ba,
    ]);
    if (!localStorage.getItem("authToken")) return;

    api
      .request<unknown>("/data/berita-acara", {
        method: "POST",
      ...
      .then((saved) => {
        ...
        setBeritaAcaraList((prev) =>
          prev.map((item) => (item.id === ba.id ? persisted : item)),
        );
      })
      .catch((error) => {
        setBeritaAcaraList((prev) => prev.filter((item) => item.id !== ba.id));
        toast.error(
          `Berita Acara gagal disimpan ke database: ${error.message}`,
        );
      });
  };
  const updateBeritaAcara = (id: string, updates: Partial<BeritaAcara>) => {
    ...
    api
      .request<unknown>(`/data/berita-acara/${encodeURIComponent(id)}`, {
        method: "PATCH",
      ...
      .catch((error) => {
        ...
        toast.error(
          `Berita Acara gagal diperbarui di database: ${error.message}`,
        );
      });
  };
  const deleteBeritaAcara = (id: string) => {
    ...
    api
      .request(`/data/berita-acara/${encodeURIComponent(id)}`, {
        method: "DELETE",
      })
      .catch((error) => {
        if (removed) setBeritaAcaraList((prev) => [...prev, removed]);
        toast.error(
          `Berita Acara gagal dihapus dari database: ${error.message}`,
        );
      });
  };
  ...
  const updateQuotation = async (
    ...
    try {
      const saved = await api.request<Quotation>(
      ...
      toast.error(
        `Quotation gagal diperbarui: ${error instanceof Error ? error.message : "Kesalahan database"}`,
      );
      return undefined;
    }
  };
  const deleteQuotation = (id: string) => {
    ...
    api
      .request<void>(`/quotations/${encodeURIComponent(id)}`, {
        method: "DELETE",
      })
      .catch((error) => {
  ...
  const addSuratJalan = (sj: SuratJalan) => {
    setSuratJalanList((prev) => [
      ...prev.filter((item) => item.id !== sj.id),
      sj,
    ]);
    if (!localStorage.getItem("authToken")) return;

    api
      .request<unknown>("/data/surat-jalan", {
        method: "POST",
      ...
      .then((saved) => {
        ...
        setSuratJalanList((prev) =>
          prev.map((item) => (item.id === sj.id ? persisted : item)),
        );
      })
      .catch((error) => {
  ...
  const createSuratJalanWithStockOut = async (
    sj: SuratJalan,
    stockOut?: StockOut,
  ) => {
    if (!localStorage.getItem("authToken")) {
      ...
      if (stockOut)
        setStockOutList((prev) => [
          ...prev.filter((item) => item.id !== stockOut.id),
          stockOut,
        ]);
      return;
    }
  ...
  const updateSuratJalan = (id: string, updates: Partial<SuratJalan>) => {
    ...
    api
      .request<unknown>(`/data/surat-jalan/${encodeURIComponent(id)}`, {
        method: "PATCH",
        body: JSON.stringify({ payload: merged }),
      })
      .then((saved) => {
      ...
      .catch((error) => {
        ...
        toast.error(
          `Surat Jalan gagal diperbarui di database: ${error.message}`,
        );
      });
  };
  const deleteSuratJalan = (id: string) => {
    ...
    api
      .request(`/data/surat-jalan/${encodeURIComponent(id)}`, {
        method: "DELETE",
      })
      .catch((error) => {
        if (removed) setSuratJalanList((prev) => [...prev, removed]);
        toast.error(
          `Surat Jalan gagal dihapus dari database: ${error.message}`,
        );
      });
  };
  const addAsset = (a: Asset) => {
    ...
    api
      ...
      .then((saved) =>
        setAssetList((prev) =>
          prev.map((item) => (item.id === a.id ? saved : item)),
        ),
      )
      .catch((error) => {
  ...
  const addMaintenance = (m: MaintenanceRecord) => {
    ...
    api
      .request<MaintenanceRecord>("/maintenances", {
        method: "POST",
        body: JSON.stringify(m),
      })
      .then((saved) =>
        setMaintenanceList((prev) =>
          prev.map((item) => (item.id === m.id ? saved : item)),
        ),
      )
      .catch((error) => {
  ...
  const addInvoice = (inv: Invoice) => {
    ...
    api
      ...
      .then((saved) => {
        setInvoiceList((prev) =>
          prev.map((item) => (item.id === inv.id ? saved : item)),
        );
      })
      .catch((error) => {
  ...
  const updateInvoice = (id: string, updates: Partial<Invoice>) => {
    ...
    const merged = { ...current, ...updates };
    setInvoiceList((prev) =>
      prev.map((invoice) => (invoice.id === id ? merged : invoice)),
    );
    if (!localStorage.getItem("authToken")) return;
    ...
    api
      ...
      .then((saved) => {
        setInvoiceList((prev) =>
          prev.map((invoice) => (invoice.id === id ? saved : invoice)),
        );
      })
      .catch((error) => {
        setInvoiceList((prev) =>
          prev.map((invoice) => (invoice.id === id ? current : invoice)),
        );
  ...
  const createInvoiceWithAR = async (inv: Invoice): Promise<boolean> => {
    ...
    if (!resolvedCustomerId) {
      toast.error(
        "Pilih customer yang sudah terdaftar di Master Customer terlebih dahulu",
      );
      return false;
    }
    // Resolve projectName from projectList
    const matchedProject = inv.projectId
      ? projectList.find((p) => p.id === inv.projectId)
      : null;
    ...
    try {
      ...
      const persistedAr = await api.request<CustomerInvoice>(
      ...
      setCustomerInvoiceList((prev) => [
        ...prev.filter((item) => item.id !== arEntry.id),
        persistedAr,
      ]);
      addAuditLog({
      ...
      if (invoiceCreated) {
        void api.request<void>(`/invoices/${encodeURIComponent(inv.id)}`, {
          method: "DELETE",
        });
      }
      setInvoiceList((prev) => prev.filter((item) => item.id !== inv.id));
  ...
  const updatePO = (id: string, updates: Partial<any>) => {
    ...
    api
      .request<PurchaseOrder>(`/purchase-orders/${encodeURIComponent(id)}`, {
        method: "PATCH",
        body: JSON.stringify(next),
      })
      .then((saved) => {
  ...
  const approveProject = (id: string, ownerName: string) => {
    ...
    setProjectList((prev) =>
      prev.map((project) => (project.id === id ? optimistic : project)),
    );

    if (!localStorage.getItem("authToken")) return;

    api
      .request<Project>(`/projects/${encodeURIComponent(id)}/approval`, {
        method: "PATCH",
      ...
      .catch((error) => {
        setProjectList((prev) =>
          prev.map((project) => (project.id === id ? current : project)),
        );
  ...
  const updateMaterialRequest = (
    ...
    setMaterialRequestList((prev) =>
      prev.map((mr) => (mr.id === id ? { ...mr, ...updates } : mr)),
    );
  const issueMaterialRequest = (id: string, issuedBy: string) =>
    setMaterialRequestList((prev) =>
      prev.map((mr) => (mr.id === id ? { ...mr, status: "Issued" } : mr)),
    );
  const addDataCollection = (dc: DataCollection) => {
    setDataCollectionList((prev) => [
      ...prev.filter((item) => item.id !== dc.id),
      dc,
    ]);
    api
      .request<DataCollection>("/data-collections", {
  ...
  const updateDataCollection = (
    id: string,
    updates: Partial<DataCollection>,
  ) => {
    ...
    api
      .request<DataCollection>(`/data-collections/${encodeURIComponent(id)}`, {
        method: "PATCH",
        body: JSON.stringify(updates),
      })
      .then((saved) => {
  ...
  const deleteDataCollection = (id: string) => {
    ...
    api
      .request<void>(`/data-collections/${encodeURIComponent(id)}`, {
        method: "DELETE",
      })
      .catch((error) => {
  ...
  const addProject = async (project: Project): Promise<Project | undefined> => {
    ...
    try {
      ...
      const { approvalStatus, ...projectPayload } = project as Project & {
      ...
      if (approvalStatus === "Approved") {
        finalProject = await api.request<Project>(
          `/projects/${encodeURIComponent(saved.id)}/approval`,
          {
            method: "PATCH",
            body: JSON.stringify({ action: "APPROVE" }),
          },
      ...
      setProjectList((prev) =>
        prev.map((item) => (item.id === project.id ? finalProject : item)),
      );
      return finalProject;
    } catch (error) {
      ...
      if (message.includes("sudah dipakai oleh project lain")) {
        try {
          ...
          const linked = remoteProjects.find(
            (item) => item.quotationId === project.quotationId,
          );
          if (linked) {
            setProjectList((prev) => [
              ...prev.filter((item) => item.id !== linked.id),
              linked,
            ]);
            toast.info(
              `Quotation sudah terhubung ke Project ${linked.kodeProject || linked.id}.`,
            );
            return linked;
          }
        } catch {
      ...
      toast.error(
        `Project gagal disimpan ke database: ${message || "Kesalahan database"}`,
      );
      return undefined;
    }
  };
  ...
  const addVendorInvoice = (inv: VendorInvoice) => {
    ...
    if (localStorage.getItem("authToken")) {
      api
        .request<VendorInvoice>("/finance/vendor-invoices", {
          method: "POST",
        ...
        .catch((error) => {
          ...
          toast.error(
            `Invoice vendor gagal disimpan ke database: ${error.message}`,
          );
        });
    }
  };
    ...
    if (merged && localStorage.getItem("authToken")) {
      api
        .request<VendorInvoice>(
          `/finance/vendor-invoices/${encodeURIComponent(id)}`,
          {
            method: "PATCH",
            body: JSON.stringify(merged),
          },
        ...
        .catch((error) => {
          if (current)
            setVendorInvoiceList((prev) =>
              prev.map((inv) => (inv.id === id ? current : inv)),
            );
          toast.error(
            `Hutang vendor gagal disimpan ke database: ${error.message}`,
          );
        });
    }
  };
  ...
  const payVendorInvoice = async (
    ...
    const saved = await api.request<VendorInvoice>(
      `/finance/vendor-invoices/${encodeURIComponent(id)}/payments`,
      {
        method: "POST",
    ...
    setVendorInvoiceList((prev) =>
      prev.map((inv) => (inv.id === id ? { ...inv, ...saved } : inv)),
    );
    return saved;
  };
  ...
  const addSuratMasuk = (surat: SuratMasuk) => {
    ...
    if (!localStorage.getItem("authToken")) return;
    api
      .request<SuratMasuk>("/surat-masuk", {
        method: "POST",
        body: JSON.stringify(surat),
      })
      .then((saved) =>
        setSuratMasukList((prev) =>
          prev.map((item) => (item.id === surat.id ? saved : item)),
        ),
      )
      .catch((error) => {
        setSuratMasukList((prev) =>
          prev.filter((item) => item.id !== surat.id),
        );
  ...
  const updateSuratMasuk = (id: string, updates: Partial<SuratMasuk>) => {
    ...
    if (!localStorage.getItem("authToken")) return;
    api
      .request<SuratMasuk>(`/surat-masuk/${encodeURIComponent(id)}`, {
        method: "PATCH",
        body: JSON.stringify(merged),
      })
      .then((saved) =>
  ...
  const deleteSuratMasuk = (id: string) => {
    ...
    if (!localStorage.getItem("authToken")) return;
    api
      .request(`/surat-masuk/${encodeURIComponent(id)}`, { method: "DELETE" })
      .catch((error) => {
  ...
  const addSuratKeluar = (surat: SuratKeluar) => {
    ...
    if (!localStorage.getItem("authToken")) return;
    api
      .request<SuratKeluar>("/surat-keluar", {
        method: "POST",
        body: JSON.stringify(surat),
      })
      .then((saved) =>
  ...
  const updateSuratKeluar = (id: string, updates: Partial<SuratKeluar>) => {
    ...
    if (!localStorage.getItem("authToken")) return;
    api
      .request<SuratKeluar>(`/surat-keluar/${encodeURIComponent(id)}`, {
        method: "PATCH",
        body: JSON.stringify(merged),
      })
      .then((saved) =>
  ...
  const deleteSuratKeluar = (id: string) => {
    ...
    if (!localStorage.getItem("authToken")) return;
    api
      .request(`/surat-keluar/${encodeURIComponent(id)}`, { method: "DELETE" })
      .catch((error) => {
    ...
    setProjectList((prev) =>
      prev.map((p) => {
        if (p.id === request.projectId) {
          return {
            ...p,
            materialRequests: [...(p.materialRequests || []), request],
          };
        }
        return p;
      }),
  ...
  const recordProduction = async (woId: string, qty: number) =>
    handleProductionOutput(woId, qty, currentUser?.fullName || "System");
  ...
  const convertQuotationToProject = async (quoId: string) => {
    ...
    let existingProject = projectList.find(
      (project) => project.quotationId === quoId,
    );
    ...
    if (!existingProject && localStorage.getItem("authToken")) {
      try {
        const remoteProjects = await api.request<Project[]>("/projects");
        existingProject = remoteProjects.find(
          (project) => project.quotationId === quoId,
        );
        if (existingProject)
          setProjectList((prev) => [
            ...prev.filter((p) => p.id !== existingProject!.id),
            existingProject!,
          ]);
      } catch {
    ...
    if (existingProject) {
      toast.info(
        `Quotation sudah dikonversi ke Project ${existingProject.kodeProject || existingProject.id}.`,
      );
    ...
    if (!quo.grandTotal || quo.grandTotal === 0) {
      toast.error(
        "Grand Total masih Rp 0. Isi harga di Quotation sebelum konversi ke Project.",
      );
      return;
    }
    ...
    const boqManpower = (quo.manpower || []).map((m: any, i: number) => ({
      materialName:
        m.position || m.keterangan || m.materialName || `Manpower ${i + 1}`,
    ...
    const boqSections = (quo.sections || []).flatMap(
      (section: any, sectionIndex: number) =>
        (section.items || []).map((item: any, itemIndex: number) => ({
          materialName:
            item.keterangan || item.description || `Item ${itemIndex + 1}`,
          itemKode: item.id || `Q-${sectionIndex + 1}-${itemIndex + 1}`,
          category:
            section.nama || section.label || section.title || "Quotation",
    ...
    const newProj: Project = {
      ...
      boq:
        boqSections.length > 0
          ? boqSections
          : [...boqMaterials, ...boqManpower, ...boqEquipment],
      materialRequests: [],
      workingExpenses: [],
      fieldAttendance: [],
    };
    ...
    const approvedQuotation = await updateQuotation(quoId, {
      status: "Approved",
    ...
    try {
      const remoteProjects = await api.request<Project[]>("/projects");
      const savedProject = remoteProjects.find(
        (project) => project.quotationId === quoId,
      );
      if (savedProject) {
        setProjectList((prev) => [
          ...prev.filter((project) => project.id !== savedProject.id),
          savedProject,
        ]);
      }
    } catch {
      ...
      prev.map((item) => {
        ...
        if (mat) {
          return {
            ...item,
            stok:
              type === "reserve" ? item.stok - mat.qty : item.stok + mat.qty,
          };
        }
        return item;
      }),
  ...
  const refreshAll = () => {
    ...
    if (
      ...
      !quotationList.some((q) => q.id === "QO-2026-002")
    )
      setQuotationList(seedRefreshQuotations as any);
      ...
      setArchiveRegistry([
        {
          id: "REC-001",
          date: "2025-11-14",
          ref: "INV/GTP/2025/11/001",
          description: "Pembayaran Vendor Baja Utama",
          amount: 125000000,
          project: "Warehouse Cikande",
          admin: "Aris S.",
          type: "AP",
          source: "Finance",
        },
    ...
    if (localStorage.getItem("authToken")) {
      api
        ...
        .then((saved) => {
          ...
          setVendorList((prev) =>
            prev.map((item) => (item.id === vendor.id ? normalized : item)),
          );
        })
        .catch((error) => {
  ...
  const updateVendor = (id: string, updates: Partial<Vendor>) => {
    ...
    const merged = { ...current, ...updates };
    setVendorList((prev) =>
      prev.map((vendor) => (vendor.id === id ? merged : vendor)),
    );
    if (localStorage.getItem("authToken")) {
      api
        .request<unknown>(`/data/vendors/${encodeURIComponent(id)}`, {
          method: "PATCH",
          body: JSON.stringify({ payload: merged }),
        })
        .then((saved) => {
          const normalized = unwrapDataEntity<Vendor>(saved);
          setVendorList((prev) =>
            prev.map((vendor) => (vendor.id === id ? normalized : vendor)),
          );
        })
        .catch((error) => {
          setVendorList((prev) =>
            prev.map((vendor) => (vendor.id === id ? current : vendor)),
          );
  ...
  const deleteVendor = (id: string) => {
    ...
    if (localStorage.getItem("authToken")) {
      api
        .request<void>(`/data/vendors/${encodeURIComponent(id)}`, {
          method: "DELETE",
        })
        .catch((error) => {
  ...
  const addExpense = (expense: VendorExpense) => {
    ...
    if (localStorage.getItem("authToken")) {
      api
        .request<VendorExpense>("/finance/vendor-expenses", {
          method: "POST",
          body: JSON.stringify(expense),
        })
        .then((saved) => {
          setExpenseList((prev) =>
            prev.map((item) => (item.id === expense.id ? saved : item)),
          );
        })
        .catch((error) => {
          setExpenseList((prev) =>
          ...
          toast.error(
            `Tambahan biaya gagal disimpan ke database: ${error.message}`,
          );
        });
    }
    addAuditLog({
  ...
  const deleteExpense = (id: string) => {
    ...
    if (localStorage.getItem("authToken")) {
      api
        .request<void>(`/finance/vendor-expenses/${encodeURIComponent(id)}`, {
          method: "DELETE",
        })
        .catch((error) => {
          if (expense) setExpenseList((prev) => [...prev, expense]);
          toast.error(
            `Tambahan biaya gagal dihapus dari database: ${error.message}`,
          );
        });
    }
    addAuditLog({
    ...
    if (current && localStorage.getItem("authToken")) {
      api
        ...
        .catch((error) => {
          ...
          toast.error(
            `Penolakan biaya gagal disimpan ke database: ${error.message}`,
          );
        });
    }
    const expense = expenseList.find((e) => e.id === id);
  ...
  const addCustomer = (customer: Customer) => {
    ...
    if (localStorage.getItem("authToken")) {
      api
        .request<unknown>("/data/customers", {
          method: "POST",
        ...
        .then((saved) => {
          ...
          setCustomerList((prev) =>
            prev.map((item) => (item.id === customer.id ? normalized : item)),
          );
        })
        .catch((error) => {
          setCustomerList((prev) =>
  ...
  const updateCustomer = (id: string, updates: Partial<Customer>) => {
    ...
    const merged = { ...current, ...updates };
    setCustomerList((prev) =>
      prev.map((customer) => (customer.id === id ? merged : customer)),
    );
    if (localStorage.getItem("authToken")) {
      api
        .request<unknown>(`/data/customers/${encodeURIComponent(id)}`, {
          method: "PATCH",
          body: JSON.stringify({ payload: merged }),
        })
        .then((saved) => {
          ...
          setCustomerList((prev) =>
            prev.map((customer) =>
              customer.id === id ? normalized : customer,
            ),
          );
        })
        .catch((error) => {
          setCustomerList((prev) =>
            prev.map((customer) => (customer.id === id ? current : customer)),
          );
          toast.error(
            `Customer gagal diperbarui di database: ${error.message}`,
          );
        });
    }
    addAuditLog({
  ...
  const deleteCustomer = (id: string) => {
    ...
    if (localStorage.getItem("authToken")) {
      api
        .request<void>(`/data/customers/${encodeURIComponent(id)}`, {
          method: "DELETE",
        })
        .catch((error) => {
  ...
  const addCustomerInvoice = (invoice: CustomerInvoice) => {
    ...
    if (localStorage.getItem("authToken")) {
      api
        .request<CustomerInvoice>("/finance/customer-invoices", {
          method: "POST",
          body: JSON.stringify(invoice),
        })
        .then((saved) => {
          setCustomerInvoiceList((prev) =>
            prev.map((item) => (item.id === invoice.id ? saved : item)),
          );
        })
        .catch((error) => {
          setCustomerInvoiceList((prev) =>
          ...
          toast.error(
            `Invoice customer gagal disimpan ke database: ${error.message}`,
          );
        });
    }
  ...
  const updateCustomerInvoice = (
    ...
    if (merged && localStorage.getItem("authToken")) {
      api
        .request<CustomerInvoice>(
          `/finance/customer-invoices/${encodeURIComponent(id)}`,
          {
            method: "PATCH",
            body: JSON.stringify(merged),
          },
    ...
    if (updates.status === "Pending") {
      addAuditLog({
        ...
        module: "Finance",
        details: `Invoice ${invoice?.noInvoice} dikirim ke Approval Center`,
        status: "Success",
      });
    }
    if (updates.status === "Approved") {
      addAuditLog({
        ...
        module: "Finance",
        details: `Invoice ${invoice?.noInvoice} disetujui`,
        status: "Success",
      });
    }
    if (updates.status === "Rejected") {
      addAuditLog({
        ...
        module: "Finance",
        details: `Invoice ${invoice?.noInvoice} ditolak: ${(updates as any).approvalHistory?.slice(-1)[0]?.reason || "-"}`,
        status: "Warning",
      });
    }
    if (updates.status === "Revision") {
      addAuditLog({
        action: "Invoice Revision",
        module: "Finance",
        details: `Invoice ${invoice?.noInvoice} perlu revisi: ${(updates as any).approvalHistory?.slice(-1)[0]?.reason || "-"}`,
        status: "Warning",
      });
    }
  };
  ...
  const deleteCustomerInvoice = (id: string) => {
    ...
    if (localStorage.getItem("authToken")) {
      api
        .request<void>(`/finance/customer-invoices/${encodeURIComponent(id)}`, {
          method: "DELETE",
        })
        .catch((error) => {
          if (invoice) setCustomerInvoiceList((prev) => [...prev, invoice]);
          toast.error(
            `Invoice customer gagal dihapus dari database: ${error.message}`,
          );
        });
    }
  ...
  const approveCustomerInvoice = (id: string, approvedBy: string) => {
    ...
    if (arInv.invoiceId) {
      setInvoiceList((prev) =>
        prev.map((inv) =>
          inv.id === arInv.invoiceId && inv.status === "Draft"
    ...
    toast.success(
      `Invoice ${arInv.noInvoice} disetujui & dikirim ke customer.`,
    );
    addAuditLog({
      action: "Invoice Approved",
      module: "Finance",
      details: `Approve invoice ${arInv.noInvoice}`,
      status: "Success",
    });
  };
  ...
  const addInvoicePayment = async (
    ...
    const linkedInvId = invoiceId.startsWith("AR-")
      ? invoiceId.slice(3)
      : invoiceId;
    const current = customerInvoiceList.find(
      (inv) =>
        inv.id === invoiceId ||
        inv.id === linkedInvId ||
        (inv as any).invoiceId === linkedInvId,
    );
    if (!current) {
      toast.error(
        "Invoice AR tidak ditemukan. Pembayaran tidak dapat dicatat.",
      );
      return false;
    }
    ...
    const outstandingBeforePayment = Math.max(
      0,
      current.totalNominal - current.paidAmount,
    );
    ...
    if (!localStorage.getItem("authToken")) {
      toast.error(
        "Sesi login berakhir. Silakan login kembali sebelum mencatat pembayaran.",
      );
      return false;
    }
    ...
    {
      try {
        const saved = await api.request<CustomerInvoice>(
          `/finance/customer-invoices/${encodeURIComponent(current.id)}/payments`,
          {
            method: "POST",
            body: JSON.stringify(payment),
          },
        ...
        setCustomerInvoiceList((prev) =>
          prev.map((inv) => (inv.id === current.id ? saved : inv)),
        );
        setInvoiceList((prev) =>
          prev.map((inv) =>
            inv.id === linkedInvId || inv.noInvoice === current.noInvoice
              ? {
                  ...inv,
                  paidAmount: saved.paidAmount,
                  status: saved.status === "Paid" ? "Paid" : "Partial",
                }
              : inv,
          ),
        );
        ...
        toast.error(
          `Pembayaran gagal disimpan ke database: ${error instanceof Error ? error.message : "Terjadi kesalahan"}`,
        );
        return false;
      }
    }
    ...
    toast.success(
      `Pembayaran Rp ${payment.nominal.toLocaleString("id-ID")} berhasil dicatat!`,
    );
    void addAuditLog({
  ...
  const addStockOpname = (opname: StockOpname) => {
    ...
    api
      .request<StockOpname>("/inventory/stock-opnames", {
        method: "POST",
        body: JSON.stringify(opname),
      })
      .then((saved) => {
        setStockOpnameList((prev) =>
          prev.map((item) => (item.id === opname.id ? saved : item)),
        );
        return refreshProcurementInventory();
      })
      .catch((error) => {
        setStockOpnameList((prev) =>
          prev.filter((item) => item.id !== opname.id),
        );
        toast.error(
          `Stock Opname gagal disimpan ke database: ${error.message}`,
        );
      });
  ...
  const confirmStockOpname = (id: string, confirmedBy: string) => {
    ...
    setStockItemList((prev) =>
      prev.map((stock) => {
        ...
        return counted
          ? { ...stock, stok: counted.physicalQty, lastUpdate: completedAt }
          : stock;
      }),
    );
    api
      .request<StockOpname>(
        `/inventory/stock-opnames/${encodeURIComponent(id)}/confirm`,
        {
          method: "POST",
      ...
      .catch((error) => {
        ...
        setStockOpnameList((prev) =>
          prev.map((item) => (item.id === id ? opname : item)),
        );
  ...
  const updateTHL = (id: string, updates: Partial<THL>) =>
    setThlList((prev) =>
      prev.map((t) => {
        if (t.id !== id) return t;
  ...
  const deleteTHL = (id: string) => {
    ...
    void api
      .request(`/hr-thl-contracts/${encodeURIComponent(id)}`, {
        method: "DELETE",
      })
      .catch(() => {});
  };
  ...
  const updateResignation = (id: string, updates: Partial<Resignation>) =>
    setResignationList((prev) =>
      prev.map((r) => {
        if (r.id !== id) return r;
        const next = { ...r, ...updates };
  ...
  const deleteShift = (id: string) => {
    ...
    void api
      .request(`/hr-shifts/${encodeURIComponent(id)}`, { method: "DELETE" })
      .catch(() => {});
  };
  ...
  const deleteShiftSchedule = (id: string) => {
    ...
    void api
      .request(`/hr-shift-schedules/${encodeURIComponent(id)}`, {
        method: "DELETE",
      })
      .catch(() => {});
  };
  ...
  const updateKoperasiMember = async (
    id: string,
    updates: Partial<KoperasiMember>,
  ) => {
    const saved = await api.request<KoperasiMember>(
      `/koperasi/members/${encodeURIComponent(id)}`,
      { method: "PATCH", body: JSON.stringify(updates) },
    );
    setKoperasiMembers((prev) =>
      prev.map((member) => (member.id === id ? saved : member)),
    );
    return saved;
  };
  const addKoperasiSimpanan = async (simpanan: KoperasiSimpanan) => {
    const saved = await api.request<KoperasiSimpanan>("/koperasi/simpanan", {
      method: "POST",
      body: JSON.stringify(simpanan),
    });
    setKoperasiSimpananList((prev) => [
      ...prev.filter((item) => item.id !== saved.id),
      saved,
    ]);
  ...
  const bayarKoperasiAngsuran = async (id: string) => {
    ...
    if (!current)
      throw new Error("Pinjaman tidak ditemukan; muat ulang halaman");
    const saved = await api.request<KoperasiPinjaman>(
      `/koperasi/pinjaman/${encodeURIComponent(id)}/installments`,
      {
        method: "POST",
        body: JSON.stringify({
          installmentNumber: current.paidInstallments + 1,
        }),
      },
    );
    setKoperasiPinjamanList((prev) =>
  ...
  const topUpKoperasi = async (topUp: {
    id: string;
    date: string;
    amount: number;
    bankAccount: string;
    ...
    const saved = await api.request<KoperasiCashTransaction>(
      "/koperasi/top-ups",
      { method: "POST", body: JSON.stringify(topUp) },
    );
    setKoperasiTransactions((prev) => [
      saved,
      ...prev.filter((item) => item.id !== saved.id),
    ]);
    setKoperasiBalance((prev) => prev + saved.amount);
  ...
  useEffect(() => {
    ...
    const syncKoperasi = () =>
      api
        ...
        .catch((error) =>
          console.warn(
            "[Kas Koperasi] Memakai cache lokal karena API belum tersedia:",
            error,
          ),
        );
    syncKoperasi();
