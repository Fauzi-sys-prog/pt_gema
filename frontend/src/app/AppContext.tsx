...
export interface AppContextType {
  ...
  approveTopUpGudangRequest: (id: string, approverName: string) => void;
  ...
  setBankSaldoAwal: (bank: string, amount: number) => void;
  bankManualEntryList: BankManualEntry[];
  addBankManualEntry: (entry: Omit<BankManualEntry, 'id'>) => void;
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const disbursePayrollRun = async (id: string, updates: Partial<PayrollRun>) => {
    ...
    setPayrollRunList(prev => prev.map(run => run.id === id ? next : run));
    try {
      const posted = await api.request<{ postedLoanIds: string[]; employeeAdvances: Array<{ id: string; payload: EmployeeAdvance }> }>(`/koperasi/payroll-runs/${encodeURIComponent(id)}/post`, { method: 'POST', body: JSON.stringify({ period: next.period, run: next }) });
      ...
      if (posted.postedLoanIds.length > 0) {
        const summary = await api.request<{ pinjamans: KoperasiPinjaman[]; transactions: KoperasiCashTransaction[]; balance: number }>('/koperasi/summary');
  ...
  const setBankSaldoAwal = (bank: string, amount: number) => {
    const previousAmount = bankSaldoAwal[bank];
    setBankSaldoAwalMap(prev => ({ ...prev, [bank]: amount }));
    if (!localStorage.getItem('authToken')) return;
    const id = `BANK-BALANCE-${bank.replace(/[^a-z0-9]+/gi, '-').toUpperCase()}`;
    const payload = {
      id,
      date: new Date().toISOString().slice(0, 10),
      periodLabel: 'OPENING_BALANCE',
      account: bank,
      description: 'Saldo Awal Bank',
      debit: 0,
      credit: 0,
      balance: amount,
      note: 'Saldo awal rekening perusahaan',
    };
    api.request(`/finance/bank-reconciliations/${encodeURIComponent(id)}`, {
      ...
      toast.error(`Saldo awal bank gagal disimpan: ${error.message}`);
    });
  };

  const [bankManualEntryList, setBankManualEntryList] = useStoredState<BankManualEntry[]>('bankManualEntryList', []);
  const addBankManualEntry = (entry: Omit<BankManualEntry, 'id'>) => {
    ...
    if (localStorage.getItem('authToken')) {
      api.request('/finance/bank-reconciliations', {
        ...
        toast.error(`Mutasi bank gagal disimpan: ${error.message}`);
      });
    }
  };
  ...
  React.useEffect(() => {
    ...
    api.request<Array<{ id: string; date: string; periodLabel?: string; account?: string; description: string; debit: number; credit: number; balance: number; note?: string }>>('/finance/bank-reconciliations')
      .then(rows => {
        ...
        rows.forEach(row => {
          if (row.periodLabel === 'OPENING_BALANCE' && row.account) {
  ...
  const approveTopUpGudangRequest = (id: string, approverName: string) => {
    const req = topUpGudangRequestList.find(r => r.id === id);
    if (!req) return;
    const approved = { ...req, status: 'Approved' as const, approvedBy: approverName, approvedAt: new Date().toISOString() };
    const previousRequests = topUpGudangRequestList;
    const previousCash = pettyCashGudangList;
    setTopUpGudangRequestList(prev => prev.map(r => r.id === id ? approved : r));
    void persistHrAlias('/finance-warehouse-petty-cash-topups', approved).catch(() => {
      setTopUpGudangRequestList(previousRequests);
      toast.error('Persetujuan top-up kas gudang gagal disimpan');
    });
    if (req) {
      ...
      void persistHrAlias('/finance-warehouse-petty-cash', entry).catch(() => {
        setTopUpGudangRequestList(previousRequests);
        setPettyCashGudangList(previousCash);
        toast.error('Pencatatan top-up kas gudang gagal disimpan');
      });
    }
  };
  ...
  const updateProject = (id: string, updates: Partial<Project>) => {
    ...
    setProjectList(prev => prev.map(project => project.id === id ? merged : project));
    if (!localStorage.getItem('authToken')) return;

    api.request<Project>(`/projects/${encodeURIComponent(id)}`, {
      method: 'PATCH',
      ...
      toast.error(`Project gagal diperbarui di database: ${error.message}`);
    });
  };
  ...
  const updateVendorInvoice = (id: string, updates: Partial<VendorInvoice>) => {
    ...
    if (merged && localStorage.getItem('authToken')) {
      ...
      }).catch(error => toast.error(`Hutang vendor gagal disimpan ke database: ${error.message}`));
    }
  };
  ...
  const approveExpense = (id: string, approver: string) => {
    ...
    if (current && localStorage.getItem('authToken')) {
      ...
      }).catch(error => toast.error(`Approval biaya gagal disimpan ke database: ${error.message}`));
    }
    const expense = expenseList.find(e => e.id === id);
  ...
  const rejectExpense = (id: string, reason: string) => {
    ...
    if (current && localStorage.getItem('authToken')) {
      ...
      }).catch(error => toast.error(`Penolakan biaya gagal disimpan ke database: ${error.message}`));
    }
    const expense = expenseList.find(e => e.id === id);
