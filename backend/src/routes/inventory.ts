...
function inventoryProjectName(
  project: { payload: unknown } | null | undefined,
): string | undefined {
  const payload = asRecord(project?.payload);
  return (
    asTrimmedString(
      payload.namaProject ?? payload.projectName ?? payload.name,
    ) ?? undefined
  );
}

function inventoryPoNumber(
  po: { payload: unknown } | null | undefined,
): string | undefined {
  const payload = asRecord(po?.payload);
  return (
    asTrimmedString(payload.noPO ?? payload.number ?? payload.id) ?? undefined
  );
}
...
async function resolveInventoryWorkOrderRef(ref: string | null | undefined) {
  ...
  if (relationalById) {
    return (
      (await findLinkedLegacyInventoryWorkOrder(
        relationalById.id,
...
async function findInventoryItemByCodeOrName(
  tx: InventoryTx,
  code: string,
  name: string,
) {
...
async function inventoryTransaction<T>(
...
async function applyStockOutInventory(tx: InventoryTx, stockOutId: string) {
  ...
  for (const [index, item] of stockOut.items.entries()) {
    ...
    if (!inventoryItem)
      throw new Error(
        `Material '${item.itemCode || item.itemName}' tidak ditemukan di stok`,
      );
...
async function applyStockInInventory(tx: InventoryTx, stockInId: string) {
  const stockIn = await tx.inventoryStockIn.findUnique({
    where: { id: stockInId },
    include: { items: true },
  });
  ...
  const defaultLocation =
    asTrimmedString(legacy.warehouseLocation) ||
  ...
  for (const [index, item] of stockIn.items.entries()) {
    ...
    if (!inventoryItem) {
      inventoryItem = await tx.inventoryItem.create({
        data: {
          ...
          reservedQty: 0,
          onOrderQty: 0,
          lastStockUpdateAt: stockIn.tanggal,
          metadata: {
            source: "stock-in",
            stok: 0,
    ...
    await tx.inventoryStockInItem.update({
      where: { id: item.id },
      data: { inventoryItemId: inventoryItem.id },
    });
    await tx.inventoryStockMovement.create({
      data: {
        id: `${stockInId}-MOV-${String(index + 1).padStart(3, "0")}`,
        tanggal: stockIn.tanggal,
        direction: "IN",
        referenceNo: stockIn.number,
        referenceType: stockIn.type,
        inventoryItemId: inventoryItem.id,
        itemCode: inventoryItem.code,
        itemName: inventoryItem.name,
        qty: item.qty,
        ...
        createdByName: stockIn.createdByName,
        projectId: stockIn.projectId,
        stockInId,
        legacyPayload: {
          source: "stock-in",
          qcInspectionId: legacy.qcInspectionId,
        } as Prisma.InputJsonValue,
      },
    });
  }
...
function mapInventoryStockIn(row: {
  ...
  return {
    ...
    noSuratJalan:
      asTrimmedString(legacy.noSuratJalan) ?? row.suratJalanNumber ?? undefined,
    supplier: asTrimmedString(legacy.supplier) ?? row.supplierName ?? "",
    projectId: asTrimmedString(legacy.projectId) ?? row.projectId ?? undefined,
    projectName:
      asTrimmedString(legacy.projectName) ?? inventoryProjectName(row.project),
    tanggal:
      asTrimmedString(legacy.tanggal) ?? row.tanggal.toISOString().slice(0, 10),
    type: asTrimmedString(legacy.type) ?? row.type,
    status: asTrimmedString(legacy.status) ?? row.status,
    createdBy:
      asTrimmedString(legacy.createdBy) ?? row.createdByName ?? "SYSTEM",
    ...
    receivingId:
      row.receivingId ?? asTrimmedString(legacy.receivingId) ?? undefined,
    stockCategory: asTrimmedString(legacy.stockCategory) ?? undefined,
    items: row.items.map((item, index) => ({
      kode: item.itemCode,
      nama: item.itemName,
      kategori:
        asTrimmedString(legacyItems[index]?.kategori) ??
        asTrimmedString(legacy.stockCategory) ??
        undefined,
      qty: item.qty,
      satuan: item.unit,
...
function mapInventoryStockOut(row: {
  ...
  return {
    ...
    noWorkOrder:
      asTrimmedString(legacy.noWorkOrder) ?? row.workOrderId ?? undefined,
    workOrderId:
      asTrimmedString(legacy.workOrderId) ?? row.workOrderId ?? undefined,
    productionReportId:
      asTrimmedString(legacy.productionReportId) ??
      row.productionReportId ??
      undefined,
    projectId: asTrimmedString(legacy.projectId) ?? row.projectId ?? undefined,
    projectName:
      asTrimmedString(legacy.projectName) ?? inventoryProjectName(row.project),
...
function mapInventoryOpname(row: {
  ...
  return {
    ...
    confirmedBy:
      asTrimmedString(legacy.confirmedBy) ?? row.confirmedByName ?? undefined,
...
async function writeAuditLog(
  ...
  db: typeof prisma | InventoryTx = prisma,
) {
...
async function assertRefs(
  ...
  const workOrderRef = asTrimmedString(
    payload.workOrderId ?? payload.noWorkOrder,
  );
  if (
    resource === "stock-outs" &&
    asTrimmedString(payload.type) === "Project Issue" &&
    !projectId
  ) {
    throw new Error(
      "stock-outs: projectId wajib untuk transaksi Project Issue",
    );
  }
  if (projectId) {
    ...
    if (!row)
      throw new Error(`${resource}: projectId '${projectId}' tidak ditemukan`);
  }
  if (poId) {
    ...
    if (projectId && row.projectId && row.projectId !== projectId)
      throw new Error(
        `${resource}: projectId '${projectId}' tidak match dengan projectId PO '${row.projectId}'`,
      );
  }
  if (workOrderRef) {
    ...
    if (projectId && row?.projectId && row.projectId !== projectId)
      throw new Error(
        `${resource}: projectId '${projectId}' tidak match dengan projectId WO '${row.projectId}'`,
      );
  }
}
...
export async function validateStockIn(
  tx: InventoryTx,
  payload: Record<string, unknown>,
  id: string,
) {
  ...
  if (
    ...
    items.some(
      (item) =>
        !asTrimmedString(item.kode ?? item.itemKode) ||
        !Number.isFinite(Number(item.qty)) ||
        Number(item.qty) <= 0,
    )
  ) {
    throw new Error(
      "Stok masuk tidak valid: isi SKU dan qty positif pada setiap baris.",
    );
  }
  const receivingId = asTrimmedString(payload.receivingId);
  if (
    !receivingId &&
    ["Receiving", "Purchase Receiving"].includes(String(payload.type))
  ) {
    throw new Error(
      "Stok masuk penerimaan tidak valid: pilih dokumen Receiving.",
    );
  }
  if (!receivingId) return;
  const receiving = await tx.procurementReceiving.findUnique({
    where: { id: receivingId },
    include: { items: true },
  });
  if (
    !receiving ||
    !["Partial", "Complete", "Completed"].includes(receiving.status)
  ) {
    throw new Error("Receiving tidak ditemukan atau belum dapat diposting.");
  }
  const duplicate = await tx.inventoryStockIn.findFirst({
    where: {
      id: { not: id },
      OR: [
        { receivingId },
        ...
        ...(receiving.suratJalanNo
          ? [
              {
                suratJalanNumber: receiving.suratJalanNo,
  ...
  if (duplicate)
    throw new Error(
      "Receiving tidak dapat diposting ulang: sudah memiliki Stok Masuk.",
    );
  ...
  for (const item of items) {
    ...
    if (qty > remaining)
      throw new Error(
        "Qty stok masuk tidak boleh melebihi barang baik pada Receiving.",
      );
    allowed.set(code, remaining - qty);
  }
  if ([...allowed.values()].some((qty) => qty > 0.000001)) {
    throw new Error(
      "Stok masuk tidak lengkap: seluruh qty barang baik harus diposting bersama.",
    );
  }
  ...
  switch (resource) {
      ...
      await inventoryTransaction(async (tx) => {
        ...
        await tx.inventoryItem.create({
          data: {
            ...
            code,
            name,
            category: asTrimmedString(payload.kategori) || "General",
            unit,
            location,
            minStock: toFiniteNumber(payload.minStock, 0),
            onHandQty: openingQty,
            reservedQty: toFiniteNumber(payload.reserved, 0),
            onOrderQty: toFiniteNumber(payload.onOrderQty, 0),
            unitPrice:
              payload.hargaSatuan == null
                ? undefined
                : toFiniteNumber(payload.hargaSatuan, 0),
            supplierName: asTrimmedString(payload.supplier) || undefined,
            status: asTrimmedString(payload.status) || undefined,
            lastStockUpdateAt: payload.lastUpdate
              ? new Date(String(payload.lastUpdate))
              : undefined,
    ...
    case "stock-ins":
      {
        await inventoryTransaction(async (tx) => {
          ...
          const tanggal = new Date(
            inventoryDateString(asTrimmedString(payload.tanggal)),
          );
          ...
          const receivingId = asTrimmedString(payload.receivingId);
          const status = receivingId
            ? "Draft"
            : asTrimmedString(payload.status) || "Draft";

          payload.status = status;
          const location =
            asTrimmedString(payload.warehouseLocation) ||
            (asTrimmedString(payload.type) === "Production Output"
              ? "Gudang Barang Jadi"
              : "Gudang Utama");
          const rawItems = (
          ...
          for (const raw of rawItems) {
            ...
            if (status !== "Posted") {
              normalizedItems.push({
                ...
                expiryDate: asTrimmedString(raw.expiryDate)
                  ? new Date(String(raw.expiryDate))
                  : undefined,
                notes: asTrimmedString(raw.notes) || undefined,
              });
              continue;
            }
            const existingItem = await findInventoryItemByCodeOrName(
              tx,
              code,
              name,
            );
          ...
          await tx.inventoryStockIn.create({
            data: {
              id: entityId,
              number: asTrimmedString(payload.noStockIn) || entityId,
              tanggal,
              type: asTrimmedString(payload.type) || "Receiving",
              status,
              supplierName,
              suratJalanNumber:
                asTrimmedString(payload.noSuratJalan) || undefined,
              notes: asTrimmedString(payload.notes) || undefined,
              createdByName,
              poId: asTrimmedString(payload.poId) || undefined,
                ...
                create:
                  status === "Posted"
                    ? normalizedItems.map((item, index) => ({
                        ...
                        direction: "IN",
                        referenceNo:
                          asTrimmedString(payload.noStockIn) || entityId,
                        referenceType:
                          asTrimmedString(payload.type) || "Stock In",
                        inventoryItem: item.inventoryItemId
                          ? { connect: { id: item.inventoryItemId } }
                          : undefined,
                        itemCode: item.code,
                        ...
                        createdByName,
                        projectId:
                          asTrimmedString(payload.projectId) || undefined,
                        legacyPayload: {
                          source: "stock-in",
                          stockInId: entityId,
                        } as Prisma.InputJsonValue,
    ...
    case "stock-outs": {
      ...
      await inventoryTransaction(async (tx) => {
        await tx.inventoryStockOut.create({
          data: {
            id: entityId,
            number: asTrimmedString(payload.noStockOut) || entityId,
            tanggal: new Date(
              inventoryDateString(asTrimmedString(payload.tanggal)),
            ),
            type: asTrimmedString(payload.type) || "Project Issue",
            status: asTrimmedString(payload.status) || "Draft",
            recipientName: asTrimmedString(payload.penerima) || undefined,
            notes: asTrimmedString(payload.notes) || undefined,
            createdByName: asTrimmedString(payload.createdBy) || undefined,
            projectId: asTrimmedString(payload.projectId) || undefined,
            workOrderId: resolvedWorkOrder?.id || undefined,
            productionReportId:
              asTrimmedString(payload.productionReportId) || undefined,
            legacyPayload: payload as Prisma.InputJsonValue,
            items: {
    ...
    case "stock-movements":
      await prisma.inventoryStockMovement.create({
        data: {
          ...
          direction: asTrimmedString(payload.type) || "IN",
          referenceNo: asTrimmedString(payload.refNo) || entityId,
          referenceType: asTrimmedString(payload.refType) || "Manual",
          itemCode: asTrimmedString(payload.itemKode) || "",
          itemName: asTrimmedString(payload.itemNama) || "",
          qty: toFiniteNumber(payload.qty, 0),
          unit: asTrimmedString(payload.unit) || "pcs",
          location: asTrimmedString(payload.lokasi) || "Gudang Utama",
          stockBefore: toFiniteNumber(payload.stockBefore, 0),
          stockAfter: toFiniteNumber(payload.stockAfter, 0),
          batchNo: asTrimmedString(payload.batchNo) || undefined,
          expiryDate: asTrimmedString(payload.expiryDate)
          ...
          createdByName: asTrimmedString(payload.createdBy) || undefined,
          projectId: asTrimmedString(payload.projectId) || undefined,
          stockInId: asTrimmedString(payload.stockInId) || undefined,
          stockOutId: asTrimmedString(payload.stockOutId) || undefined,
          stockOpnameId: asTrimmedString(payload.stockOpnameId) || undefined,
          legacyPayload: payload as Prisma.InputJsonValue,
        },
      });
      break;
    case "stock-opnames":
      ...
      await prisma.inventoryStockOpname.create({
        data: {
          id: entityId,
          number: asTrimmedString(payload.noOpname) || entityId,
          tanggal: new Date(
            inventoryDateString(asTrimmedString(payload.tanggal)),
          ),
          location: asTrimmedString(payload.lokasi) || "Gudang Utama",
          ...
          notes: asTrimmedString(payload.notes) || undefined,
          createdByName: asTrimmedString(payload.createdBy) || undefined,
          confirmedByName: asTrimmedString(payload.confirmedBy) || undefined,
          confirmedAt: asTrimmedString(payload.confirmedAt)
            ? new Date(String(payload.confirmedAt))
            : undefined,
          legacyPayload: payload as Prisma.InputJsonValue,
          items: {
            create: (Array.isArray(payload.items) ? payload.items : [])
...
async function updateResource(
  ...
  switch (resource) {
    case "stock-items":
      await inventoryTransaction(async (tx) => {
        ...
        if (
          ...
          payload.updatedAt !== current.updatedAt.toISOString()
        ) {
        ...
        const changed = await tx.inventoryItem.updateMany({
          ...
          data: {
            code: asTrimmedString(payload.kode) || id,
            name: asTrimmedString(payload.nama) || id,
            category: asTrimmedString(payload.kategori) || "General",
            unit: asTrimmedString(payload.satuan) || "pcs",
            location: asTrimmedString(payload.lokasi) || "Gudang Utama",
            minStock: toFiniteNumber(payload.minStock, 0),
            ...
            unitPrice:
              payload.hargaSatuan == null
                ? null
                : toFiniteNumber(payload.hargaSatuan, 0),
            supplierName: asTrimmedString(payload.supplier) || null,
            status: asTrimmedString(payload.status) || null,
            lastStockUpdateAt: asTrimmedString(payload.lastUpdate)
              ? new Date(String(payload.lastUpdate))
              : null,
            metadata: payload as Prisma.InputJsonValue,
          },
    ...
    case "stock-ins":
      await inventoryTransaction(async (tx) => {
        const currentStockIn = await tx.inventoryStockIn.findUniqueOrThrow({
          where: { id },
          include: { items: true },
        });
        if (currentStockIn.status === "Posted") {
          throw new Error("INVENTORY_TRANSACTION_POSTED");
        }
        ...
        await tx.inventoryStockIn.update({
          where: { id },
          data: {
            number: asTrimmedString(payload.noStockIn) || id,
            tanggal: new Date(
              inventoryDateString(asTrimmedString(payload.tanggal)),
            ),
            type: asTrimmedString(payload.type) || "Receiving",
            status: asTrimmedString(payload.status) || "Draft",
            supplierName: asTrimmedString(payload.supplier) || null,
            suratJalanNumber: asTrimmedString(payload.noSuratJalan) || null,
            notes: asTrimmedString(payload.notes) || null,
            createdByName: asTrimmedString(payload.createdBy) || null,
            poId: asTrimmedString(payload.poId) || null,
            projectId: asTrimmedString(payload.projectId) || null,
            receivingId: asTrimmedString(payload.receivingId) || null,
            legacyPayload: payload as Prisma.InputJsonValue,
            items: {
              deleteMany: {},
              create: (Array.isArray(payload.items) ? payload.items : [])
                .map((raw, index) => {
                  ...
                  return {
                    ...
                    itemCode: asTrimmedString(item.kode ?? item.itemKode) || "",
                    itemName: asTrimmedString(item.nama ?? item.itemName) || "",
                    qty: toFiniteNumber(item.qty, 0),
    ...
    case "stock-outs": {
      ...
      await inventoryTransaction(async (tx) => {
        const currentStockOut = await tx.inventoryStockOut.findUniqueOrThrow({
          where: { id },
          select: { status: true },
        });
        if (currentStockOut.status === "Posted") {
          throw new Error("INVENTORY_TRANSACTION_POSTED");
        }
        await reverseStockOutInventory(tx, id);
        await tx.inventoryStockOut.update({
          where: { id },
          data: {
            number: asTrimmedString(payload.noStockOut) || id,
            tanggal: new Date(
              inventoryDateString(asTrimmedString(payload.tanggal)),
            ),
            type: asTrimmedString(payload.type) || "Project Issue",
            status: asTrimmedString(payload.status) || "Draft",
            recipientName: asTrimmedString(payload.penerima) || null,
            notes: asTrimmedString(payload.notes) || null,
            createdByName: asTrimmedString(payload.createdBy) || null,
            projectId: asTrimmedString(payload.projectId) || null,
            workOrderId: resolvedWorkOrder?.id || null,
            productionReportId:
              asTrimmedString(payload.productionReportId) || null,
            legacyPayload: payload as Prisma.InputJsonValue,
            items: {
              deleteMany: {},
              create: (Array.isArray(payload.items) ? payload.items : [])
    ...
    case "stock-movements":
      await prisma.inventoryStockMovement.update({
        ...
        data: {
          ...
          direction: asTrimmedString(payload.type) || "IN",
          referenceNo: asTrimmedString(payload.refNo) || id,
          referenceType: asTrimmedString(payload.refType) || "Manual",
          itemCode: asTrimmedString(payload.itemKode) || "",
          itemName: asTrimmedString(payload.itemNama) || "",
          qty: toFiniteNumber(payload.qty, 0),
          ...
          stockBefore: toFiniteNumber(payload.stockBefore, 0),
          stockAfter: toFiniteNumber(payload.stockAfter, 0),
          batchNo: asTrimmedString(payload.batchNo) || null,
          expiryDate: asTrimmedString(payload.expiryDate)
            ? new Date(String(payload.expiryDate))
            : null,
          supplierName: asTrimmedString(payload.supplier) || null,
          poNumber: asTrimmedString(payload.noPO) || null,
          createdByName: asTrimmedString(payload.createdBy) || null,
          projectId: asTrimmedString(payload.projectId) || null,
          stockInId: asTrimmedString(payload.stockInId) || null,
          stockOutId: asTrimmedString(payload.stockOutId) || null,
          stockOpnameId: asTrimmedString(payload.stockOpnameId) || null,
          legacyPayload: payload as Prisma.InputJsonValue,
        },
      });
      break;
    case "stock-opnames":
      await inventoryTransaction(async (tx) => {
        const currentOpname = await tx.inventoryStockOpname.findUniqueOrThrow({
          where: { id },
          include: { items: true },
        });

        if (currentOpname.status === "Completed") {
          throw new Error("STOCK_OPNAME_COMPLETED");
        }

        const requestedStatus = asTrimmedString(payload.status);
        if (requestedStatus === "Completed") {
          throw new Error("STOCK_OPNAME_CONFIRM_REQUIRED");
        }
        ...
        await tx.inventoryStockOpname.update({
          where: { id },
          data: {
            number: asTrimmedString(payload.noOpname) || id,
            tanggal: new Date(
              inventoryDateString(asTrimmedString(payload.tanggal)),
            ),
            location: asTrimmedString(payload.lokasi) || "Gudang Utama",
...
async function deleteResource(
  ...
  switch (resource) {
    ...
    case "stock-outs":
      ...
      return;
    case "stock-movements":
      await prisma.inventoryStockMovement.delete({ where: { id } });
      return;
    case "stock-opnames":
      await inventoryTransaction(async (tx) => {
        const currentOpname = await tx.inventoryStockOpname.findUniqueOrThrow({
        ...
        await tx.inventoryStockOpname.delete({ where: { id } });
  ...
  inventoryRouter.patch(
    ...
    async (req: AuthRequest, res: Response) => {
      ...
      try {
        ...
        if (err instanceof Error && err.message === "STOCK_VERSION_CONFLICT")
          return sendError(res, 409, {
            code: "STOCK_VERSION_CONFLICT",
            message:
              "Data stok berubah atau versi belum tersedia. Muat ulang data sebelum menyimpan.",
            legacyError: "Muat ulang data stok sebelum menyimpan.",
          });
        if (err instanceof Error && err.message === "STOCK_BALANCE_READ_ONLY")
          return sendError(res, 409, {
            code: "STOCK_BALANCE_READ_ONLY",
            message:
              "Saldo stok tidak dapat diubah langsung. Gunakan transaksi Stock In, Stock Out, atau penyesuaian stok.",
            legacyError:
              "Saldo stok tidak dapat diubah langsung. Gunakan transaksi stok.",
          });
        if (
          err instanceof Error &&
          err.message === "INVENTORY_TRANSACTION_POSTED"
        )
          return sendError(res, 409, {
            code: "INVENTORY_TRANSACTION_POSTED",
            message:
              "Transaksi inventory yang sudah Posted bersifat final dan tidak dapat diubah atau dihapus.",
            legacyError: "Transaksi Posted tidak dapat diubah atau dihapus.",
          });
        if (err instanceof Error && err.message === "STOCK_OPNAME_COMPLETED")
          return sendError(res, 409, {
            code: "STOCK_OPNAME_COMPLETED",
            message:
        ...
        if (
          err instanceof Error &&
          err.message === "STOCK_OPNAME_CONFIRM_REQUIRED"
        )
          return sendError(res, 409, {
            code: "STOCK_OPNAME_CONFIRM_REQUIRED",
            message:
              "Stock Opname hanya dapat diselesaikan melalui proses konfirmasi.",
            legacyError:
              "Gunakan proses konfirmasi untuk menyelesaikan Stock Opname.",
          });
  ...
  inventoryRouter.put(
    ...
    async (req: AuthRequest, res: Response) => {
      ...
      return sendError(res, 409, {
        ...
        message:
          "Bulk inventory writes are disabled; submit records individually",
        legacyError:
          "Bulk inventory writes are disabled; submit records individually",
      });
    },
...
inventoryRouter.post(
  "/inventory/surat-jalan-issues",
  authenticate,
  async (req: AuthRequest, res: Response) => {
    if (
      !hasRoleAccess(req.user?.role, [
        "OWNER",
        "ADMIN",
        "MANAGER",
        "SUPPLY_CHAIN",
    ...
    try {
      const result = await prisma.$transaction(
        async (tx) => {
          const sjItems = Array.isArray(suratJalan.items)
            ...
            : [];
          await tx.logisticsSuratJalan.create({
            data: {
              id: sjId,
              noSurat: asTrimmedString(suratJalan.noSurat) || sjId,
              tanggal: new Date(
                inventoryDateString(asTrimmedString(suratJalan.tanggal)),
              ),
              sjType: asTrimmedString(suratJalan.sjType) || "Material Delivery",
              tujuan: asTrimmedString(suratJalan.tujuan) || "",
              alamat: asTrimmedString(suratJalan.alamat) || "",
              upPerson: asTrimmedString(suratJalan.upPerson) || undefined,
              noPO: asTrimmedString(suratJalan.noPO) || undefined,
              projectId: asTrimmedString(suratJalan.projectId) || undefined,
              assetId: asTrimmedString(suratJalan.assetId) || undefined,
              sopir: asTrimmedString(suratJalan.sopir) || undefined,
              noPolisi: asTrimmedString(suratJalan.noPolisi) || undefined,
              pengirim: asTrimmedString(suratJalan.pengirim) || undefined,
              deliveryStatus:
                asTrimmedString(suratJalan.deliveryStatus) || "Pending",
              workflowStatus:
                asTrimmedString(
                  suratJalan.workflowStatus || suratJalan.status,
                ) || "PREPARED",
              items: {
                create: sjItems
                  .map((item, index) => ({
          ...
          if (Object.keys(stockOut).length > 0) {
            ...
            await tx.inventoryStockOut.create({
              data: {
                id: stockOutId,
                number: asTrimmedString(stockOut.noStockOut) || stockOutId,
                tanggal: new Date(
                  inventoryDateString(asTrimmedString(stockOut.tanggal)),
                ),
                type: asTrimmedString(stockOut.type) || "Project Issue",
                status: "Posted",
                recipientName: asTrimmedString(stockOut.penerima) || undefined,
                notes: asTrimmedString(stockOut.notes) || undefined,
                createdByName: asTrimmedString(stockOut.createdBy) || undefined,
                projectId: asTrimmedString(stockOut.projectId) || undefined,
                legacyPayload: stockOut as Prisma.InputJsonValue,
                items: {
                  create: (Array.isArray(stockOut.items) ? stockOut.items : [])
                    .map(asRecord)
                    .map((item, index) => ({
                      ...
                      itemName: asTrimmedString(item.nama) || "",
                      qty: toFiniteNumber(item.qty, 0),
                      unit: asTrimmedString(item.unit || item.satuan) || "pcs",
                      batchNo: asTrimmedString(item.batchNo) || undefined,
                    }))
                    .filter((item) => item.itemCode),
                },
            ...
            savedStockOut = await tx.inventoryStockOut.findUnique({
              where: { id: stockOutId },
              include: { items: true, project: { select: { payload: true } } },
            });
          }
          const savedSuratJalan = await tx.logisticsSuratJalan.findUnique({
            where: { id: sjId },
            include: { items: true },
          });
      ...
      const message =
        err instanceof Error ? err.message : "Gagal menyimpan Surat Jalan";
      return sendError(res, 400, {
        code: "SURAT_JALAN_ISSUE_FAILED",
        message,
        legacyError: message,
      });
    }
...
inventoryRouter.post(
  "/inventory/stock-opnames/:id/confirm",
  authenticate,
  async (req: AuthRequest, res: Response) => {
    ...
    const id = String(req.params.id || "");
    const confirmedBy =
      asTrimmedString(asRecord(req.body).confirmedBy) ||
      req.user?.id ||
      "SYSTEM";
    try {
      await inventoryTransaction(async (tx) => {
        const opname = await tx.inventoryStockOpname.findUnique({
          where: { id },
          include: { items: true },
        });
        ...
        for (const [index, line] of opname.items.entries()) {
          const item = line.inventoryItemId
            ...
            : await tx.inventoryItem.findFirst({
                where: {
                  OR: [{ code: line.itemCode }, { name: line.itemName }],
                },
              });
          if (!item)
            throw new Error(
              `Material '${line.itemCode || line.itemName}' tidak ditemukan di master stok`,
            );
          if (
          ...
          if (item.onHandQty !== line.systemQty) {
          ...
          await tx.inventoryItem.update({
            ...
            data: {
              ...
              metadata: {
                ...
                stok: stockAfter,
                lastUpdate: confirmedAt.toISOString(),
              },
            },
          });
      ...
      if (error instanceof Error && error.message === "STOCK_OPNAME_STALE") {
        return sendError(res, 409, {
          code: "STOCK_OPNAME_STALE",
          message:
            "Stok sistem berubah sejak Stock Opname dibuat. Muat ulang data dan lakukan pengecekan ulang sebelum konfirmasi.",
          legacyError:
            "Stok berubah sejak Stock Opname dibuat. Muat ulang sebelum konfirmasi.",
        });
      }

      if (
        error instanceof Error &&
        error.message === "STOCK_OPNAME_INVALID_QTY"
      ) {
        return sendError(res, 400, {
          code: "STOCK_OPNAME_INVALID_QTY",
          message:
            "Jumlah stok pada Stock Opname tidak valid. System Qty dan Physical Qty tidak boleh negatif.",
          legacyError: "Jumlah Stock Opname tidak valid.",
        });
      }
