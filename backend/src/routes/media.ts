...
import { removeStoredMediaUrl, storeImageDataUrl } from "../utils/mediaStorage";
...
async function persistStoredMedia<T>(url: string, persist: () => Promise<T>): Promise<T> {
  try {
    return await persist();
  } catch (error) {
    await removeStoredMediaUrl(url).catch(() => undefined);
    throw error;
  }
}
...
mediaRouter.post("/media/qc-drawings", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  const drawingAsset = await persistStoredMedia<any>(stored.url, () => (prisma as any).productionQcDrawingAsset.create({
    data: {
...
mediaRouter.post("/media/lhp-photos", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  const metadata = stored.metadata as Record<string, unknown>;
  const photoAsset = await persistStoredMedia<any>(stored.url, () => (prisma as any).productionExecutionReportPhotoAsset.create({
    data: {
...
mediaRouter.post("/media/invoice-transfer-proofs", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  // Validate the target invoice before writing any bytes to disk. This avoids
  // orphaned files when a client submits a stale/non-existent invoice id.
  if (resource === "invoices") {
    const invoice = await prisma.invoiceRecord.findUnique({ where: { id: invoiceId }, select: { id: true } });
    if (!invoice) {
      return sendError(res, 404, { code: "INVOICE_NOT_FOUND", message: "Invoice tidak ditemukan", legacyError: "Invoice tidak ditemukan" });
    }
  } else {
    const invoice = await prisma.financeCustomerInvoice.findUnique({ where: { id: invoiceId }, select: { id: true } });
    if (!invoice) {
      return sendError(res, 404, { code: "CUSTOMER_INVOICE_NOT_FOUND", message: "Customer invoice tidak ditemukan", legacyError: "Customer invoice tidak ditemukan" });
    }
  }
  ...
  await persistStoredMedia(stored.url, async () => {
    if (resource === "invoices") {
...
mediaRouter.post("/media/pod-assets", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  // Persist record DB supaya file POD tidak orphan: file + row dalam satu operasi,
  // row dihapus kalau ada kegagalan setelah file tersimpan.
  const storedMeta = stored.metadata as { mimeType?: unknown; sizeBytes?: unknown; fileName?: unknown };
  const storedFileName = String(storedMeta.fileName || stored.url.split("/").pop() || `${suratJalanId}-${Date.now()}`);
  const mimeType = typeof storedMeta.mimeType === "string" ? storedMeta.mimeType : "application/octet-stream";
  const sizeBytes = typeof storedMeta.sizeBytes === "number" ? storedMeta.sizeBytes : 0;

  const podAsset = await prisma.logisticsPodAsset.create({
    data: {
      id: `POD-${suratJalanId}-${kind}-${Date.now()}`,
      suratJalanId,
  ...
  if (!podAsset) {
    try { await removeStoredMediaUrl(stored.url); } catch { /* best effort */ }
    return sendError(res, 500, {
      code: "POD_ASSET_PERSIST_FAILED",
      message: "Gagal menyimpan catatan aset POD",
      legacyError: "Gagal menyimpan catatan aset POD",
    });
  }
