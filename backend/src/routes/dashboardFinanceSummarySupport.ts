...
/**
 * Small, presentation-ready summary used by the main executive dashboard.
 * It deliberately reads transaction tables directly so the dashboard does not
 * depend on the browser cache in AppContext.
 */
export async function buildExecutiveDashboardSummaryPayload() {
  ...
  const [invoices, expenses, projects, employeeCount, stockItemCount] = await Promise.all([
    prisma.financeCustomerInvoice.findMany({
    ...
    prisma.financeVendorExpense.findMany({
      where: { tanggal: { gte: start } },
      select: { tanggal: true, totalNominal: true, status: true, updatedAt: true },
    }),
    prisma.projectRecord.findMany({
      select: { status: true, payload: true, updatedAt: true },
    }),
    prisma.employeeRecord.count(),
    prisma.inventoryItem.count(),
  ]);

  const isInvoiceIncluded = (status: string) => !["DRAFT", "REJECTED", "CANCELLED", "VOID"].includes(status.toUpperCase());
  const isExpenseIncluded = (status: string) => ["APPROVED", "PAID", "POSTED"].includes(status.toUpperCase());
  ...
  const byYear = new Map(years.map((row) => [Number(row.year), row]));
  const monthly = Array.from({ length: 12 }, (_, index) => ({ month: index + 1, revenue: 0, expense: 0 }));
  ...
  for (const row of years) row.profit = row.revenue - row.expense;
  const segmentData = [...customers.entries()]
    .sort(([, a], [, b]) => b - a)
    .slice(0, 5)
  ...
  return {
    ...
    metrics: {
      activeProjects: projects.filter((project) => String(project.status || "").toUpperCase() === "IN PROGRESS").length,
      totalProjects: projects.length,
      employees: employeeCount,
      stockItems: stockItemCount,
      invoiceCount: invoices.filter((invoice) => isInvoiceIncluded(invoice.status)).length,
    },
    lastUpdatedAt: maxDate([
...
export async function buildFinanceProjectPlSummaryPayload(params: {
  ...
  const rows = projectRows.map((project) => {
    ...
    const postedStatuses = new Set(["Approved", "Partial Paid", "Paid", "Overdue"]);
    const revenue = customerInvoices
      .filter((inv) => inv.projectId === id && postedStatuses.has(String(inv.status || "")))
    ...
    const vendorExpenseCost = vendorExpenses
      .filter((exp) => exp.projectId === id && ["Approved", "Paid"].includes(String(exp.status || "")))
    ...
    const totalActualCost = materialCost + laborCost + externalCost;
...
export async function buildFinanceYearEndSummaryPayload(params: {
  ...
  const totalRev = invoiceRows.reduce(
    (sum, inv) => sum + (postedStatuses.has(readString(inv, "status") || "") ? readNumber(inv, "subtotal") : 0),
    0
  );
  const totalVend = vendorInvRows.reduce((sum, item) => {
    if (!postedStatuses.has(readString(item, "status") || "")) return sum;
    const total = readNumber(item, "totalAmount");
