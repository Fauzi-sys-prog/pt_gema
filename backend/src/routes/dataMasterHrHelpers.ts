...
export function sanitizeHrLeavePayload(id: string, payload: Record<string, unknown>) {
  ...
  const normalizeDate = (value: unknown) => {
    ...
    if (!text || !/^\d{4}-\d{2}-\d{2}$/.test(text)) return today;
    const date = new Date(`${text}T00:00:00.000Z`);
    return Number.isNaN(date.getTime()) ? today : text;
  };
  const startDate = normalizeDate(payload.startDate);
  const requestedEndDate = normalizeDate(payload.endDate);
  ...
  const suppliedLeaveNo = asTrimmedString(payload.leaveNo);
  const leaveNo = suppliedLeaveNo && !suppliedLeaveNo.includes("NaN")
    ? suppliedLeaveNo
    : `LV/${startDate.slice(0, 4)}/${startDate.slice(5, 7)}/${id.slice(-4).toUpperCase()}`;
  ...
  return {
    ...
    startDate,
    endDate,
    // Backend menjadi sumber kebenaran; jangan percaya totalDays dari klien.
