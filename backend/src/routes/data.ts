...
import { ensureFinancialYearsOpen } from "../middlewares/financialYearLock";
import {
  createEntitySchema,
...
import {
  findFleetAssetContextOrThrow,
  findProductionWorkOrderProjectContext,
  findSuratJalanProjectContext,
  findWorkOrderRelationContext,
  ...
  resolveMachineAssetIdOrThrow,
  syncLegacyWorkOrderRecordFromProduction,
  syncProductionTrackerForWorkOrder,
} from "./dataRelationHelpers";
import {
  buildInvoiceItemCreateManyData,
  buildInvoiceRecordWriteData,
  ensureInvoiceReadyForBilling,
  mapInvoiceRecord,
...
import {
  ...
  mapFinanceVendorExpenseToLegacyPayload,
  mapFinanceVendorInvoiceToLegacyPayload,
  mapFinanceWorkingExpenseSheetToLegacyPayload,
  mapHrKasbonToLegacyPayload,
  mapProcurementPurchaseOrderToLegacyPayload,
  mapProcurementReceivingToLegacyPayload,
  mapProjectLaborEntryToLegacyPayload,
  parsePettySource,
} from "./dataFinanceProcurementMappers";
import {
  mapInventoryItemToLegacyPayload,
  mapInventoryMovementToLegacyPayload,
  mapInventoryOpnameToLegacyPayload,
  mapInventoryStockInToLegacyPayload,
  mapInventoryStockOutToLegacyPayload,
} from "./dataInventoryMappers";
import {
  mapLogisticsProofOfDeliveryToLegacyPayload,
  mapLogisticsSuratJalanToLegacyPayload,
...
import {
  ...
  mapAssetRecord,
  mapHrLeaveRecord,
  mapHrOnlineStatusRecord,
  mapMaintenanceRecord,
  mapPayrollRecord,
  sanitizeAppSettingRecordPayload,
  sanitizeAppSettingsPayload,
  sanitizeAssetPayload,
  sanitizeHrLeavePayload,
  sanitizeHrOnlineStatusPayload,
  sanitizeMaintenancePayload,
  sanitizePayrollPayload,
} from "./dataMasterHrHelpers";
import {
  mapSuratKeluarRecord,
  mapSuratMasukRecord,
  mapTemplateSuratRecord,
  sanitizeSuratKeluarPayload,
  sanitizeSuratMasukPayload,
  sanitizeTemplateSuratPayload,
} from "./dataCorrespondenceHelpers";
import {
  mapProductionExecutionReportToLegacyPayload,
  mapProductionMaterialRequestToLegacyPayload,
  mapProductionQcInspectionToLegacyPayload,
  mapProductionTrackerEntryToLegacyPayload,
  mapProductionWorkOrderToLegacyPayload,
} from "./dataProductionMappers";
import {
  ...
  sanitizeStockItemPayload,
  sanitizeStockOutPayload,
  sanitizeSuratJalanPayload,
  sanitizeVendorExpensePayload,
...
async function relationalMasterFindMany(resource: string) {
  switch (resource) {
    case "vendors": {
      const rows = await prisma.vendorRecord.findMany({
        orderBy: { updatedAt: "desc" },
      });
      return rows.map((row) =>
        toEntityRow(
          row.id,
          mapVendorToDedicatedPayload(row),
          row.createdAt,
          row.updatedAt,
        ),
      );
    ...
    case "customers": {
      ...
      return rows.map((row) =>
        toEntityRow(
          row.id,
          mapCustomerToDedicatedPayload(row),
          row.createdAt,
          row.updatedAt,
        ),
      );
...
async function relationalMasterFindUnique(resource: string, entityId: string) {
  switch (resource) {
    case "vendors": {
      const row = await prisma.vendorRecord.findUnique({
        where: { id: entityId },
      });
      return row
        ? toEntityRow(
    ...
    case "customers": {
      const row = await prisma.customerRecord.findUnique({
        where: { id: entityId },
      });
      return row
        ? toEntityRow(
...
async function relationalMasterCreate(
  resource: string,
  entityId: string,
      ...
      await prisma.vendorRecord.create({
        data: {
          ...
          rating:
            record.rating == null
              ? undefined
              : Math.round(toFiniteNumber(record.rating, 0)),
...
async function relationalMasterUpdate(
  resource: string,
  entityId: string,
  payload: Prisma.InputJsonValue,
) {
...
async function relationalFleetCreate(
  resource: string,
  entityId: string,
  payload: Prisma.InputJsonValue,
) {
    ...
    case "fleet-health": {
      ...
      if (!assetId || !projectId) {
        throw new PayloadValidationError(
          "fleet-health: assetId dan projectId wajib diisi",
        );
      }
      ...
      await prisma.fleetHealthEntry.create({
        data: {
          ...
          projectId,
          tanggal: new Date(
            inventoryDateString(asTrimmedString(record.date ?? record.tanggal)),
          ),
          equipmentName:
            asTrimmedString(record.equipmentName) ||
            asset.equipmentName ||
            asset.assetCode ||
            assetId,
          hoursUsed: toFiniteNumber(record.hoursUsed, 0),
          operatorName:
            asTrimmedString(record.operatorName) || "Field Supervisor",
          fuelConsumption:
            record.fuelConsumption == null
              ? undefined
              : toFiniteNumber(record.fuelConsumption, 0),
...
async function relationalFleetUpdate(
  resource: string,
  entityId: string,
  payload: Prisma.InputJsonValue,
) {
...
async function relationalInventoryFindUnique(
  resource: string,
  entityId: string,
) {
  switch (resource) {
...
async function relationalInventoryCreate(
  resource: string,
  entityId: string,
  payload: Prisma.InputJsonValue,
) {
    ...
    case "stock-ins": {
      await prisma.inventoryStockIn.create({
        data: {
          ...
          tanggal: new Date(
            inventoryDateString(asTrimmedString(record.tanggal)),
          ),
    ...
    case "stock-outs": {
      const workOrderContext = await findWorkOrderRelationContext(
        asTrimmedString(record.workOrderId) ||
          asTrimmedString(record.noWorkOrder),
      );
      await prisma.inventoryStockOut.create({
        data: {
          ...
          productionReportId:
            asTrimmedString(record.productionReportId) || undefined,
          legacyPayload: payload,
          items: {
            create: (Array.isArray(record.items) ? record.items : [])
              .map((raw, index) => {
...
async function relationalInventoryUpdate(
  resource: string,
  entityId: string,
  payload: Prisma.InputJsonValue,
) {
    ...
    case "stock-outs": {
      ...
      await prisma.inventoryStockOut.update({
        ...
        data: {
          ...
          productionReportId:
            asTrimmedString(record.productionReportId) || null,
          legacyPayload: payload,
          items: {
            deleteMany: {},
            create: (Array.isArray(record.items) ? record.items : [])
...
async function relationalInventoryReplaceAll(
  ...
  const existingIdsByResource: Record<string, string[]> = {
    "stock-items": (
      await prisma.inventoryItem.findMany({ select: { id: true } })
    ).map((row) => row.id),
    "stock-ins": (
      await prisma.inventoryStockIn.findMany({ select: { id: true } })
    ).map((row) => row.id),
    "stock-outs": (
      await prisma.inventoryStockOut.findMany({ select: { id: true } })
    ).map((row) => row.id),
    "stock-movements": (
      await prisma.inventoryStockMovement.findMany({ select: { id: true } })
    ).map((row) => row.id),
    "stock-opnames": (
      await prisma.inventoryStockOpname.findMany({ select: { id: true } })
    ).map((row) => row.id),
  };

  const incomingIds = new Set(items.map((item) => item.entityId));
  for (const item of items) {
    const existing = await relationalInventoryFindUnique(
      resource,
      item.entityId,
    );
    if (existing) {
...
async function relationalLogisticsDocsFindMany(resource: string) {
  switch (resource) {
    ...
    case "berita-acara": {
      const rows = await prisma.projectBeritaAcara.findMany({
        orderBy: { updatedAt: "desc" },
      });
      return rows.map((row) =>
        toEntityRow(
          row.id,
          mapProjectBeritaAcaraToLegacyPayload(row),
          row.createdAt,
          row.updatedAt,
        ),
      );
    ...
    case "spk-records": {
      const rows = await prisma.projectSpkRecord.findMany({
        orderBy: { updatedAt: "desc" },
      ...
      return rows.map((row) =>
        toEntityRow(
          row.id,
          mapProjectSpkToLegacyPayload(row),
          row.createdAt,
          row.updatedAt,
        ),
      );
...
async function relationalLogisticsDocsFindUnique(
  resource: string,
  entityId: string,
) {
  switch (resource) {
    ...
    case "berita-acara": {
      const row = await prisma.projectBeritaAcara.findUnique({
        where: { id: entityId },
      });
      return row
        ? toEntityRow(
...
async function relationalLogisticsDocsCreate(
  resource: string,
  entityId: string,
  payload: Prisma.InputJsonValue,
) {
  ...
  switch (resource) {
    case "surat-jalan": {
      await prisma.logisticsSuratJalan.create({
        data: {
          ...
          podTime: asTrimmedString(record.podTime)
            ? new Date(String(record.podTime))
            : undefined,
          podPhoto: asTrimmedString(record.podPhoto) || undefined,
          podSignature: asTrimmedString(record.podSignature) || undefined,
          expectedReturnDate: asTrimmedString(record.expectedReturnDate)
            ? new Date(String(record.expectedReturnDate))
            : undefined,
          actualReturnDate: asTrimmedString(record.actualReturnDate)
            ? new Date(String(record.actualReturnDate))
            : undefined,
          returnStatus: asTrimmedString(record.returnStatus) || undefined,
          workflowStatus:
            asTrimmedString(record.workflowStatus || record.status) ||
            "PREPARED",
          items: {
            create: (Array.isArray(record.items) ? record.items : [])
              .map((raw, index) => {
                ...
                return {
                  ...
                  namaItem:
                    asTrimmedString(item.namaItem || item.namaBarang) || "",
    ...
    case "proof-of-delivery": {
      const suratJalanContext = await findSuratJalanProjectContext(
        asTrimmedString(record.suratJalanId),
      );
      const workOrderContext = await findWorkOrderRelationContext(
        asTrimmedString(record.workOrderId),
      );
      ...
      await prisma.logisticsProofOfDelivery.create({
        data: {
          ...
          receiverName:
            asTrimmedString(record.receiverName || record.receiver) || "",
          deliveredAt: asTrimmedString(record.deliveredAt || record.podTime)
            ? new Date(String(record.deliveredAt || record.podTime))
            : new Date(),
    ...
    case "berita-acara": {
      const suratJalanContext = await findSuratJalanProjectContext(
        asTrimmedString(record.refSuratJalan),
      );
      const resolvedProjectId =
      ...
      await prisma.projectBeritaAcara.create({
        data: {
          id: entityId,
          noBA:
            asTrimmedString(record.noBA || record.noBeritaAcara) || entityId,
          tanggal: new Date(
            inventoryDateString(asTrimmedString(record.tanggal)),
          ),
          ...
          pihakPertamaJabatan:
            asTrimmedString(record.pihakPertamaJabatan) || undefined,
          pihakPertamaNama:
            asTrimmedString(record.pihakPertamaNama) || undefined,
          pihakKedua: asTrimmedString(record.pihakKedua) || "",
          pihakKeduaJabatan:
            asTrimmedString(record.pihakKeduaJabatan) || undefined,
          ...
          contentHTML: sanitizeRichHtml(
            asTrimmedString(record.contentHTML || record.content) || "",
          ),
          ...
          status: asTrimmedString(record.status) || "Draft",
          noPO: asTrimmedString(record.noPO) || undefined,
          tanggalPO: asTrimmedString(record.tanggalPO)
            ? new Date(String(record.tanggalPO))
            : undefined,
          tanggalPelaksanaanMulai: asTrimmedString(
            record.tanggalPelaksanaanMulai,
          )
            ? new Date(String(record.tanggalPelaksanaanMulai))
            : undefined,
          tanggalPelaksanaanSelesai: asTrimmedString(
            record.tanggalPelaksanaanSelesai,
          )
            ? new Date(String(record.tanggalPelaksanaanSelesai))
            : undefined,
          approvedBy: asTrimmedString(record.approvedBy) || undefined,
          approvedAt: asTrimmedString(record.approvedAt)
            ? new Date(String(record.approvedAt))
            : undefined,
          rejectionReason: asTrimmedString(record.rejectionReason) || undefined,
          projectId: resolvedProjectId || undefined,
    ...
    case "spk-records": {
      ...
      await prisma.projectSpkRecord.create({
        data: {
          ...
          workOrderId: workOrderContext?.relationId || undefined,
          spkNumber:
            asTrimmedString(record.noSPK || record.spkNumber) || entityId,
          title: asTrimmedString(record.title || record.pekerjaan) || "SPK",
          pekerjaan: asTrimmedString(record.pekerjaan) || undefined,
          date: new Date(
            inventoryDateString(asTrimmedString(record.tanggal || record.date)),
          ),
          urgent: Boolean(record.urgent),
          status: asTrimmedString(record.status) || "Active",
          technicians: {
            create: (Array.isArray(record.teknisi)
              ? record.teknisi
              : String(record.teknisi || "")
                  .split(",")
                  .map((v) => v.trim())
                  .filter(Boolean)
            ).map((name, index) => ({
          ...
          attachments: {
            create: (Array.isArray(record.invoiceImages)
              ? record.invoiceImages
              : []
            )
              .map((url, index) => ({
...
async function relationalLogisticsDocsUpdate(
  resource: string,
  entityId: string,
  payload: Prisma.InputJsonValue,
) {
    ...
    case "berita-acara": {
      ...
      await prisma.projectBeritaAcara.update({
        ...
        data: {
          ...
          pihakPertama: asTrimmedString(record.pihakPertama) || "",
          pihakPertamaJabatan:
            asTrimmedString(record.pihakPertamaJabatan) || null,
          ...
          approvedAt: asTrimmedString(record.approvedAt)
            ? new Date(String(record.approvedAt))
            : null,
          rejectionReason: asTrimmedString(record.rejectionReason) || null,
          projectId: resolvedProjectId || null,
...
async function relationalLogisticsDocsDelete(
  resource: string,
  entityId: string,
) {
  switch (resource) {
...
async function relationalLogisticsDocsReplaceAll(
  resource: string,
  items: Array<{ entityId: string; payload: Prisma.InputJsonValue }>,
) {
  const existingIdsByResource: Record<string, string[]> = {
    "surat-jalan": (
      await prisma.logisticsSuratJalan.findMany({ select: { id: true } })
    ).map((row) => row.id),
    "proof-of-delivery": (
      await prisma.logisticsProofOfDelivery.findMany({ select: { id: true } })
    ).map((row) => row.id),
    "berita-acara": (
      await prisma.projectBeritaAcara.findMany({ select: { id: true } })
    ).map((row) => row.id),
    "spk-records": (
      await prisma.projectSpkRecord.findMany({ select: { id: true } })
    ).map((row) => row.id),
  };
  const incomingIds = new Set(items.map((item) => item.entityId));
  for (const item of items) {
    const existing = await relationalLogisticsDocsFindUnique(
      resource,
      item.entityId,
    );
    if (existing)
      await relationalLogisticsDocsUpdate(
        resource,
        item.entityId,
        item.payload,
      );
    else
      await relationalLogisticsDocsCreate(
        resource,
        item.entityId,
        item.payload,
      );
  ...
  for (const existingId of existingIdsByResource[resource] || []) {
    if (!incomingIds.has(existingId))
      await relationalLogisticsDocsDelete(resource, existingId);
  }
}
...
async function relationalProductionFindMany(resource: string) {
  switch (resource) {
    case "work-orders": {
      const rows = await prisma.productionWorkOrder.findMany({
        orderBy: { updatedAt: "desc" },
    ...
    case "production-trackers": {
      const rows = await prisma.productionTrackerEntry.findMany({
        orderBy: { updatedAt: "desc" },
      });
      return rows.map((row) =>
        toEntityRow(
    ...
    case "qc-inspections": {
      ...
      return rows.map((row: any) =>
        toEntityRow(
          row.id,
          mapProductionQcInspectionToLegacyPayload(row),
          row.createdAt,
          row.updatedAt,
        ),
      );
    ...
    case "material-requests": {
      const rows = await prisma.productionMaterialRequest.findMany({
        orderBy: { updatedAt: "desc" },
        include: { items: true },
      });
      return rows.map((row) =>
...
async function relationalProductionFindUnique(
  resource: string,
  entityId: string,
) {
  switch (resource) {
    case "work-orders": {
      const row = await prisma.productionWorkOrder.findUnique({
        where: { id: entityId },
        include: { bomItems: true },
      });
      return row
    ...
    case "production-trackers": {
      const row = await prisma.productionTrackerEntry.findUnique({
        where: { id: entityId },
      });
      return row
        ? toEntityRow(
    ...
    case "material-requests": {
      const row = await prisma.productionMaterialRequest.findUnique({
        where: { id: entityId },
        include: { items: true },
      });
      return row
...
async function syncQcReleaseToWarehouse(
  entityId: string,
  record: Record<string, unknown>,
  tx?: Prisma.TransactionClient,
) {
  const db = tx ?? prisma;
  const workOrderId = asTrimmedString(
    record.workOrderId || record.woId || record.noWorkOrder,
  );
  ...
  const stockInId = `SI-QC-${entityId}`;
  // Only a passed QC result may create warehouse stock. Partial results stay
  // as draft inspection output and rejected results never enter available stock.
  const shouldEnterWarehouse =
    record.sendToWarehouse === true && status === "Passed" && qtyPassed > 0;
  const existing = await db.inventoryStockIn.findUnique({
    where: { id: stockInId },
  });
  if (!shouldEnterWarehouse) {
  ...
  const warehouseLocation =
    asTrimmedString(record.warehouseLocation) || "Gudang Barang Jadi";
  const stockCategory = asTrimmedString(record.stockCategory) || "Barang Jadi";
  const itemCode = `FG-${workOrder.number}`;
  const itemName = asTrimmedString(record.itemNama) || workOrder.itemToProduce;
  const legacyPayload = {
    id: stockInId,
    noStockIn: `SI-FG-${entityId}`,
    tanggal: inventoryDateString(asTrimmedString(record.tanggal)),
    type: "Production Output",
    status: "Draft",
    createdBy: asTrimmedString(record.inspectorName) || "QC System",
    projectId: workOrder.projectId,
    workOrderId: workOrder.id,
    qcInspectionId: entityId,
    warehouseLocation,
    stockCategory,
    notes: `Draft barang jadi dari QC ${asTrimmedString(record.batchNo) || entityId} — WO ${workOrder.number}`,
    items: [
      {
        kode: itemCode,
        nama: itemName,
        kategori: stockCategory,
        qty: qtyPassed,
        satuan: "Unit",
        unit: "Unit",
  ...
  const data = {
    number: `SI-FG-${entityId}`,
    tanggal: new Date(inventoryDateString(asTrimmedString(record.tanggal))),
    type: "Production Output",
    status: "Draft",
    notes: `Draft barang jadi dari QC ${asTrimmedString(record.batchNo) || entityId} — WO ${workOrder.number}`,
    createdByName: asTrimmedString(record.inspectorName) || "QC System",
    projectId: workOrder.projectId,
    legacyPayload,
  };
  const itemData = {
  ...
  if (existing) {
    await db.inventoryStockIn.update({
      where: { id: stockInId },
      data: { ...data, items: { deleteMany: {}, create: [itemData] } },
    });
  } else {
    await db.inventoryStockIn.create({
      data: { id: stockInId, ...data, items: { create: [itemData] } },
    });
  }
}

async function recalculateWorkOrderAndProjectProgress(
  workOrderId?: string | null,
  tx?: Prisma.TransactionClient,
) {
  const db = tx ?? prisma;
  ...
  const current = await db.productionWorkOrder.findUnique({
    where: { id: workOrderId },
    include: {
      bomItems: true,
      productionReports: { select: { outputQty: true } },
    },
  });
  if (!current) return;
  ...
  const updatedWorkOrder = await db.productionWorkOrder.update({
    where: { id: workOrderId },
  ...
  if (!project) return;
  const projectWorkOrders = await db.productionWorkOrder.findMany({
  ...
  const projectPayload = asRecord(project.payload);
  const overrideRaw = projectPayload.progressOverride;
  const hasOverride =
    overrideRaw !== null &&
    overrideRaw !== undefined &&
    Number.isFinite(Number(overrideRaw));
  const progress = hasOverride
    ? Math.max(0, Math.min(100, Number(overrideRaw)))
    : progressAuto;
...
async function relationalProductionCreate(
  resource: string,
  entityId: string,
  payload: Prisma.InputJsonValue,
) {
  ...
  switch (resource) {
    case "work-orders": {
      const resolvedMachineId = await resolveMachineAssetIdOrThrow(
        resource,
        asTrimmedString(record.machineId),
      );
      const workOrder = await prisma.productionWorkOrder.create({
        data: {
          ...
          deadline: asTrimmedString(record.deadline)
            ? new Date(String(record.deadline))
            : undefined,
          ...
          requiresBom: record.requiresBom !== false,
          bomItems: {
            create: (Array.isArray(record.bom) ? record.bom : [])
              .map((raw, index) => {
                const item = asRecord(raw);
                return {
                  id: `${entityId}-BOM-${String(index + 1).padStart(3, "0")}`,
                  itemCode:
                    asTrimmedString(item.kode || item.itemKode) || undefined,
                  itemName:
                    asTrimmedString(item.nama || item.materialName) || "",
                  ...
                  stockAvailable:
                    item.stockAvailable == null
                      ? undefined
                      : toFiniteNumber(item.stockAvailable, 0),
                };
              })
    ...
    case "production-reports": {
      const workOrderId =
        asTrimmedString(record.workOrderId || record.woId) || undefined;
      const linkedWorkOrder = workOrderId
      ...
      await prisma.$transaction(async (tx) => {
        await recalculateWorkOrderAndProjectProgress(workOrderId, tx);
      });
      return relationalProductionFindUnique(resource, entityId);
    }
    case "production-trackers": {
      ...
      await prisma.productionTrackerEntry.create({
        data: {
          ...
          qty: toFiniteNumber(record.qty, 0),
          startDate: asTrimmedString(record.startDate)
            ? new Date(String(record.startDate))
            : undefined,
          finishDate: asTrimmedString(record.finishDate)
            ? new Date(String(record.finishDate))
            : undefined,
    ...
    case "qc-inspections": {
      const dimensions = (
        Array.isArray(record.dimensions) ? record.dimensions : []
      )
        .map((raw, index) => {
          const item = asRecord(raw);
          return {
      ...
      await prisma.$transaction(async (tx) => {
        await syncQcReleaseToWarehouse(entityId, record, tx);
      });
      return relationalProductionFindUnique(resource, entityId);
    }
    case "material-requests": {
      await prisma.productionMaterialRequest.create({
        data: {
          id: entityId,
          number:
            asTrimmedString(record.noRequest || record.requestNo) || entityId,
          projectId: asTrimmedString(record.projectId) || "",
          projectName: asTrimmedString(record.projectName) || "",
          requestedBy: asTrimmedString(record.requestedBy) || "",
          requestedAt: asTrimmedString(record.requestedAt)
            ? new Date(String(record.requestedAt))
            : new Date(),
          status: asTrimmedString(record.status) || "Pending",
          items: {
            create: (Array.isArray(record.items) ? record.items : [])
...
async function relationalProductionUpdate(
  resource: string,
  entityId: string,
  payload: Prisma.InputJsonValue,
) {
  ...
  switch (resource) {
      ...
      const workOrder = await prisma.productionWorkOrder.update({
        ...
        data: {
          ...
          startDate: asTrimmedString(record.startDate)
            ? new Date(String(record.startDate))
            : null,
          endDate: asTrimmedString(record.endDate)
            ? new Date(String(record.endDate))
            : null,
    ...
    case "production-reports": {
      const previous = await prisma.productionExecutionReport.findUnique({
        where: { id: entityId },
        select: { workOrderId: true },
      });
      const workOrderId =
        asTrimmedString(record.workOrderId || record.woId) || null;
      const linkedWorkOrder = workOrderId
        ? await prisma.productionWorkOrder.findUnique({
            where: { id: workOrderId },
            select: { projectId: true },
      ...
      if (previous?.workOrderId && previous.workOrderId !== workOrderId) {
        await recalculateWorkOrderAndProjectProgress(previous.workOrderId);
      }
      await prisma.$transaction(async (tx) => {
        await recalculateWorkOrderAndProjectProgress(workOrderId, tx);
      });
  ...
  switch (resource) {
    case "work-orders":
      await prisma.$transaction(async (tx) => {
        ...
        if (!workOrder) {
          // Biarkan Prisma menghasilkan P2025 agar route mengembalikan 404.
          await tx.productionWorkOrder.delete({ where: { id: entityId } });
          return;
        }

        const [productionReportCount, qcInspectionCount] = await Promise.all([
          tx.productionExecutionReport.count({
            where: { workOrderId: entityId },
          }),
          tx.productionQcInspection.count({
            where: { workOrderId: entityId },
          }),
        ]);

        const status = workOrder.status.trim().toUpperCase();
        const workflowStatus = workOrder.workflowStatus?.trim() || "";

        const deletablePlanningStatuses = new Set([
          "DRAFT",
          "READY",
          "PLANNED",
        ]);
        ...
        if (hasTransactionalHistory) {
          throw new PayloadValidationError(
            `Work Order ${workOrder.number} tidak dapat dihapus karena sudah masuk alur transaksi produksi. Batalkan atau arsipkan Work Order tanpa menghapus histori produksi.`,
          );
        }

        await tx.productionTrackerEntry.deleteMany({
          where: {
            OR: [
        ...
        await tx.productionWorkOrder.delete({ where: { id: entityId } });
        await tx.workOrderRecord.deleteMany({ where: { id: entityId } });
      });
      return;
    case "production-reports": {
      const report = await prisma.productionExecutionReport.findUnique({
        where: { id: entityId },
        select: { workOrderId: true },
      });
      await prisma.productionExecutionReport.delete({
        where: { id: entityId },
      });
      await prisma.$transaction(async (tx) => {
        await recalculateWorkOrderAndProjectProgress(report?.workOrderId, tx);
      });
      return;
    }
    case "production-trackers":
    ...
    case "qc-inspections":
      ...
      return;
    case "material-requests":
      await prisma.productionMaterialRequest.delete({
        where: { id: entityId },
      });
      return;
    default:
...
async function relationalProductionReplaceAll(
  resource: string,
  items: Array<{ entityId: string; payload: Prisma.InputJsonValue }>,
) {
  const existingIdsByResource: Record<string, string[]> = {
    "work-orders": (
      await prisma.productionWorkOrder.findMany({ select: { id: true } })
    ).map((row) => row.id),
    "production-reports": (
      await prisma.productionExecutionReport.findMany({ select: { id: true } })
    ).map((row) => row.id),
    "production-trackers": (
      await prisma.productionTrackerEntry.findMany({ select: { id: true } })
    ).map((row) => row.id),
    "qc-inspections": (
      await prisma.productionQcInspection.findMany({ select: { id: true } })
    ).map((row) => row.id),
    "material-requests": (
      await prisma.productionMaterialRequest.findMany({ select: { id: true } })
    ).map((row) => row.id),
  };
  const incomingIds = new Set(items.map((item) => item.entityId));
  for (const item of items) {
    const existing = await relationalProductionFindUnique(
      resource,
      item.entityId,
    );
    ...
    if (resource === "work-orders") {
      normalizedPayload = (await sanitizeWorkOrderPayload(
        item.payload,
        existing ? existing.payload : undefined,
      )) as Prisma.InputJsonValue;
    }

    if (existing) {
      await relationalProductionUpdate(
        resource,
        item.entityId,
        normalizedPayload,
      );
    } else {
      await relationalProductionCreate(
        resource,
        item.entityId,
        normalizedPayload,
      );
  ...
  for (const existingId of existingIdsByResource[resource] || []) {
    ...
    // Work Orders tidak boleh terhapus hanya karena tidak ikut terkirim
    // dalam payload replace-all. Delete WO harus dilakukan secara eksplisit.
    if (resource === "work-orders") {
      continue;
    }

    await relationalProductionDelete(resource, existingId);
  }
}
  ...
  switch (resource) {
    case "working-expense-sheets": {
      ...
      return rows.map((row) =>
        toEntityRow(
          row.id,
          mapFinanceWorkingExpenseSheetToLegacyPayload(row),
          row.createdAt,
          row.updatedAt,
        ),
      );
    ...
    case "finance-petty-cash-transactions": {
      const rows = await prisma.financePettyCashTransaction.findMany({
        orderBy: { updatedAt: "desc" },
      });
      return rows.map((row) =>
        toEntityRow(
          row.id,
          mapFinancePettyCashTransactionToLegacyPayload(row),
          row.createdAt,
          row.updatedAt,
        ),
      );
    ...
    case "finance-bank-reconciliations": {
      ...
      return rows.map((row) =>
        toEntityRow(
          row.id,
          mapFinanceBankReconciliationToLegacyPayload(row),
          row.createdAt,
          row.updatedAt,
        ),
      );
    ...
    case "kasbons": {
      const rows = await prisma.hrKasbon.findMany({
        orderBy: { updatedAt: "desc" },
      });
      return rows.map((row) =>
        toEntityRow(
          row.id,
          mapHrKasbonToLegacyPayload(row),
          row.createdAt,
          row.updatedAt,
        ),
      );
    ...
    case "project-labor-entries": {
      const rows = await prisma.projectLaborEntry.findMany({
...
async function relationalFinanceMiscFindUnique(
  resource: string,
  entityId: string,
) {
  switch (resource) {
    ...
    case "finance-petty-cash-transactions": {
      const row = await prisma.financePettyCashTransaction.findUnique({
        where: { id: entityId },
      });
      return row
        ? toEntityRow(
    ...
    case "finance-bank-reconciliations": {
      const row = await prisma.financeBankReconciliation.findUnique({
        where: { id: entityId },
      });
      return row
        ? toEntityRow(
    ...
    case "project-labor-entries": {
      const row = await prisma.projectLaborEntry.findUnique({
        where: { id: entityId },
      });
      return row
        ? toEntityRow(
...
async function relationalFinanceMiscCreate(
  resource: string,
  entityId: string,
  payload: Prisma.InputJsonValue,
) {
    ...
    case "working-expense-sheets": {
      await prisma.financeWorkingExpenseSheet.create({
        data: {
          ...
          projectName:
            asTrimmedString(record.project) ||
            asTrimmedString(record.projectName) ||
            undefined,
              ...
              .map((raw, index) => {
                ...
                return {
                  id:
                    ...
                    `${entityId}-ITEM-${String(index + 1).padStart(3, "0")}`,
    ...
    case "finance-bank-reconciliations": {
      await prisma.financeBankReconciliation.create({
        data: {
          ...
          projectId: asTrimmedString(record.projectId) || undefined,
          customerInvoiceId:
            asTrimmedString(record.customerInvoiceId || record.invoiceId) ||
            undefined,
...
async function relationalFinanceMiscUpdate(
  resource: string,
  entityId: string,
  payload: Prisma.InputJsonValue,
) {
...
async function relationalFinanceMiscDelete(resource: string, entityId: string) {
  switch (resource) {
    case "working-expense-sheets":
      await prisma.financeWorkingExpenseSheet.delete({
        where: { id: entityId },
      });
      return;
    case "finance-petty-cash-transactions":
      await prisma.financePettyCashTransaction.delete({
        where: { id: entityId },
      });
      return;
    case "finance-bank-reconciliations":
      await prisma.financeBankReconciliation.delete({
        where: { id: entityId },
      });
      return;
    case "kasbons":
...
async function relationalFinanceMiscReplaceAll(
  resource: string,
  items: Array<{ entityId: string; payload: Prisma.InputJsonValue }>,
) {
  ...
  for (const item of items) {
    if (existingIds.has(item.entityId))
      await relationalFinanceMiscUpdate(resource, item.entityId, item.payload);
    else
      await relationalFinanceMiscCreate(resource, item.entityId, item.payload);
  }
  for (const existingItem of existing) {
    if (!incomingIds.has(existingItem.entityId))
      await relationalFinanceMiscDelete(resource, existingItem.entityId);
  }
}
  ...
  switch (resource) {
    ...
    case "customer-invoices": {
      ...
      return rows.map((row) =>
        toEntityRow(
          row.id,
          mapFinanceCustomerInvoiceToLegacyPayload(row),
          row.createdAt,
          row.updatedAt,
        ),
      );
    ...
    case "vendor-expenses": {
      const rows = await prisma.financeVendorExpense.findMany({
        orderBy: { updatedAt: "desc" },
      });
      return rows.map((row) =>
        toEntityRow(
          row.id,
          mapFinanceVendorExpenseToLegacyPayload(row),
          row.createdAt,
          row.updatedAt,
        ),
      );
    ...
    case "vendor-invoices": {
      const rows = await prisma.financeVendorInvoice.findMany({
        orderBy: { updatedAt: "desc" },
        include: {
      ...
      return rows.map((row) =>
        toEntityRow(
          row.id,
          mapFinanceVendorInvoiceToLegacyPayload(row),
          row.createdAt,
          row.updatedAt,
        ),
      );
...
async function relationalProcurementFinanceFindUnique(
  resource: string,
  entityId: string,
) {
  switch (resource) {
    ...
    case "vendor-expenses": {
      const row = await prisma.financeVendorExpense.findUnique({
        where: { id: entityId },
      });
      return row
        ? toEntityRow(
...
async function relationalProcurementFinanceCreate(
  resource: string,
  entityId: string,
  payload: Prisma.InputJsonValue,
) {
  ...
  switch (resource) {
    case "purchase-orders": {
      await prisma.procurementPurchaseOrder.create({
        data: {
          ...
          supplierName:
            asTrimmedString(record.supplier) ||
            vendorNameFromPayload(undefined) ||
            "",
          ...
          deliveryDate: asTrimmedString(record.deliveryDate)
            ? new Date(String(record.deliveryDate))
            : undefined,
              ...
              .map((raw, index) => {
                ...
                return {
                  ...
                  total: toFiniteNumber(
                    item.total,
                    toFiniteNumber(item.qty, 0) *
                      toFiniteNumber(item.unitPrice ?? item.harga, 0),
                  ),
            ...
            create: (Array.isArray(record.items) ? record.items : [])
              .map((raw, index) => {
                ...
                return {
                  ...
                  qtyReceived: toFiniteNumber(
                    item.qtyReceived ?? item.qtyGood ?? item.qty,
                    0,
                  ),
                  qtyGood: toFiniteNumber(
                    item.qtyGood ?? item.qtyReceived ?? item.qty,
                    0,
                  ),
                  qtyDamaged: toFiniteNumber(item.qtyDamaged, 0),
                  qtyPreviouslyReceived: toFiniteNumber(
                    item.qtyPreviouslyReceived,
                    0,
                  ),
                  unit: asTrimmedString(item.unit) || "pcs",
    ...
    case "customer-invoices": {
      await prisma.financeCustomerInvoice.create({
        data: {
          ...
          dueDate: asTrimmedString(record.dueDate)
          ...
          remark: asTrimmedString(record.remark) || undefined,
          createdBy: asTrimmedString(record.createdBy) || undefined,
          sentAt: asTrimmedString(record.sentAt)
            ? new Date(String(record.sentAt))
            : undefined,
          items: {
            create: (Array.isArray(record.items) ? record.items : [])
              .map((raw, index) => {
                ...
                return {
                  ...
                  amount: toFiniteNumber(
                    item.jumlah,
                    toFiniteNumber(item.qty, 0) *
                      toFiniteNumber(item.hargaSatuan, 0),
                  ),
                };
              })
              .filter((item) => item.description),
          ...
          payments: {
            create: (Array.isArray(record.paymentHistory)
              ...
              return {
                id:
                  asTrimmedString(item.id) ||
                  `${entityId}-PAY-${String(index + 1).padStart(3, "0")}`,
                tanggal: new Date(
                  inventoryDateString(asTrimmedString(item.tanggal)),
                ),
                nominal: toFiniteNumber(item.nominal, 0),
                method: asTrimmedString(item.metodeBayar) || "Transfer",
                proofNo: asTrimmedString(item.noBukti) || undefined,
                bankName:
                  asTrimmedString(item.rekeningTujuan || item.bankName) ||
                  undefined,
                remark: asTrimmedString(item.remark) || undefined,
                createdBy: asTrimmedString(item.createdBy) || undefined,
                paidAt: asTrimmedString(item.createdAt)
    ...
    case "vendor-expenses": {
      await prisma.financeVendorExpense.create({
        data: {
          ...
          bank: asTrimmedString(record.bank) || undefined,
          status: asTrimmedString(record.status) || "Draft",
          remark: asTrimmedString(record.remark) || undefined,
          approvedBy: asTrimmedString(record.approvedBy) || undefined,
          approvedAt: asTrimmedString(record.approvedAt)
          ...
          rejectedAt: asTrimmedString(record.rejectedAt)
            ? new Date(String(record.rejectedAt))
            : undefined,
          rejectReason: asTrimmedString(record.rejectReason) || undefined,
          paidAt: asTrimmedString(record.paidAt)
            ? new Date(String(record.paidAt))
            : undefined,
          createdBy: asTrimmedString(record.createdBy) || undefined,
        },
      });
    ...
    case "vendor-invoices": {
      const invoiceNumber =
        asTrimmedString(record.noInvoiceVendor || record.noInvoice) || entityId;
      const supplierName =
        asTrimmedString(record.supplier || record.vendorName) || "";
      const purchaseOrderId = asTrimmedString(record.purchaseOrderId);

      const duplicate = await prisma.financeVendorInvoice.findFirst({
        where: { number: invoiceNumber, supplierName },
        select: { id: true },
      });
      if (duplicate) {
        throw new Error(
          `vendor-invoices: invoice '${invoiceNumber}' dari '${supplierName}' sudah tercatat`,
        );
      }
      ...
      if (purchaseOrderId) {
        ...
        if (!purchaseOrder) {
          throw new Error(
            `vendor-invoices: purchaseOrderId '${purchaseOrderId}' tidak ditemukan`,
          );
        }
        if (
          !["APPROVED", "RECEIVED"].includes(
            purchaseOrder.status.trim().toUpperCase(),
          )
        ) {
          throw new Error(
            `vendor-invoices: PO harus berstatus Approved atau Received`,
          );
        }
      }
      ...
      await prisma.financeVendorInvoice.create({
        data: {
          ...
          tanggal: asTrimmedString(record.tanggal)
            ? new Date(String(record.tanggal))
            : undefined,
          dueDate: asTrimmedString(record.jatuhTempo)
            ? new Date(String(record.jatuhTempo))
            : undefined,
          keterangan: asTrimmedString(record.keterangan) || undefined,
          sentAt: asTrimmedString(record.sentAt)
            ? new Date(String(record.sentAt))
          ...
          rejectedAt: asTrimmedString(record.rejectedAt)
            ? new Date(String(record.rejectedAt))
            : undefined,
          rejectedReason: asTrimmedString(record.rejectedReason) || undefined,
          payments: {
            create: (Array.isArray(record.paymentHistory)
              ? record.paymentHistory
              : []
              ...
              return {
                id: asTrimmedString(payment.id) || randomUUID(),
                tanggal: new Date(
                  inventoryDateString(asTrimmedString(payment.tanggal)),
                ),
                nominal: toFiniteNumber(payment.nominal, 0),
                metodeBayar: asTrimmedString(payment.metodeBayar) || undefined,
          ...
          approvalHistory: {
            create: (Array.isArray(record.approvalHistory)
              ? record.approvalHistory
              : []
            ).map((raw) => {
              const event = asRecord(raw);
              return {
                id: asTrimmedString(event.id) || randomUUID(),
                action: asTrimmedString(event.action) || "Revision",
                actorName: asTrimmedString(event.by) || "System",
                actionAt: asTrimmedString(event.date)
                  ? new Date(String(event.date))
                  : new Date(),
                reason: asTrimmedString(event.reason) || undefined,
              };
            }),
          },
        },
...
async function relationalProcurementFinanceUpdate(
  resource: string,
  entityId: string,
  payload: Prisma.InputJsonValue,
) {
  ...
  switch (resource) {
    ...
    case "vendor-expenses": {
      await prisma.financeVendorExpense.update({
        ...
        data: {
          ...
          bank: asTrimmedString(record.bank) || null,
          status: asTrimmedString(record.status) || "Draft",
          remark: asTrimmedString(record.remark) || null,
          approvedBy: asTrimmedString(record.approvedBy) || null,
          approvedAt: asTrimmedString(record.approvedAt)
    ...
    case "vendor-invoices": {
      const existingInvoice = await prisma.financeVendorInvoice.findUnique({
        where: { id: entityId },
        select: { purchaseOrderId: true, status: true, paidAmount: true },
      });
      const purchaseOrderId =
        asTrimmedString(record.purchaseOrderId) ||
        existingInvoice?.purchaseOrderId ||
        null;
      const nextStatus =
        asTrimmedString(record.status) || existingInvoice?.status || "Unpaid";
      const nextPaidAmount =
        record.paidAmount === undefined
          ? Number(existingInvoice?.paidAmount || 0)
          : toFiniteNumber(record.paidAmount, 0);
      const requiresReceivedPO =
        ["Approved", "Partial", "Paid"].includes(nextStatus) ||
        nextPaidAmount > 0;

      if (purchaseOrderId && requiresReceivedPO) {
        const linkedPO = await prisma.procurementPurchaseOrder.findUnique({
          where: { id: purchaseOrderId },
        ...
        if (linkedPO.status !== "Received") {
          throw new Error(
            `vendor-invoices: tidak dapat approve atau membayar karena PO '${linkedPO.number}' belum Received`,
          );
        }
      }
      ...
      await prisma.financeVendorInvoice.update({
        ...
        data: {
          ...
          purchaseOrderId,
          number:
            asTrimmedString(record.noInvoiceVendor || record.noInvoice) ||
            entityId,
          noPO: asTrimmedString(record.noPO) || null,
          supplierName:
            asTrimmedString(record.supplier || record.vendorName) || "",
          ...
          rejectedAt: asTrimmedString(record.rejectedAt)
            ? new Date(String(record.rejectedAt))
            : null,
          rejectedReason: asTrimmedString(record.rejectedReason) || null,
          payments: {
            deleteMany: {},
            create: (Array.isArray(record.paymentHistory)
              ? record.paymentHistory
              ...
              return {
                ...
                nominal: toFiniteNumber(payment.nominal, 0),
                metodeBayar: asTrimmedString(payment.metodeBayar) || null,
                noBukti: asTrimmedString(payment.noBukti) || null,
                bank: asTrimmedString(payment.bank) || null,
                noRekening: asTrimmedString(payment.noRekening) || null,
                keterangan: asTrimmedString(payment.keterangan) || null,
              };
            }),
          },
          approvalHistory: {
            ...
            create: (Array.isArray(record.approvalHistory)
              ...
              return {
                ...
                actionAt: asTrimmedString(event.date)
                  ? new Date(String(event.date))
                  : new Date(),
                reason: asTrimmedString(event.reason) || null,
              };
            }),
          },
        },
...
async function relationalProcurementFinanceDelete(
  resource: string,
  entityId: string,
) {
  switch (resource) {
...
async function relationalProcurementFinanceReplaceAll(
  resource: string,
  items: Array<{ entityId: string; payload: Prisma.InputJsonValue }>,
) {
  const existingIdsByResource: Record<string, string[]> = {
    "purchase-orders": (
      await prisma.procurementPurchaseOrder.findMany({ select: { id: true } })
    ).map((row) => row.id),
    receivings: (
      await prisma.procurementReceiving.findMany({ select: { id: true } })
    ).map((row) => row.id),
    "customer-invoices": (
      await prisma.financeCustomerInvoice.findMany({ select: { id: true } })
    ).map((row) => row.id),
    "vendor-expenses": (
      await prisma.financeVendorExpense.findMany({ select: { id: true } })
    ).map((row) => row.id),
    "vendor-invoices": (
      await prisma.financeVendorInvoice.findMany({ select: { id: true } })
    ).map((row) => row.id),
  };
  const incomingIds = new Set(items.map((item) => item.entityId));
  for (const item of items) {
    const existing = await relationalProcurementFinanceFindUnique(
      resource,
      item.entityId,
    );
    if (existing)
      await relationalProcurementFinanceUpdate(
        resource,
        item.entityId,
        item.payload,
      );
    else
      await relationalProcurementFinanceCreate(
        resource,
        item.entityId,
        item.payload,
      );
  ...
  for (const existingId of existingIdsByResource[resource] || []) {
    if (!incomingIds.has(existingId))
      await relationalProcurementFinanceDelete(resource, existingId);
  }
}
...
async function dedicatedCreate(
  resource: string,
  entityId: string,
  payload: Prisma.InputJsonValue,
) {
  ...
  const row = await delegate.create({
    data: { id: entityId, payload, ...writableRefs },
  });
  return toPayloadRows([row])[0];
}

async function dedicatedUpdate(
  resource: string,
  entityId: string,
  payload: Prisma.InputJsonValue,
) {
...
async function dedicatedReplaceAll(
  ...
  if (usesRelationalMaster(resource)) {
    const existingIdsByResource: Record<string, string[]> = {
      vendors: (
        await prisma.vendorRecord.findMany({ select: { id: true } })
      ).map((row) => row.id),
      customers: (
        await prisma.customerRecord.findMany({ select: { id: true } })
      ).map((row) => row.id),
    };
    const incomingIds = new Set(items.map((item) => item.entityId));
    for (const item of items) {
      const existing = await relationalMasterFindUnique(
        resource,
        item.entityId,
      );
      if (existing)
        await relationalMasterUpdate(resource, item.entityId, item.payload);
      else await relationalMasterCreate(resource, item.entityId, item.payload);
    }
    for (const existingId of existingIdsByResource[resource] || []) {
      if (!incomingIds.has(existingId))
        await relationalMasterDelete(resource, existingId);
    }
    return;
  }
  if (usesRelationalFleet(resource)) {
    ...
    const existingIds = (
      await prisma.fleetHealthEntry.findMany({ select: { id: true } })
    ).map((row) => row.id);
    const incomingIds = new Set(items.map((item) => item.entityId));
    for (const item of items) {
      const existing = await relationalFleetFindUnique(resource, item.entityId);
      if (existing)
        await relationalFleetUpdate(resource, item.entityId, item.payload);
      else await relationalFleetCreate(resource, item.entityId, item.payload);
    }
    for (const existingId of existingIds) {
      if (!incomingIds.has(existingId))
        await relationalFleetDelete(resource, existingId);
    }
    return;
  }
  if (usesRelationalFinanceMisc(resource)) {
      ...
      const validationRefs =
        resource === "surat-jalan"
          ? {
              ...refs,
              assetId:
                extractStringFromPayload(item.payload, "assetId") ?? null,
            }
          : refs;
      await assertDedicatedRelationsExist(resource, validationRefs);
    }
    ...
    items.map(async (i) => {
      ...
      const validationRefs =
        resource === "surat-jalan"
          ? {
              ...refs,
              assetId: extractStringFromPayload(i.payload, "assetId") ?? null,
            }
          : refs;
      await assertDedicatedRelationsExist(resource, validationRefs);
      const writableRefs = resource === "surat-jalan" ? {} : refs;
  ...
  for (const row of rowsWithRefs) {
    await delegate.upsert({
      ...
      update: {
        ...
        ...(resource === "surat-jalan"
          ? {}
          : extractDedicatedRelationRefs(resource, row.payload)),
      },
    });
  }
}
  ...
  switch (resource) {
    ...
    case "stock-outs":
      return {
        projectId: projectId ?? null,
        workOrderId:
          extractStringFromPayload(payload, "workOrderId", "noWorkOrder") ??
          null,
      };
    case "invoices":
      return {
    ...
    case "production-trackers":
      return {
        projectId: projectId ?? null,
        workOrderId:
          extractStringFromPayload(payload, "workOrderId", "woId") ?? null,
      };
    case "qc-inspections":
      return {
        projectId: projectId ?? null,
    ...
    case "berita-acara":
      return {
        projectId: projectId ?? null,
        suratJalanId:
          extractStringFromPayload(payload, "refSuratJalan") ?? null,
      };
    case "surat-jalan":
    ...
    case "finance-bank-reconciliations":
      return {
        projectId: projectId ?? null,
        invoiceId:
          extractStringFromPayload(payload, "invoiceId", "customerInvoiceId") ??
          null,
        vendorInvoiceId:
          extractStringFromPayload(payload, "vendorInvoiceId") ?? null,
      };
    case "finance-petty-cash-transactions":
      return {
        projectId: projectId ?? null,
    ...
    case "proof-of-delivery":
      return {
        projectId: projectId ?? null,
        suratJalanId:
          extractStringFromPayload(payload, "suratJalanId", "doId") ?? null,
        workOrderId:
          extractStringFromPayload(payload, "workOrderId", "woId") ?? null,
      };
    case "spk-records":
...
async function assertDedicatedRelationsExist(
  ...
  if (resource === "proof-of-delivery" && !refs.suratJalanId) {
    throw new PayloadValidationError(
      "proof-of-delivery: suratJalanId wajib diisi",
    );
  }
  ...
  if (refs.projectId) {
    // Internal production WOs still need a real project relation so that
    // LHP, QC, and inventory transactions remain durable in the database.
    if (refs.projectId === "INTERNAL") {
      await prisma.projectRecord.upsert({
        where: { id: "INTERNAL" },
        update: { namaProject: "INTERNAL PRODUKSI", status: "ACTIVE" },
        create: {
          id: "INTERNAL",
          namaProject: "INTERNAL PRODUKSI",
          kodeProject: "INTERNAL",
          status: "ACTIVE",
          payload: {
            id: "INTERNAL",
            namaProject: "INTERNAL PRODUKSI",
  ...
  if (refs.poId) {
    ...
    if (!po)
      throw new PayloadValidationError(
        `${resource}: poId '${refs.poId}' tidak ditemukan`,
      );
  ...
  if (refs.suratJalanId) {
    ...
    if (!suratJalan) {
      throw new PayloadValidationError(
        `${resource}: suratJalanId '${refs.suratJalanId}' tidak ditemukan`,
      );
    }
    suratJalanProjectId = suratJalan.projectId ?? null;
    if (
      refs.projectId &&
      suratJalan.projectId &&
      suratJalan.projectId !== refs.projectId
    ) {
      throw new PayloadValidationError(
  ...
  if (
    workOrderProjectId &&
    suratJalanProjectId &&
    workOrderProjectId !== suratJalanProjectId
  ) {
    throw new PayloadValidationError(
  ...
  if (refs.assetId) {
    const asset = await prisma.assetRecord.findUnique({
      where: { id: refs.assetId },
      select: { id: true, projectId: true },
    });
    if (!asset)
      throw new PayloadValidationError(
        `${resource}: assetId '${refs.assetId}' tidak ditemukan`,
      );
    if (
      refs.projectId &&
      asset.projectId &&
      asset.projectId !== refs.projectId
    ) {
      throw new PayloadValidationError(
  ...
  if (refs.vendorId) {
    const vendor = await prisma.vendorRecord.findUnique({
      where: { id: refs.vendorId },
      select: { id: true },
    });
    if (!vendor)
      throw new PayloadValidationError(
        `${resource}: vendorId '${refs.vendorId}' tidak ditemukan`,
      );
  }
  if (refs.customerId) {
    ...
    if (!customer)
      throw new PayloadValidationError(
        `${resource}: customerId '${refs.customerId}' tidak ditemukan`,
      );
  }
  if (refs.invoiceId) {
    ...
    if (!invoice)
      throw new PayloadValidationError(
        `${resource}: invoiceId '${refs.invoiceId}' tidak ditemukan`,
      );
    if (
      refs.projectId &&
      invoice.projectId &&
      invoice.projectId !== refs.projectId
    ) {
      throw new PayloadValidationError(
  ...
  if (refs.vendorInvoiceId) {
    ...
    if (!vendorInvoice)
      throw new PayloadValidationError(
        `${resource}: vendorInvoiceId '${refs.vendorInvoiceId}' tidak ditemukan`,
      );
    if (
      refs.projectId &&
      vendorInvoice.projectId &&
      vendorInvoice.projectId !== refs.projectId
    ) {
      throw new PayloadValidationError(
  ...
  if (refs.quotationId) {
    ...
    if (!quotation)
      throw new PayloadValidationError(
        `${resource}: quotationId '${refs.quotationId}' tidak ditemukan`,
      );
  }
  if (refs.employeeId) {
    ...
    if (!employee)
      throw new PayloadValidationError(
        `${resource}: employeeId '${refs.employeeId}' tidak ditemukan`,
      );
  }
  if (refs.templateId) {
    const template = await prisma.templateSuratRecord.findUnique({
      where: { id: refs.templateId },
      select: { id: true },
    });
    if (!template)
      throw new PayloadValidationError(
        `${resource}: templateId '${refs.templateId}' tidak ditemukan`,
      );
  }
  if (refs.updatedByUserId) {
    const user = await prisma.user.findUnique({
      where: { id: refs.updatedByUserId },
      select: { id: true },
    });
    if (!user)
      throw new PayloadValidationError(
        `${resource}: updatedByUserId '${refs.updatedByUserId}' tidak ditemukan`,
      );
  }
}
...
async function syncPurchaseOrderProgressFromStockIn(
  stockInPayload: Record<string, unknown>,
): Promise<void> {
  ...
  const normalizedNoPO = String(noPOFromPayload || "")
    .trim()
    .toLowerCase();
  ...
  if (relationalPo) {
    ...
    const quantityMaps =
      buildReceivedQuantitiesFromRelationalStockIns(linkedStockIns);
  ...
  if (!poRow && normalizedNoPO) {
    ...
    poRow =
      poRows.find(
        (row) =>
          String(asRecord(row.payload).noPO || "")
            .trim()
            .toLowerCase() === normalizedNoPO,
      ) || null;
  }
  ...
  const poNo = String(asTrimmedString(poPayload.noPO) || "")
    .trim()
    .toLowerCase();
  ...
  await dedicatedUpdate(
    "purchase-orders",
    poRow.id,
    normalizedPoPayload as Prisma.InputJsonValue,
  );
}

async function syncPurchaseOrderProgressFromReceiving(
  receivingPayload: Record<string, unknown>,
): Promise<void> {
  ...
  if (relationalPo) {
    ...
    const quantityMaps =
      buildReceivedQuantitiesFromRelationalReceivings(linkedReceivings);
    const updatedItems = applyReceivedQuantitiesToRelationalPoItems(
      relationalPo.items,
      quantityMaps,
    );
...
function registerDedicatedContractRoutes(basePath: string, resource: string) {
    ...
    async (req: AuthRequest, res: Response) => {
      ...
      try {
        ...
        return res.json(
          rows.map((row: any) =>
            toDedicatedContractPayload(
              row as { entityId: string; payload: unknown },
            ),
          ),
        );
      } catch (err) {
    ...
    async (req: AuthRequest, res: Response) => {
      ...
      if (!parsedParam.success) {
        return sendError(res, 400, {
          ...
          legacyError: "Invalid route params",
        });
      }
      if (!canReadDataResource(resource, req.user?.role)) {
        return sendError(res, 403, {
      ...
      try {
        const row = await dedicatedFindUnique(
          resource,
          parsedParam.data.entityId,
        );
        if (!row) {
          return sendError(res, 404, {
            code: "ENTITY_NOT_FOUND",
  ...
  dataRouter.post(
    ...
    authenticate,
    async (req: AuthRequest, res: Response) => {
      const parsedBody = dedicatedContractBodySchema(resource).safeParse(
        req.body,
      );
      if (!parsedBody.success) {
        return sendError(res, 400, {
          ...
          legacyError: parsedBody.error.flatten(),
        });
      }
      ...
      try {
        ...
        const created = await dedicatedCreate(
          resource,
          entityId,
        ...
        if (
          ...
          return sendError(res, 409, {
            ...
            legacyError: "Entity already exists",
          });
        }
        return sendError(res, 500, {
          code: "INTERNAL_ERROR",
  ...
  dataRouter.patch(
    ...
    async (req: AuthRequest, res: Response) => {
      ...
      try {
        ...
        const updated = await dedicatedUpdate(
          resource,
          entityId,
          payload as Prisma.InputJsonValue,
        );
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    const domain =
      typeof query.domain === "string" && query.domain.trim()
        ...
        : null;
    const resource =
      typeof query.resource === "string" && query.resource.trim()
        ? query.resource.trim()
        : null;
    const operation =
      typeof query.operation === "string" && query.operation.trim()
        ? query.operation.trim()
        : null;
    const limitRaw =
      typeof query.limit === "string" ? Number(query.limit) : NaN;
    ...
    try {
      ...
      const filtered = rows
        .filter((row) => {
          ...
          if (operation && String(row.operation || "") !== operation)
            return false;
          return true;
        })
...
dataRouter.put(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    const projectIds = Array.from(
      new Set(
        items.map((item) => asTrimmedString(item.projectId)).filter(Boolean),
      ),
    ) as string[];
    try {
      if (projectIds.length > 0) {
        const existingProjects = await prisma.projectRecord.findMany({
        ...
        if (invalidProjectId) {
          return sendError(res, 400, {
            ...
            message: `Project '${invalidProjectId}' not found`,
            legacyError: `Project '${invalidProjectId}' not found`,
          });
        }
      }
      await prisma.$transaction(async (tx) => {
      ...
      await writeDataAuditLog(req, "bulk-upsert", "assets", null, {
        count: items.length,
      });
      ...
      if (payload.projectId) {
        ...
        if (!project)
          return sendError(res, 400, {
            ...
            legacyError: `Project '${payload.projectId}' not found`,
          });
      }
...
dataRouter.patch(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      const existing = await prisma.assetRecord.findUnique({
      ...
      if (merged.projectId) {
        ...
        if (!project)
          return sendError(res, 400, {
            code: "PROJECT_NOT_FOUND",
            message: `Project '${merged.projectId}' not found`,
            legacyError: `Project '${merged.projectId}' not found`,
          });
      }
      const saved = await prisma.assetRecord.update({
        where: { id: req.params.id },
        data: merged,
      });
...
dataRouter.put(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    const assetIds = Array.from(
      new Set(
        items.map((item) => asTrimmedString(item.assetId)).filter(Boolean),
      ),
    ) as string[];
    const projectIds = Array.from(
      new Set(
    ...
    try {
      ...
      if (invalidAssetId)
        return sendError(res, 400, {
          ...
          message: `Asset '${invalidAssetId}' not found`,
          legacyError: `Asset '${invalidAssetId}' not found`,
        });
      const invalidProjectId = projectIds.find((id) => !projectSet.has(id));
      if (invalidProjectId)
        return sendError(res, 400, {
      ...
      await prisma.$transaction(async (tx) => {
        for (const item of items) {
          ...
          const asset = payload.assetId
            ? assetMap.get(payload.assetId)
            : undefined;
          if (payload.assetId && !asset)
            throw new PayloadValidationError(
              `maintenances: assetId '${payload.assetId}' tidak ditemukan`,
            );
          ...
          if (
            ...
            throw new PayloadValidationError(
              `maintenances: projectId '${payload.projectId}' tidak match dengan projectId asset '${asset.projectId}'`,
            );
          }
      ...
      await writeDataAuditLog(req, "bulk-upsert", "maintenances", null, {
        count: items.length,
      });
      return res.json({ message: "Synced", count: items.length });
    } catch (err) {
...
dataRouter.post(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      const asset = payload.assetId
        ? await prisma.assetRecord.findUnique({
      ...
      if (payload.assetId && !asset)
        return sendError(res, 400, {
          code: "ASSET_NOT_FOUND",
          message: `Asset '${payload.assetId}' not found`,
          legacyError: `Asset '${payload.assetId}' not found`,
        });
      const projectId = payload.projectId ?? asset?.projectId ?? null;
      if (
        payload.projectId &&
        ...
        return sendError(res, 400, {
          ...
          message: `Project '${payload.projectId}' mismatch with asset project '${asset.projectId}'`,
          legacyError: `Project '${payload.projectId}' mismatch with asset project '${asset.projectId}'`,
        });
      }
      if (projectId) {
        const project = await prisma.projectRecord.findUnique({
        ...
        if (!project)
          return sendError(res, 400, {
            ...
            legacyError: `Project '${projectId}' not found`,
          });
      }
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      const asset = merged.assetId
        ? await prisma.assetRecord.findUnique({
            where: { id: merged.assetId },
            select: { id: true, projectId: true, assetCode: true, name: true },
          })
        : null;
      if (merged.assetId && !asset)
        return sendError(res, 400, {
          code: "ASSET_NOT_FOUND",
          message: `Asset '${merged.assetId}' not found`,
          legacyError: `Asset '${merged.assetId}' not found`,
        });
      const projectId = merged.projectId ?? asset?.projectId ?? null;
      if (
        merged.projectId &&
        asset?.projectId &&
        merged.projectId !== asset.projectId
      ) {
        return sendError(res, 400, {
          code: "PROJECT_MISMATCH",
          message: `Project '${merged.projectId}' mismatch with asset project '${asset.projectId}'`,
          legacyError: `Project '${merged.projectId}' mismatch with asset project '${asset.projectId}'`,
        });
      }
      if (projectId) {
        const project = await prisma.projectRecord.findUnique({
...
dataRouter.put(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    const customerIds = Array.from(
      new Set(
        items.map((item) => asTrimmedString(item.customerId)).filter(Boolean),
      ),
    ) as string[];
    try {
      ...
      if (invalidCustomerId)
        return sendError(res, 400, {
          ...
          message: `Customer '${invalidCustomerId}' not found`,
          legacyError: `Customer '${invalidCustomerId}' not found`,
        });
      await prisma.$transaction(async (tx) => {
        ...
        for (const item of items) {
          ...
          const project = invoice.projectId
            ? projectMap.get(String(invoice.projectId))
            : undefined;
          if (
            invoice.customerId &&
            !customerSet.has(String(invoice.customerId))
          ) {
            throw new PayloadValidationError(
              `invoices: customerId '${invoice.customerId}' tidak ditemukan`,
            );
          }
          if (invoice.projectId && !project) {
            throw new PayloadValidationError(
              `invoices: projectId '${invoice.projectId}' tidak ditemukan`,
            );
          }
          if (
            invoice.projectId &&
            ...
            throw new PayloadValidationError(
              `invoices: customerId '${invoice.customerId}' tidak match dengan customer project '${project.customerId}'`,
            );
          }
          await ensureInvoiceReadyForBilling(invoice);
          await tx.invoiceRecord.upsert({
            where: { id: item.id },
            update: buildInvoiceRecordWriteData(
              invoiceData,
              project?.customerId ?? null,
            ),
            create: buildInvoiceRecordWriteData(
              invoiceData,
              project?.customerId ?? null,
            ),
          });
          ...
          if (invoiceItems.length > 0) {
            await tx.invoiceItem.createMany({
              data: buildInvoiceItemCreateManyData(item.id, invoiceItems),
            });
          }
        }
        await tx.invoiceRecord.deleteMany({
          where: { id: { notIn: Array.from(incomingIds) } },
        });
      });
      await writeDataAuditLog(req, "bulk-upsert", "invoices", null, {
        count: items.length,
      });
      return res.json({ message: "Synced", count: items.length });
    } catch (err) {
...
dataRouter.post(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      const normalized = sanitizeInvoicePayload(parsed.data.id, parsed.data);
      const { items: invoiceItems, ...invoiceData } = normalized;
      const project = normalized.projectId
        ? await prisma.projectRecord.findUnique({
            where: { id: String(normalized.projectId) },
            select: { id: true, customerId: true },
          })
        : null;
      if (normalized.projectId && !project)
        return sendError(res, 400, {
          code: "PROJECT_NOT_FOUND",
          message: `Project '${normalized.projectId}' not found`,
          legacyError: `Project '${normalized.projectId}' not found`,
        });
      if (normalized.customerId) {
        const customer = await prisma.customerRecord.findUnique({
          where: { id: String(normalized.customerId) },
          select: { id: true },
        });
        if (!customer)
          return sendError(res, 400, {
            code: "CUSTOMER_NOT_FOUND",
            message: `Customer '${normalized.customerId}' not found`,
            legacyError: `Customer '${normalized.customerId}' not found`,
          });
        if (
          project?.customerId &&
          project.customerId !== normalized.customerId
        ) {
          return sendError(res, 400, {
            code: "CUSTOMER_MISMATCH",
            message: `Customer '${normalized.customerId}' mismatch with project customer '${project.customerId}'`,
            legacyError: `Customer '${normalized.customerId}' mismatch with project customer '${project.customerId}'`,
          });
        }
      }
      ...
      const created = await prisma.$transaction(async (tx) => {
        const saved = await tx.invoiceRecord.create({
          data: buildInvoiceRecordWriteData(
            invoiceData,
            project?.customerId ?? null,
          ),
        });
        if (invoiceItems.length > 0) {
          await tx.invoiceItem.createMany({
            data: buildInvoiceItemCreateManyData(parsed.data.id, invoiceItems),
          });
        }
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      const saved = await prisma.$transaction(async (tx) => {
        ...
        if (invoiceItems.length > 0) {
          await tx.invoiceItem.createMany({
            data: buildInvoiceItemCreateManyData(req.params.id, invoiceItems),
          });
        }
        return tx.invoiceRecord.findUniqueOrThrow({
          where: { id: req.params.id },
...
dataRouter.get(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      const rows = await prisma.suratMasukRecord.findMany({
        orderBy: { updatedAt: "desc" },
      });
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        await tx.suratMasukRecord.deleteMany({
          where: { id: { notIn: Array.from(incomingIds) } },
        });
        await writeDataAuditLog(
          req,
...
dataRouter.post(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      const created = await prisma.$transaction(async (tx) => {
        const row = await tx.suratMasukRecord.create({ data: payload });
        await writeDataAuditLog(
          req,
          "create",
          "surat-masuk",
...
dataRouter.patch(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      const existing = await prisma.suratMasukRecord.findUnique({
        where: { id: req.params.id },
      });
      if (!existing)
        return sendError(res, 404, {
      ...
      const merged = sanitizeSuratMasukPayload(req.params.id, {
        ...mapSuratMasukRecord(existing),
        ...(req.body as Record<string, unknown>),
      });
      if (merged.projectId) {
        const project = await prisma.projectRecord.findUnique({
      ...
      const saved = await prisma.$transaction(async (tx) => {
        const row = await tx.suratMasukRecord.update({
          where: { id: req.params.id },
          data: merged,
        });
        await writeDataAuditLog(
...
dataRouter.delete(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      await prisma.$transaction(async (tx) => {
        await tx.suratMasukRecord.delete({ where: { id: req.params.id } });
        await writeDataAuditLog(
          req,
          "delete",
          "surat-masuk",
...
dataRouter.get(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      const rows = await prisma.templateSuratRecord.findMany({
        orderBy: { updatedAt: "desc" },
      });
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      await prisma.$transaction(async (tx) => {
        ...
        await tx.templateSuratRecord.deleteMany({
          where: { id: { notIn: Array.from(incomingIds) } },
        });
      });
      await writeDataAuditLog(req, "bulk-upsert", "template-surat", null, {
        count: items.length,
      });
      return res.json({ message: "Synced", count: items.length });
    } catch {
...
dataRouter.patch(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      const existing = await prisma.templateSuratRecord.findUnique({
        where: { id: req.params.id },
      });
      if (!existing)
        return sendError(res, 404, {
      ...
      const merged = sanitizeTemplateSuratPayload(req.params.id, {
        ...mapTemplateSuratRecord(existing),
        ...(req.body as Record<string, unknown>),
      });
      const saved = await prisma.templateSuratRecord.update({
        ...
        data: {
          ...merged,
          variables: merged.variables as Prisma.InputJsonValue,
        },
      });
...
dataRouter.get(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      const rows = await prisma.suratKeluarRecord.findMany({
        orderBy: { updatedAt: "desc" },
      });
...
dataRouter.put(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    const templateIds = Array.from(
      new Set(
        items.map((item) => asTrimmedString(item.templateId)).filter(Boolean),
      ),
    ) as string[];
    try {
      const [projects, templates] = await Promise.all([
        projectIds.length
          ...
          : Promise.resolve([] as Array<{ id: string }>),
        templateIds.length
          ? prisma.templateSuratRecord.findMany({
      ...
      if (invalidTemplateId)
        return sendError(res, 400, {
          ...
          message: `Template '${invalidTemplateId}' not found`,
          legacyError: `Template '${invalidTemplateId}' not found`,
        });
      await prisma.$transaction(async (tx) => {
        const incomingIds = new Set(items.map((item) => item.id));
        for (const item of items) {
        ...
        await tx.suratKeluarRecord.deleteMany({
          where: { id: { notIn: Array.from(incomingIds) } },
        });
        await writeDataAuditLog(
          req,
    ...
    try {
      ...
      if (payload.templateId) {
        ...
        if (!template)
          return sendError(res, 400, {
            code: "TEMPLATE_NOT_FOUND",
            message: `Template '${payload.templateId}' not found`,
            legacyError: `Template '${payload.templateId}' not found`,
          });
      }
      const created = await prisma.$transaction(async (tx) => {
        const row = await tx.suratKeluarRecord.create({ data: payload });
        await writeDataAuditLog(
          req,
          "create",
          "surat-keluar",
...
dataRouter.patch(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      const existing = await prisma.suratKeluarRecord.findUnique({
        where: { id: req.params.id },
      });
      if (!existing)
        return sendError(res, 404, {
      ...
      const merged = sanitizeSuratKeluarPayload(req.params.id, {
        ...mapSuratKeluarRecord(existing),
        ...(req.body as Record<string, unknown>),
      });
      if (merged.projectId) {
        const project = await prisma.projectRecord.findUnique({
      ...
      if (merged.templateId) {
        const template = await prisma.templateSuratRecord.findUnique({
          where: { id: merged.templateId },
          select: { id: true },
        });
        if (!template)
          return sendError(res, 400, {
            code: "TEMPLATE_NOT_FOUND",
            message: `Template '${merged.templateId}' not found`,
            legacyError: `Template '${merged.templateId}' not found`,
          });
      }
      const saved = await prisma.$transaction(async (tx) => {
        const row = await tx.suratKeluarRecord.update({
          where: { id: req.params.id },
          data: merged,
        });
        await writeDataAuditLog(
...
dataRouter.delete(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      await prisma.$transaction(async (tx) => {
        await tx.suratKeluarRecord.delete({ where: { id: req.params.id } });
        await writeDataAuditLog(
          req,
          "delete",
          "surat-keluar",
...
dataRouter.get(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      const rows = await prisma.appSettingRecord.findMany({
        orderBy: { updatedAt: "desc" },
      });
...
dataRouter.put(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    const userIds = Array.from(
      new Set(
        items
          .map((item) => asTrimmedString(item.updatedByUserId))
          .filter(Boolean),
      ),
    ) as string[];
    try {
      if (userIds.length > 0) {
        ...
        if (invalidUserId)
          return sendError(res, 400, {
            code: "USER_NOT_FOUND",
            message: `User '${invalidUserId}' not found`,
            legacyError: `User '${invalidUserId}' not found`,
          });
      }
      await prisma.$transaction(async (tx) => {
        const incomingIds = new Set(items.map((item) => item.id));
        ...
        await tx.appSettingRecord.deleteMany({
          where: { id: { notIn: Array.from(incomingIds) } },
        });
      });
      await writeDataAuditLog(req, "bulk-upsert", "app-settings", null, {
        count: items.length,
      });
      return res.json({ message: "Synced", count: items.length });
    } catch (err) {
...
dataRouter.post(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      const payload = sanitizeAppSettingRecordPayload(
        parsed.data.id,
        parsed.data,
      );
      if (payload.updatedByUserId) {
        const user = await prisma.user.findUnique({
          where: { id: payload.updatedByUserId },
          select: { id: true },
        });
        if (!user)
          return sendError(res, 400, {
            code: "USER_NOT_FOUND",
            message: `User '${payload.updatedByUserId}' not found`,
            legacyError: `User '${payload.updatedByUserId}' not found`,
          });
      }
...
dataRouter.patch(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      const existing = await prisma.appSettingRecord.findUnique({
        where: { id: req.params.id },
      });
      if (!existing)
        return sendError(res, 404, {
      ...
      const merged = sanitizeAppSettingRecordPayload(req.params.id, {
        ...mapAppSettingRecord(existing),
        ...(req.body as Record<string, unknown>),
      });
      if (merged.updatedByUserId) {
        const user = await prisma.user.findUnique({
          where: { id: merged.updatedByUserId },
          select: { id: true },
        });
        if (!user)
          return sendError(res, 400, {
            code: "USER_NOT_FOUND",
            message: `User '${merged.updatedByUserId}' not found`,
            legacyError: `User '${merged.updatedByUserId}' not found`,
          });
      }
      const saved = await prisma.appSettingRecord.update({
        where: { id: req.params.id },
        data: merged,
      });
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      await prisma.$transaction(async (tx) => {
        ...
        await tx.hrLeaveRecord.deleteMany({
          where: { id: { notIn: Array.from(incomingIds) } },
        });
      });
      await writeDataAuditLog(req, "bulk-upsert", "hr-leaves", null, {
        count: items.length,
      });
      return res.json({ message: "Synced", count: items.length });
    } catch (err) {
...
dataRouter.patch(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      const existing = await prisma.hrLeaveRecord.findUnique({
        where: { id: req.params.id },
      });
      if (!existing)
        return sendError(res, 404, {
...
dataRouter.get(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      const rows = await prisma.hrOnlineStatusRecord.findMany({
        orderBy: { updatedAt: "desc" },
      });
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      await prisma.$transaction(async (tx) => {
        ...
        await tx.hrOnlineStatusRecord.deleteMany({
          where: { id: { notIn: Array.from(incomingIds) } },
        });
      });
      await writeDataAuditLog(req, "bulk-upsert", "hr-online-status", null, {
        count: items.length,
      });
      return res.json({ message: "Synced", count: items.length });
    } catch (err) {
...
dataRouter.post(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      const payload = sanitizeHrOnlineStatusPayload(
        parsed.data.id,
        parsed.data,
      );
      const created = await prisma.hrOnlineStatusRecord.create({
        data: payload,
      });
...
dataRouter.patch(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      const existing = await prisma.hrOnlineStatusRecord.findUnique({
        where: { id: req.params.id },
      });
      if (!existing)
        return sendError(res, 404, {
      ...
      const merged = sanitizeHrOnlineStatusPayload(req.params.id, {
        ...mapHrOnlineStatusRecord(existing),
        ...(req.body as Record<string, unknown>),
      });
      const saved = await prisma.hrOnlineStatusRecord.update({
        where: { id: req.params.id },
        data: merged,
      });
...
dataRouter.delete(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      await prisma.hrOnlineStatusRecord.delete({
        where: { id: req.params.id },
      });
...
dataRouter.put(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    const employeeIds = Array.from(
      new Set(
        items.map((item) => asTrimmedString(item.employeeId)).filter(Boolean),
      ),
    ) as string[];
    try {
      const currentPayrolls = await prisma.payrollRecord.findMany({
        select: { id: true, year: true },
      });
      const incomingIds = new Set(items.map((item) => item.id));
      const payrollYears = new Set<number>([
        ...items.map((item) =>
          toFiniteNumber(item.year, new Date().getFullYear()),
        ),
        ...currentPayrolls
          .filter((row) => !incomingIds.has(row.id))
          .map((row) => row.year),
      ]);
      if (!(await ensureFinancialYearsOpen(res, payrollYears))) return;
      if (employeeIds.length > 0) {
        ...
        const invalidEmployeeId = employeeIds.find(
          (id) => !employeeSet.has(id),
        );
        if (invalidEmployeeId)
          return sendError(res, 400, {
            code: "EMPLOYEE_NOT_FOUND",
            message: `Employee '${invalidEmployeeId}' not found`,
            legacyError: `Employee '${invalidEmployeeId}' not found`,
          });
      }
      await prisma.$transaction(async (tx) => {
        for (const item of items) {
...
dataRouter.post(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      if (!(await ensureFinancialYearsOpen(res, [payload.year]))) return;
      if (payload.employeeId) {
        ...
        if (!employee)
          return sendError(res, 400, {
            code: "EMPLOYEE_NOT_FOUND",
            message: `Employee '${payload.employeeId}' not found`,
            legacyError: `Employee '${payload.employeeId}' not found`,
          });
      }
      const created = await prisma.$transaction(async (tx) => {
        const row = await tx.payrollRecord.create({ data: payload });
        await writeDataAuditLog(
          req,
          "create",
          "payrolls",
...
dataRouter.patch(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      const merged = sanitizePayrollPayload(req.params.id, {
        ...mapPayrollRecord(existing),
        ...(req.body as Record<string, unknown>),
      });
      if (!(await ensureFinancialYearsOpen(res, [existing.year, merged.year])))
        return;
      if (merged.employeeId) {
        const employee = await prisma.employeeRecord.findUnique({
          where: { id: merged.employeeId },
          select: { id: true },
        });
        if (!employee)
          return sendError(res, 400, {
            code: "EMPLOYEE_NOT_FOUND",
            message: `Employee '${merged.employeeId}' not found`,
            legacyError: `Employee '${merged.employeeId}' not found`,
          });
      }
      const saved = await prisma.$transaction(async (tx) => {
...
dataRouter.delete(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      if (!(await ensureFinancialYearsOpen(res, [existing.year]))) return;
      await prisma.$transaction(async (tx) => {
        await tx.payrollRecord.delete({ where: { id: req.params.id } });
        await writeDataAuditLog(
          req,
          "delete",
          "payrolls",
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    if (!parsedParam.success) {
      return sendError(res, 400, {
        ...
        legacyError: "Invalid resource name",
      });
    }
    ...
    if (isBlockedGenericResource(resource)) {
      return sendError(res, 403, {
        ...
        message: `Use dedicated endpoint for resource '${resource}'`,
        legacyError: `Use dedicated endpoint for resource '${resource}'`,
      });
    }
    if (!canReadDataResource(resource, req.user?.role)) {
      return sendError(res, 403, {
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      if (usesDedicatedResourceTable(resource)) {
        ...
        if (resource === "receivings") {
          await syncPurchaseOrderProgressFromReceiving(
            asRecord(created.payload),
          );
        }
        await writeDataAuditLog(req, "create", resource, entityId);
      ...
      if (!mediaCommitted) await removeStoredMediaUrls(newlyStoredMedia);
      if (err instanceof PayloadValidationError) {
        return sendError(res, 400, {
          code: err.code,
          message: err.message,
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      const updated = await prisma.$transaction(async (tx) => {
        await tx.$executeRaw`SELECT pg_advisory_xact_lock(4001, hashtext(${`${resource}:${entityId}`}))`;
        const changed = await tx.appEntity.updateMany({
          where: {
            resource,
            entityId,
        ...
        if (changed.count !== 1) throw new Error("ENTITY_VERSION_CONFLICT");
        await writeDataAuditLog(
          req,
          "update",
          resource,
        ...
        return tx.appEntity.findUniqueOrThrow({
          where: {
            resource_entityId: {
              resource,
              entityId,
      ...
      if (!mediaCommitted) await removeStoredMediaUrls(newlyStoredMedia);
      if (err instanceof Error && err.message === "ENTITY_VERSION_CONFLICT") {
        return sendError(res, 409, {
          code: "ENTITY_VERSION_CONFLICT",
          message:
            "Data berubah oleh pengguna lain. Muat ulang lalu coba lagi.",
          legacyError: "Entity version conflict",
        });
      }
      if (err instanceof PayloadValidationError) {
        return sendError(res, 400, {
