...
import { materializeMediaDataUrls, removeStoredMediaUrls } from "../utils/mediaStorage";
...
async function syncQcReleaseToWarehouse(entityId: string, record: Record<string, unknown>, tx?: Prisma.TransactionClient) {
  const db = tx ?? prisma;
  const workOrderId = asTrimmedString(record.workOrderId || record.woId || record.noWorkOrder);
  if (!workOrderId) return;
  const workOrder = await prisma.productionWorkOrder.findUnique({ where: { id: workOrderId } });
  if (!workOrder) return;
  ...
  await db.productionWorkOrder.update({
    ...
    data: {
      status: completed ? "Completed" : status === "Rejected" ? "In Progress" : "In Progress",
      completedQty: qtyPassed,
      workflowStatus: completed ? "QC_PASSED" : status === "Rejected" ? "QC_REJECTED" : "QC_PARTIAL",
    },
  });
  ...
  const shouldEnterWarehouse = record.sendToWarehouse === true && status === "Passed" && qtyPassed > 0;
  const existing = await db.inventoryStockIn.findUnique({ where: { id: stockInId } });
  if (!shouldEnterWarehouse) {
    if (existing?.status === "Draft") await prisma.inventoryStockIn.delete({ where: { id: stockInId } });
    return;
  }
  if (existing?.status === "Posted") return;

  const warehouseLocation = asTrimmedString(record.warehouseLocation) || "Gudang Barang Jadi";
  ...
  const legacyPayload = {
    ...
    items: [{ kode: itemCode, nama: itemName, kategori: stockCategory, qty: qtyPassed, satuan: "Unit", unit: "Unit", lokasi: warehouseLocation, batchNo: asTrimmedString(record.batchNo) || undefined }],
  ...
  const itemData = { id: `${stockInId}-ITEM-001`, itemCode, itemName, qty: qtyPassed, unit: "Unit", batchNo: asTrimmedString(record.batchNo) || undefined };
  if (existing) {
    await db.inventoryStockIn.update({ where: { id: stockInId }, data: { ...data, items: { deleteMany: {}, create: [itemData] } } });
  } else {
    await db.inventoryStockIn.create({ data: { id: stockInId, ...data, items: { create: [itemData] } } });
  }
}

async function recalculateWorkOrderAndProjectProgress(workOrderId?: string | null, tx?: Prisma.TransactionClient) {
  const db = tx ?? prisma;
  if (!workOrderId) return;
  ...
  const project = await db.projectRecord.findUnique({ where: { id: current.projectId } });
  if (!project) return;
  ...
  const totalTarget = projectWorkOrders.reduce((total, row) => total + Math.max(0, row.targetQty), 0);
  const totalCompleted = projectWorkOrders.reduce(
    (total, row) => total + Math.min(Math.max(0, row.completedQty), Math.max(0, row.targetQty)),
    0,
  );
  const progressAuto = totalTarget > 0 ? Math.round((totalCompleted / totalTarget) * 100) : 0;
...
async function relationalProductionCreate(resource: string, entityId: string, payload: Prisma.InputJsonValue) {
  ...
  switch (resource) {
    ...
    case "production-reports": {
      const workOrderId = asTrimmedString(record.workOrderId || record.woId) || undefined;
      const linkedWorkOrder = workOrderId
        ? await prisma.productionWorkOrder.findUnique({ where: { id: workOrderId }, select: { projectId: true } })
        : null;
      await prisma.productionExecutionReport.create({
        data: {
          id: entityId,
          projectId: linkedWorkOrder?.projectId || asTrimmedString(record.projectId) || "",
          workOrderId,
    ...
    case "qc-inspections": {
      ...
      await (prisma as any).productionQcInspection.create({
        data: {
          ...
          workflowStatus: record.sendToWarehouse === false ? "NO_WAREHOUSE" : "WAREHOUSE_PENDING",
          dimensions: {
            create: dimensions,
          },
        },
...
async function relationalProductionUpdate(resource: string, entityId: string, payload: Prisma.InputJsonValue) {
  ...
  switch (resource) {
    ...
    case "production-reports": {
      ...
      const workOrderId = asTrimmedString(record.workOrderId || record.woId) || null;
      const linkedWorkOrder = workOrderId
        ? await prisma.productionWorkOrder.findUnique({ where: { id: workOrderId }, select: { projectId: true } })
        : null;
  ...
  switch (resource) {
    ...
    case "vendor-invoices": {
      const rows = await prisma.financeVendorInvoice.findMany({
        orderBy: { updatedAt: "desc" },
        include: { payments: { orderBy: { tanggal: "asc" } }, approvalHistory: { orderBy: { actionAt: "asc" } } },
      });
...
async function relationalProcurementFinanceCreate(resource: string, entityId: string, payload: Prisma.InputJsonValue) {
  ...
  switch (resource) {
    ...
    case "vendor-invoices": {
      const invoiceNumber = asTrimmedString(record.noInvoiceVendor || record.noInvoice) || entityId;
      const supplierName = asTrimmedString(record.supplier || record.vendorName) || "";
      ...
      if (duplicate) {
        throw new Error(`vendor-invoices: invoice '${invoiceNumber}' dari '${supplierName}' sudah tercatat`);
      }
      ...
      if (purchaseOrderId) {
        ...
        if (!purchaseOrder) {
          throw new Error(`vendor-invoices: purchaseOrderId '${purchaseOrderId}' tidak ditemukan`);
        }
        if (!["APPROVED", "RECEIVED"].includes(purchaseOrder.status.trim().toUpperCase())) {
          throw new Error(`vendor-invoices: PO harus berstatus Approved atau Received`);
        }
      }
      ...
      await prisma.financeVendorInvoice.create({
        data: {
          ...
          payments: {
            create: (Array.isArray(record.paymentHistory) ? record.paymentHistory : []).map((raw) => {
          ...
          approvalHistory: {
            create: (Array.isArray(record.approvalHistory) ? record.approvalHistory : []).map((raw) => {
              ...
              return {
                ...
                actionAt: asTrimmedString(event.date) ? new Date(String(event.date)) : new Date(),
...
async function relationalProcurementFinanceUpdate(resource: string, entityId: string, payload: Prisma.InputJsonValue) {
  ...
  switch (resource) {
    ...
    case "vendor-invoices": {
      ...
      const purchaseOrderId = asTrimmedString(record.purchaseOrderId) || existingInvoice?.purchaseOrderId || null;
      const nextStatus = asTrimmedString(record.status) || existingInvoice?.status || "Unpaid";
      const nextPaidAmount = record.paidAmount === undefined
        ? Number(existingInvoice?.paidAmount || 0)
        : toFiniteNumber(record.paidAmount, 0);
      const requiresReceivedPO = ["Approved", "Partial", "Paid"].includes(nextStatus) || nextPaidAmount > 0;
      ...
      if (purchaseOrderId && requiresReceivedPO) {
        ...
        if (linkedPO.status !== "Received") {
          throw new Error(`vendor-invoices: tidak dapat approve atau membayar karena PO '${linkedPO.number}' belum Received`);
        }
      }
    ...
    if (refs.projectId === "INTERNAL") {
      await prisma.projectRecord.upsert({
        ...
        create: {
          ...
          status: "ACTIVE",
          payload: { id: "INTERNAL", namaProject: "INTERNAL PRODUKSI", kodeProject: "INTERNAL", status: "ACTIVE" },
        },
        select: { id: true },
      });
    }
...
dataRouter.put("/surat-masuk/bulk", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await writeDataAuditLog(req, "bulk-upsert", "surat-masuk", null, { count: items.length }, tx);
    });
    return res.json({ message: "Synced", count: items.length });
  } catch {
    return sendError(res, 500, { code: "INTERNAL_ERROR", message: "Internal server error", legacyError: "Internal server error" });
...
dataRouter.post("/surat-masuk", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const created = await prisma.$transaction(async (tx) => {
      ...
      await writeDataAuditLog(req, "create", "surat-masuk", row.id, undefined, tx);
      return row;
    });
...
dataRouter.patch("/surat-masuk/:id", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const saved = await prisma.$transaction(async (tx) => {
      const row = await tx.suratMasukRecord.update({ where: { id: req.params.id }, data: merged });
      await writeDataAuditLog(req, "update", "surat-masuk", row.id, undefined, tx);
      return row;
    });
...
dataRouter.delete("/surat-masuk/:id", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      await writeDataAuditLog(req, "delete", "surat-masuk", req.params.id, undefined, tx);
    });
    return res.status(204).send();
  } catch (err) {
    if (err instanceof Prisma.PrismaClientKnownRequestError && err.code === "P2025") {
...
dataRouter.put("/surat-keluar/bulk", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await writeDataAuditLog(req, "bulk-upsert", "surat-keluar", null, { count: items.length }, tx);
    });
    return res.json({ message: "Synced", count: items.length });
  } catch {
    return sendError(res, 500, { code: "INTERNAL_ERROR", message: "Internal server error", legacyError: "Internal server error" });
...
dataRouter.post("/surat-keluar", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const created = await prisma.$transaction(async (tx) => {
      ...
      await writeDataAuditLog(req, "create", "surat-keluar", row.id, undefined, tx);
      return row;
    });
...
dataRouter.patch("/surat-keluar/:id", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const saved = await prisma.$transaction(async (tx) => {
      const row = await tx.suratKeluarRecord.update({ where: { id: req.params.id }, data: merged });
      await writeDataAuditLog(req, "update", "surat-keluar", row.id, undefined, tx);
      return row;
    });
...
dataRouter.delete("/surat-keluar/:id", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      await writeDataAuditLog(req, "delete", "surat-keluar", req.params.id, undefined, tx);
    });
    return res.status(204).send();
  } catch (err) {
    if (err instanceof Prisma.PrismaClientKnownRequestError && err.code === "P2025") {
...
dataRouter.put("/payrolls/bulk", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    const currentPayrolls = await prisma.payrollRecord.findMany({ select: { id: true, year: true } });
    const incomingIds = new Set(items.map((item) => item.id));
    const payrollYears = new Set<number>([
      ...items.map((item) => toFiniteNumber(item.year, new Date().getFullYear())),
      ...currentPayrolls.filter((row) => !incomingIds.has(row.id)).map((row) => row.year),
    ]);
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await writeDataAuditLog(req, "bulk-upsert", "payrolls", null, { count: items.length }, tx);
    });
    return res.json({ message: "Synced", count: items.length });
  } catch {
    return sendError(res, 500, { code: "INTERNAL_ERROR", message: "Internal server error", legacyError: "Internal server error" });
...
dataRouter.post("/payrolls", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const created = await prisma.$transaction(async (tx) => {
      ...
      await writeDataAuditLog(req, "create", "payrolls", row.id, undefined, tx);
      return row;
    });
...
dataRouter.patch("/payrolls/:id", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (!(await ensureFinancialYearsOpen(res, [existing.year, merged.year]))) return;
    ...
    const saved = await prisma.$transaction(async (tx) => {
      const row = await tx.payrollRecord.update({ where: { id: req.params.id }, data: merged });
      await writeDataAuditLog(req, "update", "payrolls", row.id, undefined, tx);
      return row;
    });
...
dataRouter.delete("/payrolls/:id", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    const existing = await prisma.payrollRecord.findUnique({ where: { id: req.params.id }, select: { year: true } });
    if (!existing) return sendError(res, 404, { code: "NOT_FOUND", message: "Not found", legacyError: "Not found" });
    if (!(await ensureFinancialYearsOpen(res, [existing.year]))) return;
    await prisma.$transaction(async (tx) => {
      await tx.payrollRecord.delete({ where: { id: req.params.id } });
      await writeDataAuditLog(req, "delete", "payrolls", req.params.id, undefined, tx);
    });
    return res.status(204).send();
  } catch (err) {
    if (err instanceof Prisma.PrismaClientKnownRequestError && err.code === "P2025") {
...
dataRouter.post("/data/:resource", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const created = await prisma.$transaction(async (tx) => {
      const row = await tx.appEntity.create({ data: {
        resource,
        entityId,
      ...
      await writeDataAuditLog(req, "create", resource, entityId, undefined, tx);
      return row;
    });
    mediaCommitted = true;
    if (resource === "stock-ins") {
...
dataRouter.patch("/data/:resource/:entityId", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      const changed = await tx.appEntity.updateMany({ where: {
        resource,
        entityId,
      ...
      await writeDataAuditLog(req, "update", resource, entityId, undefined, tx);
      return tx.appEntity.findUniqueOrThrow({ where: {
        resource_entityId: {
          resource,
          entityId,
        },
    ...
    if (err instanceof Error && err.message === "ENTITY_VERSION_CONFLICT") {
      return sendError(res, 409, { code: "ENTITY_VERSION_CONFLICT", message: "Data berubah oleh pengguna lain. Muat ulang lalu coba lagi.", legacyError: "Entity version conflict" });
    }
    if (err instanceof PayloadValidationError) {
      return sendError(res, 400, {
        code: err.code,
