...
function isBlockedGenericResource(resource: string): boolean {
  return BLOCKED_GENERIC_READ_RESOURCES.has(resource);
}

function isBlockedGenericWriteResource(resource: string): boolean {
...
const RELATIONAL_MASTER_RESOURCES = new Set([
  "vendors",
  "customers",
]);
...
function usesDedicatedResourceTable(resource: string): boolean {
  return Object.prototype.hasOwnProperty.call(DEDICATED_RESOURCE_DELEGATES, resource);
}

function usesRelationalInventoryRead(resource: string): boolean {
  return RELATIONAL_INVENTORY_READ_RESOURCES.has(resource);
}

function usesRelationalLogisticsDocs(resource: string): boolean {
  return RELATIONAL_LOGISTICS_DOC_RESOURCES.has(resource);
}

function usesRelationalProduction(resource: string): boolean {
  return RELATIONAL_PRODUCTION_RESOURCES.has(resource);
}

function usesRelationalProcurementFinance(resource: string): boolean {
  return RELATIONAL_PROCUREMENT_FINANCE_RESOURCES.has(resource);
}

function usesRelationalFinanceMisc(resource: string): boolean {
  return RELATIONAL_FINANCE_MISC_RESOURCES.has(resource);
}

function usesRelationalFleet(resource: string): boolean {
  return RELATIONAL_FLEET_RESOURCES.has(resource);
}

function usesRelationalMaster(resource: string): boolean {
...
function getDedicatedDelegate(resource: string): DedicatedDelegate | null {
...
async function dedicatedFindMany(resource: string) {
  ...
  const delegate = getDedicatedDelegate(resource);
  if (!delegate) return [];
...
function canWriteDataResource(resource: string, role?: Role): boolean {
...
function canReadDataResource(resource: string, role?: Role): boolean {
  if (!role) return false;
...
function canViewAuditLogs(role?: Role): boolean {
...
const WORKFLOW_TRANSITIONS: Record<string, Record<string, string[]>> = {
  "work-orders": {
    // UI allows direct status jumps from Draft -> In Progress/QC/Completed.
...
function validateWorkflowStatusWrite(params: {
  resource: string;
  payload: unknown;
  role?: Role;
...
async function writeDataAuditLog(
  req: AuthRequest,
