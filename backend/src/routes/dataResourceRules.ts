...
// Hanya OWNER/ADMIN/SPV/MANAGER yang trusted melewati matrix.
// Role lain (termasuk enum utama) dibatasi lewat DATA_WRITE_ROLES_BY_RESOURCE
// + ROLE_ALIASES supaya aksesnya sesuai fungsi kerja masing-masing.
...
const ROLE_ALIASES: Partial<Record<Role, Role[]>> = {
  ...
  OPERATIONS: ["PRODUKSI"],
  // Role enum utama dipetakan ke singkatan yang dipakai matrix.
  FINANCE_ACCOUNTING: ["FINANCE"],
  SALES_MARKETING: ["SALES"],
  HR: ["HR"],
  OPERATIONAL_PRODUCTION: ["PRODUKSI", "SUPPLY_CHAIN", "WAREHOUSE"],
  MANAGER: ["OWNER"],
};
...
export function canReadDataResource(resource: string, role?: Role): boolean {
  if (!role) return false;
  // Read tetap terbuka untuk semua role ter-autentikasi; matrix hanya membatasi tulis.
  return true;
}
