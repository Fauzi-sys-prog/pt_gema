...
import { quotationSellingOnly } from "../utils/quotationSellingOnly";
...
function toQuotationMeta(item: Record<string, unknown>) {
  return {
    ...
    totalSelling: readNumber(item, "totalSelling"),
    discount: readNumber(item, "discount"),
    ppnPercent: readNumber(item, "ppnPercent"),
    ppnAmount:
      readNumber(item, "ppnAmount") ??
      (readBoolean(item, "includePpn")
        ? (readNumber(item, "grandTotal") || 0) * ((readNumber(item, "ppnPercent") || 0) / 100)
        : 0),
    grandTotal: readNumber(item, "grandTotal"),
    grandTotalWithTax:
      readNumber(item, "grandTotalWithTax") ??
      ((readNumber(item, "grandTotal") || 0) +
        (readBoolean(item, "includePpn")
          ? (readNumber(item, "grandTotal") || 0) * ((readNumber(item, "ppnPercent") || 0) / 100)
          : 0)),
    internalApprovalStatus: readString(item, "internalApprovalStatus"),
    internalApprovedBy: readString(item, "internalApprovedBy"),
    internalApprovedAt: readDate(item, "internalApprovedAt"),
    internalRejectReason: readString(item, "internalRejectReason"),
    clientApprovalStatus: readString(item, "clientApprovalStatus"),
    clientApprovedAt: readDate(item, "clientApprovedAt"),
    clientRejectReason: readString(item, "clientRejectReason"),
    revisionNo: Math.max(0, Math.trunc(readNumber(item, "revisionNo") || 0)),
    convertedToProject: readBoolean(item, "convertedToProject"),
    createdBy: readString(item, "createdBy"),
    dataCollectionId: readString(item, "dataCollectionId"),
  };
}

async function syncQuotationDetails(
  tx: Prisma.TransactionClient,
  quotationId: string,
  payload: Record<string, unknown>,
) {
  const sections = Array.isArray(payload.sections) ? payload.sections.map(asRecord) : [];

  await tx.quotationSection.deleteMany({ where: { quotationId } });
  for (const [sectionPosition, section] of sections.entries()) {
    const sourceSectionId = readString(section, "id") || String(sectionPosition + 1);
    const sectionId = `${quotationId}:section:${sourceSectionId}`;
    ...
    await tx.quotationSection.create({
      data: {
        ...
        name: readString(section, "nama") || readString(section, "title") || readString(section, "label") || `Section ${sectionPosition + 1}`,
        label: readString(section, "label"),
        subtotal: readNumber(section, "subtotal") || items.reduce((sum, item) => sum + (readNumber(item, "hargaJualUnit") || readNumber(item, "hargaJual") || 0) * (readNumber(item, "qty") || readNumber(item, "jumlah") || 0), 0),
        payload: section as Prisma.InputJsonValue,
        items: {
          create: items.map((item, itemPosition) => {
            const qty = readNumber(item, "qty") || readNumber(item, "jumlah") || 0;
            const sellingPerUnit = readNumber(item, "hargaJualUnit") || readNumber(item, "hargaJual") || readNumber(item, "sellingPerUnit") || 0;
            return {
              id: `${sectionId}:item:${readString(item, "id") || itemPosition + 1}`,
              position: itemPosition,
              description: readString(item, "keterangan") || readString(item, "description") || `Item ${itemPosition + 1}`,
              subDescription: readString(item, "subKeterangan") || readString(item, "subDescription"),
              qty,
              unit: readString(item, "satuan") || readString(item, "unit"),
              sellingPerUnit,
              discount: readNumber(item, "disc") || readNumber(item, "discount") || 0,
              totalSelling: qty * sellingPerUnit,
  ...
  const revisions = Array.isArray(payload.revisionHistory) ? payload.revisionHistory.map(asRecord) : [];
  for (const revision of revisions) {
    const revisionNo = Math.trunc(readNumber(revision, "no") || readNumber(revision, "revisionNo") || 0);
    if (revisionNo <= 0) continue;
    await tx.quotationRevision.upsert({
      where: { quotationId_revisionNo: { quotationId, revisionNo } },
      update: {
        trigger: readString(revision, "trigger"),
        reason: readString(revision, "reason"),
        revisedBy: readString(revision, "by"),
        grandTotal: readNumber(revision, "grandTotal"),
        snapshot: revision as Prisma.InputJsonValue,
      },
      create: {
        quotationId,
        revisionNo,
...
function approvalStageAction(
  stage: "INTERNAL" | "CLIENT",
  status: string
): QuotationApprovalAction | null {
  const normalized = status.toUpperCase();
  if (normalized === "APPROVED") return `${stage}_APPROVE` as QuotationApprovalAction;
  if (normalized === "REJECTED") return `${stage}_REJECT` as QuotationApprovalAction;
  if (normalized === "REVISION") return `${stage}_REVISION` as QuotationApprovalAction;
  return null;
}
...
function normalizeWorkflowStatus(value: unknown): WorkflowStatus {
  ...
  if (raw === "SENT" || raw === "PENDING APPROVAL") return "Sent";
  if (raw === "REVIEW" || raw === "REVIEW_SPV" || raw === "PENDING REVIEW") return "Review";
  if (raw === "APPROVED" || raw === "IN PROGRESS" || raw === "COMPLETE") return "Approved";
...
async function generateFinalNoPenawaran(excludeId: string, tanggal: string | null): Promise<string> {
  ...
  let maxSeq = 0;
  // Support nomor lama (001/PEN/GMT/VIII/2026) dan format quotation saat ini
  // (001/A/PEN/GTP/VIII/2026), lalu jadikan satu nomor urut per tahun.
  const pattern = /^(\d{1,4})(?:\/[A-Z]+)?\/PEN\/(?:GMT|GTP)\/([IVXLC]+)\/(\d{4})$/i;
  const allRows = [
  ...
  return `${next}/A/PEN/GTP/${monthRoman}/${year}`;
}
...
async function createQuotationApprovalLogSafe(input: {
  ...
  try {
    await db.quotationApprovalLog.create({
      data: {
...
function normalizeQuotationPayload(id: string, payload: unknown): QuotationPayload {
  const shaped = ensureQuotationPayloadShape(id, quotationSellingOnly(payload));
...
function buildBoqRowsFromQuotation(quotationPayload: Record<string, unknown>): Record<string, unknown>[] {
  ...
  for (const group of groups) {
    for (const item of toArrayOfRecords(pricingItems[group.key])) {
      ...
      const unitPrice = 0; // Project budget must be supplied in BOQ, never from selling prices.
      const totalCost = 0;
      rows.push({
  ...
  if (rows.length === 0 && Array.isArray(quotationPayload.sections)) {
    for (const sectionValue of quotationPayload.sections) {
      ...
      for (const item of toArrayOfRecords(section.items)) {
        ...
        rows.push({
          ...
          materialName:
            ...
            `Item ${idx}`,
          qtyEstimate: qty,
          unit: readString(item, "satuan") || readString(item, "unit") || "Unit",
          unitPrice,
          totalCost: qty * unitPrice,
          hargaJualUnit: toNumber(item.hargaJualUnit, 0) || toNumber(item.hargaJual, 0),
...
async function upsertProjectFromQuotation(
  payload: QuotationPayload,
  db?: Prisma.TransactionClient,
): Promise<void> {
  const run = async (database: Prisma.TransactionClient) => {
  ...
  const rows = await database.projectRecord.findMany({
    select: {
      id: true,
      quotationId: true,
      approvalStatus: true,
  ...
  const projectRecordMeta = toProjectRecordMeta(projectId, projectPayload, quotationId);
    ...
    await database.appEntity.upsert({
      where: {
        resource_entityId: {
...
quotationsRouter.put("/quotations/bulk", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    await writeAuditLog(req, "bulk-upsert", "quotations", null, { count: items.length });
...
quotationsRouter.post("/quotations", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  // Nomor quotation harus ditentukan server dari database, bukan dari jumlah
  // data di browser. Ini mencegah P2002 saat data legacy/dua user membuat
  // quotation pada waktu yang berdekatan.
  ...
  if (nextStatus === "Approved") {
    ...
    if (String(item.noPenawaran || "").startsWith("AUTO/QUO/")) {
      item.noPenawaran = await generateFinalNoPenawaran(item.id, readString(item, "tanggal"));
    }
  }
  ...
  try {
    ...
    const savedPayload = await prisma.$transaction(async (tx) => {
      ...
      await syncQuotationDetails(tx, hydrated.id, hydrated);
      ...
      if (hydrated.convertedToProject === true) {
        await upsertProjectFromQuotation(hydrated, tx);
      }
      await createQuotationApprovalLogSafe({
        quotationId: hydrated.id,
        ...
        metadata: buildAuditMetadata(req, {
          ...
          noPenawaran: readString(hydrated, "noPenawaran"),
          dataCollectionId: readString(hydrated, "dataCollectionId"),
          sourceType: readString(hydrated, "sourceType"),
        }),
      }, tx);
    ...
    await writeAuditLog(req, "create", "quotations", savedPayload.id, { noQuotation: savedPayload.noQuotation });
    ...
    if (err instanceof Prisma.PrismaClientKnownRequestError && err.code === "P2002") {
      const target = Array.isArray(err.meta?.target) ? err.meta.target.join(", ") : String(err.meta?.target || "data unik");
      return sendError(res, 409, {
        code: "DUPLICATE_QUOTATION_DATA",
        message: `Data quotation duplikat pada: ${target}`,
        legacyError: `Data quotation duplikat pada: ${target}`,
      });
    }
...
quotationsRouter.patch("/quotations/:id", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (readBoolean(existingPayload, "convertedToProject") === true) {
      return sendError(res, 409, {
        code: "QUOTATION_LOCKED",
        message: "Quotation sudah dikonversi menjadi project dan tidak dapat diubah",
        legacyError: "Quotation sudah dikonversi menjadi project dan tidak dapat diubah",
      });
    }
    const actor = await resolveActorSnapshot(req.user?.id, req.user?.role);
    const previousStatus = normalizeWorkflowStatus(existingPayload.status);
    const previousInternalApproval = readString(existingPayload, "internalApprovalStatus") || "Pending";
    const previousClientApproval = readString(existingPayload, "clientApprovalStatus") || "Pending";
    ...
    if (nextStatus === "Approved" && previousStatus !== "Approved") {
      ...
      if (String(normalizedMerged.noPenawaran || "").startsWith("AUTO/QUO/")) {
    ...
    const savedPayload = await prisma.$transaction(async (tx) => {
      ...
      const nextInternalApproval = readString(hydrated, "internalApprovalStatus") || "Pending";
      if (previousInternalApproval !== nextInternalApproval) {
        const action = approvalStageAction("INTERNAL", nextInternalApproval);
        if (action) await createQuotationApprovalLogSafe({
          quotationId: id, action, actorUserId: req.user?.id || null, actorRole: req.user?.role || null,
          fromStatus: previousInternalApproval, toStatus: nextInternalApproval,
          reason: readString(hydrated, "internalRejectReason"),
          metadata: buildAuditMetadata(req, { stage: "internal", actorName: actor.actorName }),
        }, tx);
      }
      const nextClientApproval = readString(hydrated, "clientApprovalStatus") || "Pending";
      if (previousClientApproval !== nextClientApproval) {
        const action = approvalStageAction("CLIENT", nextClientApproval);
        if (action) await createQuotationApprovalLogSafe({
          quotationId: id, action, actorUserId: req.user?.id || null, actorRole: req.user?.role || null,
          fromStatus: previousClientApproval, toStatus: nextClientApproval,
          reason: readString(hydrated, "clientRejectReason") || readString(hydrated, "clientNote"),
          metadata: buildAuditMetadata(req, { stage: "client", actorName: actor.actorName }),
        }, tx);
      }
    ...
    await writeAuditLog(req, "update", "quotations", savedPayload.id, { noQuotation: savedPayload.noQuotation });
...
quotationsRouter.delete("/quotations/:id", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    await writeAuditLog(req, "delete", "quotations", id);
