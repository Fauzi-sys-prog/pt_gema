...
function registerResourceRoutes(basePath: string, resource: string) {
  ...
  hrRouter.post(basePath, authenticate, async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      if (resource === "employees") {
        const employeePayload = sanitizeEmployeePayload(payload.id, payload);
        const duplicate = await prisma.employeeRecord.findFirst({
          where: { employeeId: employeePayload.employeeId, NOT: { id: payload.id } },
          select: { id: true },
        });
        if (duplicate) {
          return sendError(res, 409, { code: "EMPLOYEE_ID_EXISTS", message: "Employee ID already exists", legacyError: "Employee ID already exists" });
        }
        const row = await prisma.$transaction(async (tx) => {
          const created = await tx.employeeRecord.create({ data: employeePayload });
          await writeAuditLog(req, "create", resource, payload.id, undefined, tx);
          return created;
        });
  ...
  hrRouter.patch(`${basePath}/:id`, authenticate, async (req: AuthRequest, res: Response) => {
    ...
    try {
      if (resource === "employees") {
        ...
        const employeePayload = sanitizeEmployeePayload(id, merged);
        const duplicate = await prisma.employeeRecord.findFirst({
          where: { employeeId: employeePayload.employeeId, NOT: { id } },
          select: { id: true },
        });
        if (duplicate) {
          return sendError(res, 409, { code: "EMPLOYEE_ID_EXISTS", message: "Employee ID already exists", legacyError: "Employee ID already exists" });
        ...
        const saved = await prisma.$transaction(async (tx) => {
          const updated = await tx.employeeRecord.update({ where: { id }, data: employeePayload });
  ...
  hrRouter.delete(`${basePath}/:id`, authenticate, async (req: AuthRequest, res: Response) => {
    ...
    try {
      await prisma.$transaction(async (tx) => {
        if (resource === "employees") {
          await tx.employeeRecord.delete({ where: { id } });
        } else if (resource === "attendances") {
