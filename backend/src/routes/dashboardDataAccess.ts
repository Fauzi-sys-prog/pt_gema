...
export type DashboardPayloadRow = {
  entityId: string;
  payload: unknown;
  updatedAt: Date;
};
...
export async function loadDashboardWorkflowRows(resource: string): Promise<FinanceQueueRow[]> {
...
export async function loadDashboardPayloadRows(resource: string): Promise<DashboardPayloadRow[]> {
  return loadDashboardWorkflowRows(resource);
}

export async function findFinanceResourceDoc(
  resource: string,
  entityId: string
): Promise<FinanceResourceDoc | null> {
  if (resource === "purchase-orders") {
...
export async function updateFinanceResourceDoc(
  resource: string,
  entityId: string,
  source: "app" | "dedicated",
  payload: Record<string, unknown>
