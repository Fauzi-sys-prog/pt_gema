...
import {
  ...
  buildExecutiveDashboardSummaryPayload,
...
dashboardRouter.get("/dashboard/executive-summary", authenticate, async (req: AuthRequest, res: Response) => {
  if (!canReadFinance(req.user?.role)) {
    return sendError(res, 403, { code: "FORBIDDEN", message: "Forbidden", legacyError: "Forbidden" });
  }
  try {
    return res.json(await buildExecutiveDashboardSummaryPayload());
  } catch {
...
dashboardRouter.post("/dashboard/finance-approval-action", authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const execute = (db: Prisma.TransactionClient | typeof prisma = prisma) => executeFinanceApprovalAction({
      input,
      role: req.user?.role,
      userId: req.user?.id ?? null,
      actor,
      ...
      syncProjectFromQuotation: (params) => upsertProjectFromQuotationForApprovalSync(params, db),
