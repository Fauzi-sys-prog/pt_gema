...
export async function removeStoredMediaUrl(url: string): Promise<void> {
  const uploadMarker = "/uploads/";
  const markerIndex = url.indexOf(uploadMarker);
  if (markerIndex < 0) return;
  const relativePath = url.slice(markerIndex + uploadMarker.length);
  const uploadRoot = path.resolve(env.uploadDir);
  const absoluteFile = path.resolve(uploadRoot, relativePath);
  if (absoluteFile !== uploadRoot && absoluteFile.startsWith(`${uploadRoot}${path.sep}`)) {
    await fs.unlink(absoluteFile).catch((error: NodeJS.ErrnoException) => {
...
export async function materializeMediaDataUrls(params: {
  ...
  for (const rule of rules) {
    ...
    if (!stored) continue;
    params.storedUrls?.push(stored.url);
...
export async function removeStoredMediaUrls(urls: Iterable<string>): Promise<void> {
  await Promise.all(Array.from(urls, (url) => removeStoredMediaUrl(url).catch(() => undefined)));
}
